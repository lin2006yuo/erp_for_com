<template>
    <page-dialog size="full"
        ...
        <import-excel v-model="showImport" :warehouseId="warehouse_id" @files-data="files_data" :select-mark="selectMark"></import-excel>
        <batch-remark
    ...
    export default {
        ...
        methods: {
                ...
                this.goodsList.forEach(data => {
                    if (this.supplierId === data.id) {
                        if (data.supplier === checkSupplier[0].company_name) {
                            ...
                            this.purchase_prices[data.sku_id] = checkSupplier[0].audited_price;
                            this.new_currency = checkSupplier[0].audited_currency_code;
                            data.min_qty = checkSupplier[0].min_qty; //供应商变化最小起订数 跟着变
                        }
                    }
                });
            },
            ...
            save(isApproval) {
                ...
                if (!!min_qty && this.warehouse_id !== 550) {
                    this.$reqKey('url_add_plan_stocking', false);
                ...
                let summary = this.$refs.editModule.map(row => {
                    ...
                    return {
                        ...
                        supply_chain_finance: row.tableData.supply_chain_finance,
                        invoice: row.tableData.invoice,
                        tax_rate: row.tableData.tax_rate,
                    }
                });
                ...
                this.filterLists.forEach(row => {
                    if (this.selectMark === 'normal_purchasing') {
                        if (row.goods.some(item => Number(item.purchase_price) === 0)) {
                            isZero = true
                        }
                    } else if (this.selectMark === 'purchase_order_overcharged') {
                ...
                this.goodsList.forEach(goods => {
                    ...
                    if(!ret[key]){
                        this.$set(ret, key, {
                            ...
                            goods: [],
                            attachment: [],
                            supply_chain_finance: goods.supply_chain_finance,
                            invoice: goods.invoice,
                            tax_rate: goods.tax_rate,
                            select_option: goods.select_option,
                            purchase_plan_type: this.selectMark
                        })
                    }
