    ...
    <el-row class="c-return">
        <rainbow-header title="退货" background-color="#008CD7">
            <div class="fr mt-mini mr-sm" slot="header-right">
                <super-select v-model="form.list.mod_return"
                              :disabled="returnList.length<=0"
                              @change="change_return"
                              :placeholder="placeHolder"
                              class="inline width-lg"
                              :options="returnList"
                              storage-key="ebayReturnListDetails">
                </super-select>
        ...
        methods:{
            ...
            change_return(val){
                ...
                this.$http(api_edit_combList,params).then(res=>{
                    this.form.set.return_policy = res.return_policy;
                    this.form.set.return_type = res.return_type;
                    this.form.list.return_time = res.return_time;
                    this.form.set.restocking_fee_code = res.restocking_fee_code;
                    this.form.set.extended_holiday = res.extension===1?true:false;
                    this.form.set.return_shipping_option = res.return_shipping_option;
                    this.form.set.return_description = res.return_description;
                }).catch(code=>{
                        console.log(code);
                })
            },
