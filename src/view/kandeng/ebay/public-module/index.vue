    ...
    <style lang="stylus" scoped>
        .public-module .ebay-kandeng-card{
    ...
    <script>
        ...
        import {url_save_common_model,url_save_combList,url_delete_promotion,url_delete_model,url_delete_trans} from '../../../../api/ebay-kandeng-public-module';
        ...
        export default{
            ...
            computed:{
                has(){
                    return this.$hasPermission(url_save_combList);
                },
                showLine(){
...
//                            {inter:1,trans_code:"",cost:"",add_cost:"",extra_cost:"",is_all_location:false,location:this.get_local(this.locationList)},
                ...
                get_common_modeList(){
                    let params = {
                        ...
                        moduleName:this.params.name.trim(),
                ...
                edit_model(row){
                    ...
                    if(row.type!=='trans'){
                        this.$http(api_edit_combList,params).then(res=>{
                            if(!!res.extension){res.extension = res.extension?true:false}
                            if(!!res.local_pickup){res.local_pickup = res.local_pickup?true:false}
                            if(!!res.auto_pay){res.auto_pay = res.auto_pay?true:false}
                            if(!!res.pay_method){res.pay_method = JSON.parse(res.pay_method)}
                            if(!!res.link_paypal){res.link_paypal = res.link_paypal?true:false}
                            if(!!res.registration){res.registration = res.registration?true:false}
                            if(!!res.violations){res.violations = res.violations?true:false}
                            if(!!res.strikes){res.strikes = res.strikes?true:false}
                            if(!!res.credit){res.credit = res.credit?true:false}
                            if(!!res.requirements){res.requirements = res.requirements?true:false}
                            if(!!res.minimum_feedback){res.minimum_feedback = res.minimum_feedback?true:false}
                            if(!!res.exclude){res.exclude = JSON.parse(res.exclude)}
                            if(!!res.quantity) res.quantity = String(res.quantity);
                            this.modelType = res.model_type;
                            if(!!res.trans){
                                res.trans.forEach(res=>{
                                    if(res.location.includes("Worldwide")){
                                        res.location=this.get_local(this.locationList)
                            ...
                            this.add_edit_form = res;
                            let curForm = window.clone(res);
                            this.$nextTick(()=>{
