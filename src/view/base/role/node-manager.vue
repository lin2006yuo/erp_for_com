...
<script>
    ...
    export default{
        ...
        async mounted(){
            ...
            this.mcanodes.forEach(item => {
                 this.moduleId[item.id] = item.module;  // 每一个id对应着一个小模块。不会有重复的id.
            });
        },
        methods: {
            open() {
               if (this.activeName === "") {
                   this.activeName = "用户系统"
               }
            },
            setting(node){
                this.node = node;
            ...
            handleData(name, obj) {
                ...
                if (!ret[name]) {
                    ...
                    ret[str] = clone(obj[str]);
                } else {
                    ret[name] = clone(obj[name]);
                }
                return ret;
            }
        },
            ...
            modules(){
                ...
                this.mcanodes.forEach(mca => {
                    ...
                    if(this.filter){
                        switch (this.filter){
                            case 1:
                                if(this.is_relate(mca) && this.relate_state(mca)){  // 关联的api包含这一个name的时候，
                                    break;
                                }
                ...
                if (!this.activeName) {
                    ret = this.handleData(this.activeName, ret);
                } else {
                     ret = this.handleData(this.activeName, ret);
                }
                return ret;
        ...
        watch: {
            ...
            role:{
                deep:true,
                handler(){
                    // 一旦获获取的角色发生变化的话，就去取这个角色下面选中的id数据。
                    this.selectes = {};
                    /** 获取这个角色下面所有选中的api */
