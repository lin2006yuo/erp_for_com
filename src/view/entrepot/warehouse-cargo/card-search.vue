<template>
    <div class="c-card-search">
        <label-item label="请选择仓库：" class="mb-xs mt-sm">
            <super-select v-model="paramsData.warehouse_id" class="inline width-sm"
                          :options="warehouses"
                          storage-key="cargoWarehouse"
                          @change="select_warehouse"></super-select>
            <!--<el-select v-model="paramsData.warehouse_id" class="inline width-sm" @change="select_warehouse">
                <el-option
                        :key="item.value"
    ...
    export default {
        ...
        methods: {
            ...
            select_warehouse(val) {
                ...
                this.cargo_class_list(this.paramsData.warehouse_id);
                this.search();
            },
            select_status(val) {
            ...
            warehouses_init() {
                ...
                this.$http(api_get_warehouse_default, params).then(res => {
                    ...
                    this.paramsData.warehouse_id = this.warehouses[0].value
                }).catch(code => {
                    this.$message({
                        type: 'error',
