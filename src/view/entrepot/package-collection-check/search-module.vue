    ...
    export default {
        ...
        methods: {
            init(){
                ...
                this.$emit('init-data', clone(this.searchData));
                this.searchData = {
                    code:'',
                    weight:'',
                    warningText:''
            ...
            websocket_weghing() {
                this.trim_search();
                let search = clone(this.searchData);
                let data = {
                    id: `${search.code}_${Date.now()}`,
                    route: 'weigh-setting-params',
                    sendmode: 1,
                    package_id: `${search.code}_${Date.now()}`,
                };
                local_websocket.sendRequest(data, (result) => {
                    if(result.status === 1) {
                        this.searchData.weight = result.direct_results;
                        this.weight_enter();
                    } else {
