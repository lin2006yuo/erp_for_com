                ...
                <div>
                    <table class="sku-table mt-sm" cellpadding="0" cellspacing="0">
                        ...
                        <template v-else>
                            ...
                            <tr>
                                <td>商品尺寸</td>
                                <td><span class="font-16" :class="{red: skuData.over_weight === 1, 'bold-font':skuData.over_weight === 1}"
                                    v-if="skuData.length || skuData.width || skuData.height"
                                >长：{{skuData.length}} cm&nbsp;宽：{{skuData.width}} cm&nbsp;高：{{skuData.height}} cm</span></td>
                            </tr>
                            <tr>
                                <td>商品单重</td>
                                <td><span class="font-16" v-show="skuData.weight">{{skuData.weight}}g</span></td>
                            </tr>
    ...
    export default {
        ...
        data() {
            return {
                ...
                activeName: '1',
                logData: [],
                action: 'http://127.0.0.1:10093/print',
                printVisible: false,
                batchData: [],
                columnHeight: 100,
        ...
        methods: {
            ...
            submit(params) {
                this.$http(api_twice_sorting, params).then(res => {
                    this.isScan = false;
                    this.scansNumber = 0;
                    res.voice && this.audio_play(Number(res.voice).toString().split('').join(' '), 4);
                    this.skuData = res;
