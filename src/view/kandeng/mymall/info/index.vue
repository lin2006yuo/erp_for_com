    ...
    <page>
        <page class="over-flow-auto">
            <relevance-goods @add-account="add_account" :goods-name="publish.zh_name"
                             :web="geturl" :brand="publish.brand" :spu="publish.parent_sku"
...
<script>
    ...
    import {api_get_account, api_get_mymallaccount} from '@/api/mymall'
...
//            表单验证
            ...
            goods_description(){
                    this.isloading=true;
                    this.title=this.publish.parent_sku
                    this.$http(api_get_goods_description,this.publish.goods_id,{page:1,page_size:20}).then(res=>{
                        this.count=res.count
                        this.trot_data=res.list;
                        this.isloading=false;
...
//            刊登到平台
            ...
            // 获取可用账户
            async get_account(value){
                let res = await this.$http(api_get_mymallaccount,value);
                if(res.data){
        ...
        computed: {
            ...
            geturl(){
                if(this.publish.landing_page_url==''){
                    return 'javascript:;'
                }  else{
                    return this.publish.landing_page_url
                }
            },
        },
        watch: {
        ...
        components: {
            ...
            tortTable:require('../list/tort-table').default,
