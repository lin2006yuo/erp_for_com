<template>
    <page-dialog v-model="show" size="large" :title="titleName" :close-on-click-modal="false" class="sendRules-dialog"
                 @change="close_send" @open="open">
                    ...
                    <el-row>
                        ...
                        <el-col :span="8">
                            <el-form-item label="平台：" required>
                                <el-select v-model="platform" class="inline" style="width:200px"
            ...
            <el-row :gutter="8" class="send-margin">
                <el-col v-show="leftSpan" :span="leftSpan">
                    <el-card>
                        <div slot="header">
                            <label>选择条件</label>
                            <el-checkbox class="fr fr-checkbox" v-model="showContext"
                                         :label="showContext ? '显示选项内容' :'隐藏选项内容' "></el-checkbox>
                ...
                <el-col v-if="rightSpan" v-loading="ruleCondLoading" :span="rightSpan">
                    <el-card>
                        <div slot="header">
...
<script>
    import {
        api_send_rules_addtime,
        api_send_rules_email,
        api_send_rules_item,
        api_send_rules_resources,
        api_add_send_rules,
        api_edit_send_rules,
        api_update_send_rules,
        api_getTemplates
    } from '../../../api/send-rules';
    ...
    export default {
        ...
        methods: {
            ...
            triggerRulesFun() {
                if (this.platform) {
                    if (this.triggerCache[this.platform] === undefined) {
                        this.$http(api_send_rules_addtime, {channel_id: this.platform}).then(res => {
                            this.triggerCache[this.platform] = res;
                            this.trigger = res
                        }).catch(code => {
                            this.$message({type: "error", message: code.message || code})
                        })
                    } else {
                        this.trigger = this.triggerCache[this.platform];
                    }
                }
            },
            emailRulesFun() {
                if (this.platform) {
                    if (this.sendChannelCache[this.platform] === undefined) {
                        this.$http(api_send_rules_email, {
                            channel_id: this.platform,
                            trigger_rule: this.triggerRules || ''
                        }).then(res => {
                            this.sendChannelCache[this.platform] = res;
                            this.email_ = res
                        }).catch(code => {
                            this.$message({type: "error", message: code.message || code})
                        })
                    } else {
                        this.email_ = this.sendChannelCache[this.platform];
                    }
                }
            },
            templateFun() {
            ...
            init_config(code) {
                ...
                if(code === 'source' && this.markCode === ''){
                    if(this.mdfid === 0 && !this.isAddInit){
                        this.$message({type:'warning',message:'请先选择平台！！'});
                    }
                    this.isAddInit = false;
                    return
                }
                if (!this.configs[code === 'source' ? code + this.markCode : code]) {
                    this.ruleCondLoading = true;
                    this.$set(this.configs,code === 'source' ? code+this.markCode : code,{data:[]});
                    this.$http(api_send_rules_resources, {code: code === 'source' ? code + this.markCode : code}).then(res => {
                        this.ruleCondLoading = false;
                        if (code === 'source') {
                            res.election = res.election ? true : false;
                        }
                        this.$set(this.configs, code === 'source' ? code + this.markCode : code, res);
                    })
                }
            },
            auto_action(actions) {
            ...
            item_context(item) {
                ...
                let strs = [];
                if (this.configs[node.code === 'source' ? node.code + this.markCode : node.code]) {
                    strs = calc_item_context(this.configs[node.code === 'source' ? node.code + this.markCode : node.code].type, this.configs[node.code === 'source' ? node.code + this.markCode : node.code].data, node.data.child, this);
                }
            ...
            submit() {
                    ...
                    let rowgetdata = (data) => {
                        ...
                        for (let item in data) {
                            if (data.hasOwnProperty(item) && data[item].value) {
                                ...
                                if (data[item].group === 'channel' && row.code === 'source' && this.configs[row.code + this.markCode].hasOwnProperty('election')) {
                                    ret.election = !!this.configs[row.code + this.markCode].election ? 1 : 0;
                                }
                                rets.push(ret);
                            }
                        }
                ...
                if (this.mdfid > 0) {
                    this.$http(api_update_send_rules, this.mdfid, params).then(res => {
                        ...
                        this.show = false;
                        let find = this.trigger.feedback && this.trigger.feedback.datas.find(row => {
                            return row.id === this.triggerRules;
                        });
                        let find2 = this.trigger.message && this.trigger.message.datas.find(row => {
                            return row.id === this.triggerRules;
                        });
                        if (!!find) {
...
//                         this.showContext = true;
        ...
        computed: {
            ...
            ruleConfig() {
                return this.configs[this.ruleCode === 'source' ? this.ruleCode + this.markCode : this.ruleCode] || {};
            },
            ruleTitle() {
                if (this.panelWith) {
                ...
                handler(val){
                    switch (val) {
                        case 1:
                            ...
                            break;
                        case 2:
                            this.markCode = '_amazon';
                            break;
                        case 4:
                            this.markCode = '_aliexpress';
                            break;
                        default:
                            this.markCode = '';
                            break
