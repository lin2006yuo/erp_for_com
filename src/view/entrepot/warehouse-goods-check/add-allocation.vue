<template>
    <page-dialog :title="addAction.title"
        ...
        <el-card>
            <search-card v-if="!isPrint" :params="searchData" :clears="clears" @search="search_tab"
                ...
                <label-item label="" class="inline ml-sm">
                    ...
                    <super-select v-model="searchData.areaText"
                                  class="inline ml-sm width-lg"
                                  :options="areaList"
                                  storageKey="warehouseAreaList"></super-select>
                    ...
                    <el-table
                        ...
                        <el-table-column
                            ...
                            <template slot-scope="scope">
                                <ui-tip content="良品" :width="98"></ui-tip>
                                ...
                                <integer-zero-input v-model="scope.row.quantity_check"
                                                    ...
                                                    style="width: 65px"
                                                    ref="quantity"
                                                    @focus="focus(scope.row.number, 1)"
                                                    @keyup.native="key_updown($event, scope.$index, 1)"
                                                    :disabled="!(isEdit && scope.row.status === 0)"
                                                    :class="{warningborder:scope.row.quantity_check === ''}"
                                                    v-show="isEdit && scope.row.status === 0" :min="0">
                                </integer-zero-input>
                                <ui-tip content="次品" :width="98" class="ml-sm"></ui-tip>
                                <ui-tip :content="scope.row.quantity_check_defective" :width="98"
                                        v-if="!isEdit || scope.row.status !== 0"></ui-tip>
                                <integer-zero-input v-model="scope.row.quantity_check_defective"
                                                    class="inline mr-xs"
                                                    style="width: 65px"
                                                    ref="defective_quantity"
                                                    @focus="focus(scope.row.number, 2)"
                                                    @keyup.native="key_updown($event, scope.$index, 2)"
                                                    :disabled="!(isEdit && scope.row.status === 0)"
                                                    :class="{warningborder:scope.row.quantity_check_defective === ''}"
                                                    v-show="isEdit && scope.row.status === 0" :min="0">
                                </integer-zero-input>
                                <el-button size="mini"
                        ...
                        <el-table-column
                                ...
                                label="盈亏总成本(rmb)">
                            <div>
                                <ui-tip :content="row.amount" :width="98"></ui-tip>
                            </div>
                        </el-table-column>
                        <el-table-column
                                inline-template
        ...
        <div slot="footer">
            <request-button req-key="auditPass" @click="audit(1)" v-if="isFinish">审核通过</request-button>
            <request-button req-key="auditReject" @click="audit(2)" v-if="isFinish">审核不通过</request-button>
        ...
        methods: {
            audit(data){
                ...
                if(data === 1){
                    ...
                    str = '不通过';
                    btn = 'auditReject';
                    this.$set(params, 'status', 4)
                }
                this.$confirm(`您将审核${str}盘点单${this.tableData.id}，确定进行此操作?`,'提示',{
                    confirmButtonText:"确定",
                    cancelButtonText:"取消",
                    type:"warning"
                }).then(()=>{
                    this.$http(api_finish_detail,this.tableData.id, params).then(res=>{
                        this.$message({
                            type:"success",
            ...
            heavyPlate(row) {
                this.$http(api_recheck_details, row.id).then(res => {
                    ...
                    row.initial_defective = clone(row.quantity_check_defective);
                    row.cancel = true;
                    row.plate = false;
                    row.save = true;
                    row.status = 0;
            ...
            save_check(row) {
                ...
                let data = {
                    ...
                    quantity_check_defective: row.quantity_check_defective,
                    remark: row.remark,
                };
            ...
            cancel_check(row) {
                ...
                row.quantity_check_defective = clone(row.initial_defective);
                row.cancel = false;
                row.plate = true;
                row.status = 1;
            },
            ...
            edit() {
                ...
                this.copyData.forEach(row => {
                    if (this.rawData[row.id] !== row.quantity_check || this.rawData[row.id + '1'] !== row.quantity_check_defective || row.status === 0) {
                        let item = {
                            warehouse_goods_check_id: this.tableData.id,
                            warehouse_cargo_code: row.warehouse_cargo_code,
                            sku: row.sku,
            ...
            tabData() {
                ...
                this.copyData.forEach((row, index) => {
                    ...
                    this.rawData[key2] = row.quantity_check_defective;
            ...
            focus(index, type) {
                ...
                switch(type){
                    case 1:
                        ...
                        break;
                    case 2:
                        this.$refs.defective_quantity[index].$refs.input.select();
                        break
                }
            },
            key_updown(ev, index, type) {
                if (ev.keyCode === 34) {
                    switch(type){
                        case 1:
                            ...
                            break;
                        case 2:
                            for (let i = index; i < this.$refs.defective_quantity.length - 1; i++) {
                                if (!this.$refs.defective_quantity[i + 1].disabled) {
                                    this.$refs.defective_quantity[i + 1].$refs.input.select();
                                    break;
                                }
                            }
                            break
