<template>
    <page>
        <el-row class="p-parcel-declare">
            ...
            <div class="function">
                <permission tag="ElButton" :route="apis.url_declare_add" type="primary" @click.native="add" size="mini">
                    添加
                </permission>
                <permission tag="request-button" req-key="indexSave"
                            :route="apis.url_declare_sort"
                            v-if="isChange" @click="save">确定
                </permission>
                <el-button type='primary' class='fr' size='mini' @click='defaultRule'>默认规则</el-button>
            </div>
            <data-table @refresh-data="init()" @change="isChange=true" :first-loading="firstLoading" @lookup="lookup"
                        @editup="editup" :tables="tables" :loading="loading"></data-table>
        </el-row>
        <rule-dialog ref="releDialog" @refresh-data="init()" :mdfid="mdfid" v-model="show" :isLook="isLook"
                     :title-name="titleName"></rule-dialog>
        <rule-default :title='defaultName' v-model='defaultShow'  @refresh_set="refresh_set"  :defaultData='defaultData'></rule-default>
    </page>
</template>
<style lang="stylus">
    .p-parcel-declare {
...
<script>
    ...
    import {
        api_declare_list, api_declare_rules, api_declare__info, api_declare_sort,
        url_declare_add, url_declare_sort
    } from '../../../api/parcel-declare'
    ...
    export default {
        ...
        components: {
            ...
            ruleDefault: require('./rule-default').default
        }
    }
</script>
