<template>
    <page class="p-index">
        <label-item label="请选择仓库：" class="mt-sm mb-xs">
            <super-select v-model="searchData.warehouse_id" class="inline width-sm"
                          :options="warehouses"
                          storage-key="shelfWarehouse"
    ...
    export default {
        ...
        mounted() {
            ...
            if (window.objlength(this.$page.param) !== 0) {
                this.searchData.warehouse_id = this.$page.param.warehouse_id;
                this.paramsData.warehouse_area_id = this.$page.param.warehouse_area_id;
                this.searchData.warehouse_area_id = this.$page.param.warehouse_area_id;
                this.paramsData.status = this.$page.param.status;
                this.searchData.status = this.$page.param.status;
        ...
        methods: {
            ...
            warehouses_init() {
                ...
                this.$http(api_get_warehouse_default, params).then(res => {
                    ...
                    this.searchData.warehouse_id = this.warehouses[0].value
                }).catch(code => {
                    this.$message({
                        type: 'error',
                        message: code.message || code
