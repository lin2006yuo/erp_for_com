<template>
    <div class="p-pickings">
        <mailing-method ref="mailing">
            ...
            <!-- 按钮 -->
            <p class="box-title t-left" style="padding-left:8px; margin: 0" slot="button-list">
                <permission tag="ElButton"
                            :disabled="selectData.length === 0"
                            v-if="searchData.status === 2"
                            @click.native="batch_transfer"
                            size="mini"
                            type="primary"
                            class="inline"
                            :route="apis.url_batch_pickings_shipper">批量转交</permission>
            </p>
            <!-- 打印机 -->
            ...
            <ui-table slot="goods-tabel"
                ...
                <template v-for="(order,index) in tableData">
                    <tr @click="addClass(index)"
                        ...
                        <td><el-checkbox v-model="order.isCheck" @change="changeOne"></el-checkbox></td>
                        <!--拣货单号-->
                        ...
                        <td v-if="searchData.status === 2">
                            <ui-tip :content="order.shipper_name" v-if="order.is_multi !== 1"></ui-tip>
                            <span v-else>--</span>
                        </td>
                        <!--开始拣货时间-->
                        <td v-if="searchData.status === 2">
                    ...
                    <template v-if="order.show">
                        <tr v-for="product in order_product(order)">
                            <!--这个复选框只是为了页面排版而用 所以固定不可勾选-->
                            <td><el-checkbox v-model="product.isCheck" disabled></el-checkbox></td>
                            <!--拣货单号-->
                            <td>
                            ...
                            <!--拣货人-->
                            <td v-if="searchData.status === 2">
                                <ui-tip :content="product.shipper_name"></ui-tip>
                            </td>
                            <!--开始拣货时间-->
                            <td v-if="searchData.status === 2">
                                <ui-tip :content="order.picking_start_time | filterTime"></ui-tip>
        ...
        <hand-over v-model="handVisible"
                   :trend-action="trendAction"
                   :shipper-id="curShipperId"
                   :shipper-list="shipperList"
                   @batch-hand="batch_hand"
                   @hand="hand">
        </hand-over>
...
<script>
    ...
    export default {
        ...
        methods: {
            batch_hand(id){
                let ids = this.selectData.map(row => {
                    return row.id
                });
                let params = {
                    picking_id: ids,
                ...
                this.$http(api_batch_pickings_shipper, params).then(res => {
            ...
            batch_transfer(){
                if(this.selectData.some(row => row.is_multi === 1)){
                    this.$message({type: 'warning', message: '多品多件拆分子任务的拣货单不能批量转交'});
                    return
                }
                let person = this.selectData.map(row => {
                    return row.shipper_id
                });
                let transfer = [...new Set(person)];
                if(transfer.length > 1){
                    this.$message({type: 'warning', message: '只有拣货人相同的拣货单才能批量转交'});
                    return
                }
                this.trendAction.title = '批量转交拣货单';
                this.trendAction.value = 'batchHand';
                this.curShipperId = '';
                    ...
                    this.$http(api_get_pickings, data).then(res => {
                        ...
                        this.tableData.forEach(row => {
                            ...
                            if (row.status !== 0) {
                                ...
                                if (row.status === 2 && row.is_multi !== 1) {
                        ...
                        this.loading = false;
                        this.$parent.$refs.search_module.firstLoading = false;
                    }).catch(code => {
                        this.loading = false;
            ...
            hand(id, person) {
                ...
                let index = this.tableData.findIndex(row => {
                    if (row.id === id) {
                        item = row;
                        item.shipper_id = person.value;
                        item.shipper_name = person.label;
                    }
                    return row.id === id;
                });
                if (index !== -1) {
            ...
            action(data, options) {
                ...
                switch (options.value) {
                    ...
                    case 6:
                        ...
                        this.curShipperId = data.shipper_id;
                        this.trendAction.id = data.id;
                        this.handVisible = true;
                        break;
                    case 7:
        ...
        computed: {
            selectData(){
                return this.tableData.filter(row => row.isCheck)
            },
            warehousename() {
