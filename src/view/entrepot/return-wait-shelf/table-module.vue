        ...
        <ui-table v-model="checkAll"
            ...
            <template v-for="(data, index) in tableData">
                <tr class="line" :class="[data.show ? 'active' : '',data.heighLight?'active-color':'']"
                    ...
                    <td>
                        <el-popover
                            ...
                            <span slot="reference">
                                    <img :src="data.thumb" v-if="data.thumb" height="60px" width="60px"
                    ...
                    <td>
                        <plus-minus @show-detail="show_detail(data,index)" :show="data.show"
                ...
                <template v-if="data.show">
                    <template v-if="data.skuList&&data.skuList.data.length">
                        ...
                        <tr class="subset_of_tags"
                            v-for="item in data.skuList.data">
                            <td></td>
                            <td>
                                {{item.package_number}}
                            </td>
                        ...
                        <tr>
                            <td :colspan="heads.length" style=" text-align: center">
                                <el-pagination
                                        class="t-center"
                                        small
                                        @size-change="(val) => {handle_detail_size_change(val, data)}"
                                        @current-change="(val) => handle_detail_current_change(val, data)"
                                        :current-page="data.skuList.page"
                                        :page-size="data.skuList.pageSize"
                                        layout="total, prev, pager, next"
                                        :total="data.skuList.count">
                                </el-pagination>
                            </td>
                        </tr>
                    </template>
...
<script>
    import {api_get_single_detail} from '@/api/reback-shelves';
    ...
    export default {
        ...
        methods: {
            ...
            show_detail(row, index) {
                ...
                if(row.show) {
                    if (row.skuList && row.skuList.data.length) {
                        return;
                    }
                    let data = {
                        page: 1,
            ...
            get_detail(data) {
                let searchData = {
                    ...
                    page: data.skuList.page || 1,
                    pageSize: data.skuList.pageSize || 20
                };
                this.$http(api_get_single_detail, data.id, searchData).then(res => {
                    this.$set(data, 'skuList', res);
                    data.skuList.page = Number(data.skuList.page);
                    data.skuList.pageSize = Number(data.skuList.pageSize);
                }).catch(code => {
                    this.$message({message: code.message || code, type: 'error'});
                });
            },
            //详情sku列表分页器
            handle_detail_size_change(val, data) {
                data.skuList.pageSize = val;
                this.get_detail(data);
            },
            handle_detail_current_change(val, data) {
                data.skuList.page = val;
                this.get_detail(data);
            },
        },
        watch: {
