<template>
    <div class="c-data-table">
        ...
        >
            ...
            <el-table-column inline-template label="是否有站点" width="98px">
                <div>
            ...
            <el-table-column label="操作" inline-template width="264px">
                <div>
                    <span class="operate" @click="look_over(row)">查看</span>
                    <!--<permission class="operate" :route='apis.url_get_channel'-->
                                <!--tag="span">查看</permission>-->
                    <span>|</span>
                    <span class="operate" @click="edit(row)">平台信息</span>
                    ...
                    <span v-if="show_distribution(row)">|</span>
                    <!-- <span class="operate" v-if="row.id===2" @click="proportion(row)">占比设置</span> -->
                    <span class="operate" v-if="show_distribution(row)" @click="distribution(row)">分配设置</span>
                </div>
            </el-table-column>
        </el-table>
        <el-pagination
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            proportion(row){
                this.$emit("proportion",row)
            },
            distribution(row){
                this.$emit("distribution",row);
            },
            ...
            show_distribution(row){
                return [1,2,4].includes(row.id);
            }
        },
        filters: {
            filterSite(value){
