            ...
            <div v-loading="isLoad">
                <el-form :model="gsMapForm" :rules="rules" label-width="150px">
                    <el-form-item label="平台："   prop="channel_id"    required>
                        <el-col  :span="10">
                            <super-select v-model="gsMapForm.channel_id"
                                          @select-option="change_channel" :options="channelList"
                                          v-if="isEdit" storageKey="channelList"></super-select>
                    ...
                    <el-form-item label="账号：" required>
                        <el-col :span="10">
                            <div v-if="isEdit">
                                <super-select v-model='gsMapForm.account_id'
                                              storageKey="accountList"
                                              :options="comAccountList" ></super-select>
                            </div>
                    ...
                    <el-form-item label="是否虚拟仓发货：" required>
                        <el-col  :span="10" v-if="isEdit">
                            <el-select v-model="gsMapForm.is_virtual_send" placeholder="请选择" width="150">
                                <el-option
                        ...
                        <span v-else>{{changes_options.find(r=>r.value==gsMapForm.is_virtual_send)?changes_options.find(r=>r.value==gsMapForm.is_virtual_send).label:''}}</span>
                    </el-form-item>
...
<script>
    ...
    import {api_add,api_update,api_get_channel,api_del_code,api_get_account,url_del_code,url_update,url_add} from '../../../api/goods-sku-map'
        ...
        methods:{
            ...
            posts(data){
                let arr = {
                    ...
                    is_virtual_send:this.gsMapForm.is_virtual_send,
                };
...
//            },
            isDisable(){
                return this.gsMapForm.channel_id&&this.isAdd?false:true;
            }
        ...
        watch:{
            ...
            'gsMapForm.channel_id'(val){
                if(val){
                    this.$http(api_get_account,{channel_id:this.gsMapForm.channel_id,content:''}).then(res=>{
                        this.comAccountList = res.data.map(row=>{
                            return {
                                label:row.account_name,
                                value:row.id
                            }
