        ...
        <el-row :gutter="8">
            <el-col :span="memberList">
                <el-card class="mb-xs">
                    <el-row>
                        <el-col :span="16">
                            <el-select v-model="searchData.searchKey"
                            ...
                            <el-input v-model="searchData.searchVal"
            ...
            <el-col v-if="showAdd" :span="12">
                ...
                <div class="fr mt-md mb-xs">
                    <request-button req-key="saveMember" :min-time="300" @click="save_member">保存</request-button>
                    <el-button size="mini" @click.native="memDialog=false">取消</el-button>
                </div>
            </el-col>
        </el-row>
    </page-dialog>
...
<script>
    ...
    import {api_ship_add_account} from '@/api/member'
    export default{
        data(){
            return{
                memberSize:"large",
        ...
        computed:{
            ...
            placeholder(){
                let find=this.options.find(row=>row.value===this.searchData.searchKey);
                if(find){
                    return `请选择${find.label}`;
                }
            },
            members(){
                switch (this.searchData.searchKey){
                    case 1:
                        return this.memForm.filter(row=>row.realname.indexOf(this.searchData.searchVal)>-1);
                    case 2:
                        return this.memForm.filter(row=>row.username.indexOf(this.searchData.searchVal)>-1);
                }
            },
            memberListName(){
...
//            ------------------------------------  保存
            save_member(){
                ...
                let arr= this.sellerId.filter(row=>this.ids.includes(row));
                if(arr.length > 0){
                    this.$message({type: "error", message: '不可添加相同销售员、客服成员到列表'});
                    this.$reqKey('saveMember', false);
                }else {
                    this.$http(api_ship_add_account,data).then(res=>{
