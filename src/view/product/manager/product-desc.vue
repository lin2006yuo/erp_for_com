<template>
    <el-row class="c-product-desc">
        ...
        <el-tabs type="card" @tab-click="tab_click" :active-name="action">
            <el-tab-pane v-for="(item,index) in descData" :key="item.lang_name"  :label="item.lang_name" :name="item.lang_name" >
                <el-row :gutter="8" style="margin-right: 20px!important;">
                    ...
                    <el-col  :span="showChinese?12:24">
                            ...
                            <div style="display: flex" class="mt-sm">
                                <div class="mt-sm label-align">
                                    {{item.lang_name}}：
                                </div>
                                <div style="flex: 1" class="mt-sm">
                                    <el-checkbox v-model="showHtml" class='mb-sm' v-if="!editAble">可编辑文本</el-checkbox>
                                    <el-input v-if="editAble||!showHtml"
                                            type="textarea"
                                            :disabled="!editAble"
                                    ...
                                    <simditor class="product-simditor" v-model="item.description2" v-if="!editAble&&showHtml"
                                              ref="uEditor" style="width:100%;" :toolbar="toolbar"
                                              @value-changed="content_change(index,$event)"
                                              :editorId='return_id(item)'></simditor>
                                </div>
                            </div>
                    </el-col>
                </el-row>
...
<script>
    ...
    export default{
        ...
        methods:{
            return_id(item){
                return `${item.lang_name}-simditor`;
            },
            content_change(index,res){
            },
