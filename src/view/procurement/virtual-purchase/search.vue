        ...
        <search-card @search="search_list" :params="searchData" :clears="clears">
            ...
            <el-row class="mb-mini">
                ...
                <super-select class="inline s-width-middle"
                              v-sf.virtual_supplier_id
                              v-model="searchData.virtual_supplier_id"
                              storageKey="virtual_supplier"
        ...
        methods: {
            get_supplier_list(){
                this.$http(api_get_supplier_virtual_supplier).then(res=>{
                    this.supplierList = res.map(row=>({label:row.company_name,value:row.id}));
                }).catch(code=>{
                    console.log(code)
                })
            },
