...
<script>
    ...
    import {api_channels_account} from '@/api/lianlianpay';
    export default {
        data() {
            return {
                status:[
        ...
        methods:{
            ...
            remove_tag(item) {
                let index = this.searchData.account_code.findIndex(row => row === item.value);
                if (index >= 0) {
                    this.searchData.account_code.splice(index, 1);
                }
            },
        },
        computed:{
