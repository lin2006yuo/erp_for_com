<template>
    <div class="p-search-aliexpress-inbox">
        <search-card @search="search" ref="search_card" :params="searchData" :clears="clears" @enter="search">
            <el-row>
                <label-buttons label="店铺：" :buttons="shop" @select="select_handles" class="search-sm" v-sf.status></label-buttons>
                <label-buttons label="处理状态：" :buttons="handles" :default-index="1" @select="select_handles" class="search-sm" v-sf.status></label-buttons>
                <label-buttons label="回复级别：" :buttons="priority" @select="select_priority" class="search-sm" v-sf.rank></label-buttons>
            </el-row>
            <div class="inline">
                ...
                <label class="ml-sm inline">销售：</label>
...
<!--                </el-select>-->
                <super-select
                    ...
                    :placeholder="all"
                    v-sf.customer_id
                    storageKey="api_customer_wish_inbox"
                    :options="options"
                >
                </super-select>
            </div>
    ...
    export default{
        ...
        data(){
            return {
                ...
                type:[
                    {label:'客户名称',value:'channel_order_id'},
                    {label:'平台订单号',value:'buyer_id'},
                    {label:'站内信编号',value:'site_id'},
                    {label:'买家ID',value:'order_id'}
                ],
                read:[
                    {label:'全部',value:''},
                    {label:'未读',value:0},
