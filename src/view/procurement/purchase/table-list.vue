<template>
    <el-row class="p-table-list">
        ...
        <!--操作日志-->
        <operation-log v-model="log_view" :action-log="operationLog"></operation-log>
        ...
        <el-table
                ...
                <div>
                    ...
                    <div v-for="(item,key) in row.packInfo" :key="key" class="inline" style="cursor: pointer;">
                        <span v-if="item.status===1">
                            <img  @click="look_logistics(row,item,key)" src="../../../assets/receive_before.png" title="待接收(点击查看物流)">
                        </span>
                        <span v-else-if="item.status===2">
                            <img  @click="look_logistics(row,item,key)" src="../../../assets/receive_have.png" title="已接收(点击查看物流)">
                        </span>
                        <span v-else-if="item.status===4">
                            <img  @click="look_logistics(row,item,key)" src="../../../assets/receive_unpack.png" title="收包异常(点击查看物流)">
                        </span>
                        <span v-else>
                            <img  @click="look_logistics(row,item,key)" src="../../../assets/receive_unsual.jpg" title="已拆包(点击查看物流)">
                        </span>
                    </div>
                </div>
            </el-table-column>
            ...
            <el-table-column label="贴标/套袋" min-width="80" inline-template>
                <div class="p-table-list-td-text" :title="row.label_deck|filterLabelDeck">{{row.label_deck|filterLabelDeck}}</div>
            </el-table-column>
            <el-table-column
                label="运费"
            ...
            <el-table-column
                inline-template
                label="供应链专员">
                <div class="p-table-list-td-text" :title="row.supply_chain_specialist">{{row.supply_chain_specialist|emptyByDefault}}</div>
            </el-table-column>
            <el-table-column
                inline-template
                min-width="60"
                label="审批人">
                <div class="ellipsis" :title="row.approver ">{{row.approver  | filterTrackingNumber}}</div>
            </el-table-column>
            <el-table-column
                inline-template
                label="采购日期">
...
<script>
    ...
    import {
        ...
        api_purchase_order_log,
        url_purchase_order_log
    } from "../../../api/purchase";
    ...
    const actions = {
        ...
        log: {
            name: "操作日志",
            api: url_purchase_order_log,
            action(row) {
                this.$http(api_purchase_order_log, {purchase_order_id: row.id}).then(res => {
    ...
    export default {
        ...
        methods: {
            ...
            operateSelects(row) {
                ...
                if (row.buttons.is_display_operation_detail_bt === 1){
                    ...
                    switch (updateType){
                        case 'update-logistics':
                            ...
                            if(find){
                                ...
                                find.external_number = res.external_number;
                            }
                            break;
                    }
                }).catch(code => {
