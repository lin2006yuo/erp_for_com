    ...
    <page class="p-index">
        ...
        <el-row class="mt-sm ml-sm">
            ...
            <permission tag="request-button"
                        ...
                        @click="push_yks" v-if="searchData.review_status === 1">推送给YKS
            </permission>
            <permission tag="request-button"
...
<script>
    ...
    export default{
        ...
        methods:{
            show_message(info,type){//导出提示 当返回数组时分段展示 不会自动关闭弹框 提示导出不成功的
            ...
            export_order_public(url_, data, type, show) {//导出订单公共
                let ids = JSON.stringify(this.selectList.map(row => row.virtual_finance_purchase_id));
                this.$http(url_,{ids: ids, ...data}).then(res => {
                    ...
                    this.$reqKey(type,false);
                });
            },
            export_invoice() {//导出发票
                let data = this.init_params();
                let contract  = this.invoiceList.contract_no;
                this.$set(data, 'contract_no', contract);
                this.export_order_public(api_export_invoice, data, 'export_invoice', 0);
            },
            export_deliver(){//导出送货单
                let data = this.init_params();
                let deliver = this.deliverList.deliver_code;
                this.$set(data, 'deliver_code', deliver);
                this.export_order_public(api_export_deliver, data, 'export_deliver', 1);
            },
            export_stock(){//导出入库单
                let data = this.init_params();
                let stock_code = this.stocktList.in_stock_code;
                this.$set(data, 'in_stock_code', stock_code);
                this.export_order_public(api_export_stock, data, 'export_stock', 2);
            },
            export_receipt(){//导出收货单
                let data = this.init_params();
                let receipt = this.receiptList.receipt_code;
                this.$set(data, 'receipt_code', receipt);
                this.export_order_public(api_export_receipt, data, 'export_receipt', 3);
            },
            export_order(){//导出订购单
                let data = this.init_params();
                this.export_order_public(api_export_purchase, data, 'export_order', 4);
            },
            invoice_view(){//查看发票
                this.look_order_public(api_read_invoice, api_export_invoice, "url_read_invoice", 0);
            },
            deliver_view(){//查看送货单
                this.look_order_public(api_read_deliver, api_export_deliver, "url_read_deliver", 1);
            },
            stock_view(){//查看入库单
                this.look_order_public(api_read_stock, api_export_stock, "url_read_stock", 2);
            },
            receipt_view(){//查看收货单
                this.look_order_public(api_read_receipt, api_export_receipt, "url_read_receipt", 3);
            },
            order_view(){//查看订购单
                this.look_order_public(api_read_purchase, api_export_purchase, "url_read_purchase", 4);
            },
            look_order_public(url, url2, val, show) {//查看导出单 公共
                let data = this.init_params();
                ...
                if(this.checkedall && opition === true) {
                    this.$http(url2, {select: 1, ...data}).then(res => {
                        this.export_visible = true;
                        this.$message({type: "success", message: res.message || res});
                    }).catch(code => {
                        this.$message({type: "error", message: code.message || code});
                    }).finally(() => {
                        this.$reqKey(val, false);
                    })
                }else {
                    if (opition === false) {
                        ...
                        this.$http(url, {ids: JSON.stringify(this.selectList.map(row => row.virtual_finance_purchase_id)), ...data}).then(res => {
                            switch(show){
                                case 0:
