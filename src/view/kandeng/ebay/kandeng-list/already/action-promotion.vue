<template>
    <page-dialog v-model="show" size="small" :title="$t('ebay-list.cxsetting')" @open="open"
                 class="p-batch-edit" :close-on-click-modal="false">
        <el-row class="auto-add">
            <el-col :span="8" style="text-align: right">
                <label for="">{{$t('ebay-list.cx')}}</label>
            </el-col>
            <el-col :span="16">
                <el-select v-model="promotion" :placeholder="$t('ebay-list.ps')" clearable class="has-inline mr-md" filterable style="width:120px">
                    <el-option v-for="item in promotionList" :key="item.value" :label="item.label" :value="item.value"></el-option>
                </el-select>
            </el-col>
        </el-row>
        <p class="tips">
            {{$t('ebay-list.ebayTips')}}
        </p>
        <div slot="footer" class="dialog-footer">
            <request-button @click="keep" req-key="actionPromition" :mintime="300">{{$t('common.confirm')}}</request-button>
...
<script>
    import {api_get_combList} from '../../../../../api/ebay-kandeng-public-module';
