...
<script>
    ...
    export default {
        ...
        methods: {
            blur(){
                if(!this.list.length)this.$set(this,'keyword','');
                let filter = this.list.filter(row=>row.value.indexOf(this.keyword)>-1);
                if(filter.length){
                    let find = filter.filter(row=>row.value === this.keyword);
                    if((!find.length)||(!!find.length && !find.filter(row=>row.is_select).length)){
