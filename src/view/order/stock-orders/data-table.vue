                ...
                <tbody>
                    <template v-for="(order,index) in tables.lists">
                        <tr  :key="order.order_number" :class="[order.show ? 'active' : '',
                            ...
                            <td>
                                <trends-select class="inline" @trigger="action(order,$event),acction_click($event)" :selects="m_option(order)" type="primary"></trends-select>
                            </td>
                        </tr>
    ...
    export default{
        ...
        methods:{
            ...
            acction_click(ev){    //让该行获得action效果
                ev=window.event||ev;
                ev.path?ev.path[6].click():ev.target.offsetParent.parentNode.click();
            },
            action(data,options){
                this.mdfid = data.id;
