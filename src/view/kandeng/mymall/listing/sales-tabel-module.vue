<template>
    <div class="p-sales-tabel">
        <div class="mt-xs mb-xs ml-sm">
            ...
            <el-button class="inline mb-mini" size="mini" type="primary" @click="more_export" :disabled="!tableData.length>0">批量导出</el-button>
        </div>
        <ui-table
        ...
        <export-dialog v-model="exportVisable"></export-dialog>
    </div>
</template>
<style lang="stylus">
...
<script>
    import {api_record_logs,url_record_logs,url_record_Synchronize,api_record_Synchronize,url_record_drop,api_record_drop,api_pandao_download} from '@/api/mymall'
    ...
    export default {
        ...
        methods:{
            more_export(){
                this.$http(api_pandao_download,Object.assign(this.paramlist,{enabled:1,publish_status:1,nTime:'date_uploaded'})).then(res=>{
                    if(res.status===1){
                        ...
                        this.$message({type:'error',message:res||res.message});
                    }
                })
            },
            operate(data,action){
        ...
        components:{
            ...
            exportDialog:require("@/view/product/goods_sku_map/export-dialog.vue").default,
        }
    }
</script>
