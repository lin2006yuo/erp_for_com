        ...
        <drag-drop v-model="itemData"
            ...
            <li
               ...
               v-menu-context="editAble?menu_context(item):''"
                ...
                <div v-if="item.is_default===8" class="marketing-pic" title="营销图"></div>
                <el-checkbox class='copy-box'
                             v-model="item.isCopy" @change.native="copy_cur($event,item)"
                             v-if="item.hasOwnProperty('isCopy')&&!editAble"></el-checkbox>
...
<script>
    export default {
        ...
        methods:{
            copy_cur(isCopy,item){
                this.$emit('copy-cur',item);
            },
            ...
            menu_context(item,index){
                ...
                const marketing_pic = [{label:"设为营销图",action:function () {
                    this.establish_marketing_pic(item,index);
                }}];

                switch(item.is_default){
                    case 1:
                        curList = [...detail_pic,...size_pic,...marketing_pic];
                        break;
                    case 2:
                        curList = [...mian_pic,...size_pic,...marketing_pic];
                        break;
                    case 4:
                        curList = [...mian_pic,...detail_pic,...marketing_pic];
                        break;
                    case 8:
                        curList = [...mian_pic,...detail_pic,...size_pic];
                        break;
                }
                return curList;
            },
        ...
        computed:{
            cloneItemData(){
                if(this.itemData&&this.itemData.length>0){
                    let cloneData = clone(this.itemData);
                    return cloneData.map(row=>{
                        this.$set(row,'path',this.url_join(row.path,'_500x500.'));
                        return row
                    })
                }else{
                    return []
