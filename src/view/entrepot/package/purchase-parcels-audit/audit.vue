    ...
    <div ref="page" style="height: 100%">
        <el-card class="mb-xs">
            ...
            <el-input v-model="searchData.s_bill_number" placeholder="请输入内容" class="inline mr-sm width-super ml-sm"
                      @keyup.enter.native="s_bill_enter($event)" autofocus ref="search_input">
            </el-input>
            <permission tag="ElButton" class="inline"
                        :route="apis.url_parcel_search" type="primary"
        ...
        <el-form class="el-row-reset-card" :style="cardStyle" v-loading="loading" element-loading-text="玩命加载中">
            <el-form-item>
                <el-card v-for="(item, index) in auditData" @click.native.prevent="check_package($event, index)"
                         class="mb-xs" :key="item.id">
                    ...
                    <el-table :data="item.sku_list" class="arrival-table scroll-bar" max-height="580" ref="table"
                              @selection-change="handle_selection_change(index, $event, item)" highlightCurrentRow>
                        ...
                        <el-table-column width="80" label="图片">
                            <template slot-scope="scope">
                                <el-popover placement="right" trigger="hover">
                                    ...
                                    <span slot="reference">
                                        <img :src="scope.row.thumb" @click="search_img(scope.row.thumb)"
                                             v-if="scope.row.thumb" height="60px" width="60px" style="border:none">
                                    </span>
                                </el-popover>
                            </template>
                        </el-table-column>
                        <el-table-column min-width="100" label="SKU/SKU别名">
                            <template slot-scope="scope">
                                <permission tag="uiTip" :route="apis.url_sku" :is-operate="true"
                                            :content="scope.row.sku" :width="98"
                                            @cur-click="show_goods_detail(scope.row)">
                                </permission>
                                <permission tag="ElButton" :route="apis.url_get_sku" type="primary"
                                            @click="re_check(scope.row)" v-if="scope.row.check === 0" size="mini"
                                            class="mb-sm">
                                    复核
                                </permission>
                        ...
                        <el-table-column min-width="250" inline-template label="名称">
                            <div>
                                {{row.sku_name}}
                            </div>
                        </el-table-column>
                        ...
                        <el-table-column label="缺货数量" inline-template>
                            <span>{{row.shortage_quantity}}</span>
                        </el-table-column>
                        <el-table-column prop="qty" label="采购数量">
                        </el-table-column>
                        <el-table-column inline-template label="已接收数">
                            <div>
                                <ui-tip :content="row.expected_int_qty | isZero" :width="98"></ui-tip>
                            </div>
                        </el-table-column>
                        <el-table-column label="本次良品" min-width="100" class="t-center input-number">
                            <template slot-scope="scope" class="required-sign">
                                <el-input v-model="scope.row.accepted_goods_qty"
                                          ...
                                          ref="accept" :disabled="scope.row.status === 1 || item.is_allow_receipt === 0"
                        ...
                        <el-table-column inline-template :render-header="is_require">
                            <div>
                        ...
                        <el-table-column label="操作" width="120">
                            <template slot-scope="scope">
                                <trends-select class="inline" @trigger="action(scope.row,$event,item)"
                                               :selects="options" type="primary">
                                </trends-select>
                            </template>
                        </el-table-column>
                    </el-table>
        ...
        <!--</el-card>-->
        <search-dialog v-model="dialogTableVisible" @refresh="search_btn" :purcahse-code="purchaseCode"
                       :purchase-note="purchaseNote"></search-dialog>
        <review-dialog v-model="review_visible" :goods_detail="goods_detail" @init="search_again"></review-dialog>
        <goods-detail v-model="goods_visible" :goods_detail="goods_detail"></goods-detail>
        <preview v-model="printVisible" :print-data="printData" ref="preview"></preview>
        <batch_preview v-model="batch_printVisible" :batch-data="batchData" ref="batch_preview"></batch_preview>
...
<script>
    ...
    import {
        api_get_audit,
        api_post_audit
    } from "@/api/purchase-parcels-audit";
    ...
    export default {
                    ...
                    action(data, list) {
                        ...
                        this.$confirm('确认审核?', '提示', {
                            ...
                            this.$http(api_post_audit, post).then(res => {
                                this.loading = false;
                                let index = list.sku_list.findIndex(row => {
                                    return row.id === data.id;
                                });
                                if (index !== -1) {
                                    list.sku_list.splice(index, 1);
                                }
                                this.focus();
                                this.$message({
                                    type: "success",
                                ...
                                if (this.select_sku[i]) {
                                    for (let j = 0; j < this.select_sku[i].length; j++) {
                                        ...
                                        skus.push({
                                            ...
                                            is_allow_receipt: this.auditData[i].is_allow_receipt,
                                            label_id: this.select_sku[i][j].label_id
                                        });
                                    }
                                    if (skus.length !== 0) {
                                        let info = {
                                            purchase_parcel_code: this.auditData[i].purchase_parcel_code,
                                            purchase_order_id: this.auditData[i].purchase_order_id,
                                            batch_id: this.select_sku[i][0].batch_id,
                                            skuInfo: skus
                                        };
                                        receiptParcelInfo.push(info);
                                    }
                        ...
                        p_action() {
                            ...
                            for (let i = 0; i < this.select_sku.length; i++) {
                                for (let j = 0; j < this.select_sku[i].length; j++) {
                                    ...
                                    post.purchase_order_id = this.auditData[i].purchase_order_id;
                                    post.num = this.select_sku[i][j].accepted_goods_qty;
                                    if (post.num <= 0) {
                                        this.$message({
                                            type: "error",
                                            message: `第${i + 1}个采购单的第${j + 1}个sku没有输入良品数`
                                        });
                                        return false;
                                    }
                                    post.label_type = 1;
                                    post.purchase_parcel_code = this.auditData[i].purchase_parcel_code;
                            ...
                            this.$http(api_print_label, {
                                ...
                                this.$refs.batch_preview.get_template(1, {
                                    warehouse_id: this.auditData[0].warehouse_id
                                });
        ...
        methods: {
                ...
                if (this.isChangeLabelId) {
                    return [
                        h('input', {
                            ...
                            on: {
                                change: this.batch_change_lable_id
                            }
                        }),
                        h('i', {
            ...
            batch_change_lable_id(event) {
                ...
                if (!value) return;
                this.checkedPackage.forEach(res => {
                    this.$set(res, 'label_id', value);
                });
                event.target.value = '';
                return;
            ...
            init(res) {
                ...
                this.auditData.forEach(item => {
                    item.sku_list.forEach(row => {
                        ...
                        this.$set(row, "num", i);
                        this.$set(row, 'is_allow_receipt', item.is_allow_receipt);
                        i++;
                    })
                });
            },
            ...
            //搜索
            search_btn(data) {
                if(!this.searchData.s_bill_number) {
                    this.focus();
                    return this.$message({type: 'warning', message: '单号必须填写！'});
                }
                this.loading = true;
                let search = {};
            ...
            check_package(e, index) { // 确定当前操作的包裹
                this.checkedPackage = this.auditData[index].sku_list;
            },
            trim_label_id(row) {
                ...
                this.$nextTick(() => {
                    if (this.auditData.length) {
                        for (let i = 0; i < this.auditData.length; i++) {
                            this.auditData[i].sku_list.forEach(row => {
