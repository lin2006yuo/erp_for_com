            ...
            <el-form :model="form" ref="form" :rules="rules" label-width="220px">
                <el-form-item :label="TemplateName">
                    <el-row>
                        <el-col :span="largeSpan">
                            <el-input v-model="form.list.draft_name" ></el-input>
                        </el-col>
                    </el-row>
                </el-form-item>
                <el-form-item label="平台账号：">
                    <el-row v-if="param&&param.curPage==='model'">
                        <el-col :span="commonSpan">
                            <super-select v-model="form.code"
                ...
                <el-form-item :label="$t('ebay-list.SKUAttribute')">
                    <el-radio-group :value="form.list.variation"
                        ...
                        <el-radio-button :label="1" type="primary">{{this.$t('ebay-list.MultiAttribute')}}</el-radio-button>
                    </el-radio-group>
                </el-form-item>
                <el-form-item label="是否虚拟仓发货：">
                    <el-radio  v-model="form.list.is_virtual_send" class="ml-lg" :label="1">是</el-radio>
                    <el-radio  v-model="form.list.is_virtual_send" :label="0">否</el-radio>
                </el-form-item>
                ...
                <el-form-item label="引用模块组合：">
                    <el-row>
                        <el-col :span="commonSpan">
                            <super-select v-model="form.list.comb_model_id"
...
<script>
    import {api_get_account,api_sync_store,api_get_combList,api_get_specifics,api_ebay_oe_makes,
        api_edit_combList,api_suggested_categories} from '../../../../../api/ebay-kandeng-public-module';
...
//                siteList:[],
        ...
        computed:{
            TemplateName(){
                return this.$t('ebay-list.TemplateName')+'：'
            },
            placeHolder(){
        ...
        mounted(){
            this.get_model();
            this.dealAccountsList();
            this.init_attr();
        },
        methods:{
            dealAccountsList(){//账号简称
                this.$http(api_get_account,{}).then(res=>{
                    this.accountListArray = res.data.map(row=>{
                        return {
                            label:row.code,
                            value:row.id
                        };
            ...
            add_new_value(items){
                ...
                items.detail.push({category_specific_value:this.initial_uppercase(items.add_new)});
                items.specific_value.push(this.initial_uppercase(items.add_new));
                items.add_new = '';
            },
            update_category(){
            ...
            get_category(val,number,createFlag){
            ...
            get_individual(val){
                ...
                this.$http(api_edit_combList,params).then(res=>{
                    this.form.set.individual_listing = res.individual_listing;
                }).catch(code=>{
                    console.log(code);
                })
            },
            ...
            get_barginning(val){
                ...
                this.$http(api_edit_combList,params).then(res=>{
                    this.form.list.best_offer = res.best_offer;
                    this.form.list.buy_it_nowprice = res.accept_lowest_price;
                    this.form.list.reserve_price = res.reject_lowest_price;
                }).catch(code=>{
                    console.log(code);
                })
            },
            ...
            get_picture_gallery(val){
                ...
                this.$http(api_edit_combList,params).then(res=>{
                    this.form.set.picture_gallery = res.picture_gallery;
                }).catch(code=>{
                    console.log(code);
                })
            },
            ...
            get_choice(val){
                ...
                this.$http(api_edit_combList,params).then(res=>{
                    this.form.set.choice_date = res.choice_date;
                }).catch(code=>{
                    console.log(code);
                })
            },
            ...
            get_quantity(val){
                ...
                this.$http(api_edit_combList,params).then(res=>{
                    this.form.list.quantity = res.quantity;
                }).catch(code=>{
                    console.log(code);
                })
            },
            ...
            get_pickup(val){
                ...
                this.$http(api_edit_combList,params).then(res=>{
                    this.form.set.local_pickup = res.local_pickup;
                }).catch(code=>{
                    console.log("code",code);
                })
            },
            ...
            change_specific_k(val,index){
                ...
                this.form.specificsList.forEach(res=>{
                    if(res.custom===1){
                        res.category_specific_name = this.initial_uppercase(res.category_specific_name)
                    }
                });
                this.$emit("change-specific");
            },
            //首字母大写
            initial_uppercase(data){
                let name= data.split(/\s+/);
                name= name.map(row=>{
            ...
            get_model(){//模板
                ...
                let params = {
                    type:"comb",
                    siteId:this.form.list.site,
//                    ebay_account:this.form.published-records.account_id,
                };
                this.$http(api_get_combList,params).then(res=>{
                    this.modelList = res.data.map(val=>{return {label:val.model_name,value:val.id}});
                }).catch(code=>{
                    console.log(code);
                })
            },
            ...
            change_listing_type(val){
                ...
                if(val===2){//拍卖
                    this.form.list.variation = 0;//单属性
                    this.form.list.goods_type = 1;
                }else{//一口价
                    if(!this.selectCate){this.form.list.variation = this.form.varians&&this.form.varians.length>1?1:0;}
            ...
            //初始化时获取分类的sku属性
            init_attr(){
                let name = this.form.list.primary_categoryid;
                this.get_category(name,1,true);
            },
            //获取联动属性
               ...
               return  this.$http(api_get_specifics,params).then(res=>{
                   if(res.length<=0){
                       ...
                       res.forEach((row,index)=>{
                           if(row.category_specific_name==='MPN'){
                               row.max_values=1;
                           }
                           if(row.max_values>1){
