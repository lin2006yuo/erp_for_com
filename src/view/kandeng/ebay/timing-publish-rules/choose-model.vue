<template>
    <page-dialog v-model="show" title="选择范本" size="full" @open="open"
        ...
        <div class="mb-sm">
            ...
            <label-item label="平台账号：">
                <param-account v-model="params.account_id" url="get|Publish/Ebay/getAccounts"
                               class="mb-mini mr-sm width-sm" type="ebay"
                ></param-account>
            </label-item>
            <label-item label="站点：">
            ...
            <label-item label="创建人：">
                <el-input :label="$t('ebay-list.searchContent')"  v-model="params.realname"
                          class="width-xs inline mb-mini mr-sm"></el-input>
            </label-item>
            <label class=" mb-mini">创建时间：</label>
            <el-date-picker class="inline date width-sm" v-model="params.pub_start" type="date"
                            :picker-options="pickerstart" :placeholder="$t('ebay-list.sTime')"></el-date-picker>
            --
            <el-date-picker class="inline date mr-sm width-sm" v-model="params.pub_end" type="date"
        ...
        <el-table class="scroll-bar" v-loading="loading" v-if="tableData" style="width: 100%;"
            ...
            <el-table-column prop="site" label="站点" inline-template>
                <span>{{filterSite(row.site)}}</span>
            </el-table-column>
            <el-table-column label="出售方式" inline-template>
...
<script>
    import {api_save_timing_rule,api_listing_management,api_get_ebay_site,api_get_combList} from '../../../../api/ebay-kandeng-public-module'
    ...
    export default{
        ...
        data(){
            return{
                pickerstart:{
                    disabledDate:(time)=>{
                        if(this.params.pub_end){
                            return  time.getTime() > this.params.pub_end;
                        }else {
                            return false;
                        }
                    }
                ...
                pickerend:{
                    disabledDate:(time)=>{
                        if(this.params.pub_start){
                            return  time.getTime() < this.params.pub_start;
                        }else {
                            return false;
                        }
                    }
...
//                    size:this.pageSize
            ...
            init(){
                ...
                data.pub_time='create';
                data.pub_start = data.pub_start?datef("YYYY-MM-dd HH:mm:ss",data.pub_start/1000):'';
                data.pub_end = data.pub_end?datef("YYYY-MM-dd HH:mm:ss",data.pub_end/1000):'';
                data.page = this.page;
