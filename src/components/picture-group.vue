...
//                 $targetDirs = ['eBay' => 1, 'Amazon' => 2, 'Wish' => 3, 'AliExpress' => 4];
        ...
        methods:{
            ...
            get_cur_path(){
               ...
               this.comPath = !!this.channelValue?(this.path+'\\'+this.channelValue):(this.path+'\\New');
            },
            /*在父页面被调用*/
            init_data(){
                this.list = [];
            ...
            gain_channel_data(Array,channel_id){
                ...
                imgData.forEach(row=>{
                    row.images = row.images.map(row=>{
                        this.$set(row,'isCopy',false);
                        return row;
                    }).filter(res=>res.channel_id===channel_id);
                });
                // if(channel_id===0){
                //     cur =[];
                //     imgData.forEach(row=>{
