        ...
        <table class="template scroll-bar" v-if="form.income || form.expend">
            ...
            <template>
                <tr>
                    ...
                    <!--跌价金额-->
                    <td class="expend">{{expend.shipping_symbol}}{{expend.subsidy_total|filterable}}</td>
                    <!--paypal费用-->
                ...
                <tr>
                    ...
                    <!--成本/支出-->
                    <td colspan="9" class="expend">{{expend.shipping_symbol}}{{pay}}</td>
                </tr>
                <tr v-if="form.sales&&form.sales.length>0">
                    ...
                    <td class="bold-font" colspan="5">补发</td>
                    <td class="expend" :rowspan="form.sales.length+2" :class="totalFee>=0?'income':'expend'">
                ...
                <template  v-if="form.sales&&form.sales.length>0" v-for="item in form.sales">
                    <tr>
                        ...
                        <td class="expend" colspan="5">{{expend.shipping_symbol}}{{item.delivery_amount}}</td>
                    </tr>
                </template>
                <tr  v-if="form.sales&&form.sales.length>0">
...
<script>
    ...
    export default{
        ...
        filters: {
            filterable(val) {
                if (Number(val)===0) return   Number(val).toFixed(4);
                if (val) return parseFloat(val).toFixed(4);
            },
        },
        watch:{
            form(val){
        ...
        computed:{
            ...
            pay(){
                if(this.form){
                    let c = this.expend;
                    return (parseFloat(c.shipping_fee)+parseFloat(c.package_fee)+parseFloat(c.tariff)+parseFloat(c.first_fee)+parseFloat(c.channel_cost)+parseFloat(c.cost)+parseFloat(c.subsidy_total)+parseFloat(c.p_card)+parseFloat(c.paypal_fee)).toFixed(4);
                }else{
                    return '';
                }
            },
