<template>
    <el-row class="wish-has-published">
            ...
            <template v-for="(data, index) in publishList">
                <tr :class="[data.show ? 'active' : '',data.heighLight?'active-color':'',data.lock_update===1?'redborder':'']"
                    ...
                    <td style="position: relative">
                        ...
                        <div class="icon-wrap">
                            <i v-if="data.is_tort===1" class="pointer tort-icon" @click="showTort(data)">侵权</i>
        ...
        <tort-record v-model="tortShow" :title="tortTitle" :tortInfo="tortInfo" ></tort-record>
    </el-row>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            showTort(row){
                this.tortTitle = `${row.local_spu} 侵权记录`;
                this.tortShow = true;
            ...
            init_params() {
                ...
                if (data.start_time){
                    let params = new Date(data.start_time).getTime()/1000;
                    data.start_time = datef("YYYY-MM-dd",params);
                }
                if (data.end_time) {
                    let params = new Date(data.end_time).getTime()/1000;
                    data.end_time = datef("YYYY-MM-dd",params);
                }
            ...
            init() {
                ...
                if(!!data.min_weight && !!data.max_weight && Number(data.min_weight)> Number(data.max_weight) ){
                    this.$message({type:"error",message:"最大重量值不能小于最小重量值！"})
                    return
                }
                this.loading = true;
