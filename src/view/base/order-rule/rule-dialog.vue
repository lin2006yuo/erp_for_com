    ...
    export default {
        ...
        methods: {
            ...
            click_item(item) {
                ...
                this.showContext = true;  // 点击即会勾选上显示选项内容。
                this.panelWith = item;
                let result = this.get_item(item.code);    // 看下选中项中是否包含这个
                if (!result) {
                    // selected 代表已选择
                    this.selected.push({
                        name: item.name,
            ...
            showRules() {
                ...
                this.rules.forEach(classify => {
                    let classifyChild = classify.child.filter(row => {
                        ...
                        if (this.showSelect) {
                            let node = this.selected.find(old => old.code === row.code);  // selected代表已勾选项。
                            if (!node || !node.status) {
                                return false;  // 显示勾选项的话，在当前循环中未找到对应的值。
                            }
                            return true;
                        } else {
                            return true;  // 返回全部
                    ...
                    classifys.push({label: classify.label, child: classifyChild});  // 找到匹配的数据在次形成一个新的数据list
                });               
                return classifys;
            },
            showAutoContent() {
