<template>
    <page-dialog v-model="visible" @open="open" class="c-cardboard-look" size="large" @close="close"
        ...
        <div>
            ...
            <el-table :data="cardboardInfo.parcels_detail"
                ...
                <el-table-column label="是否拆分" min-width="55" inline-template>
                    <div :class="{red: row.is_use_text === '未拆'}">
                        {{row.is_use_text}}
                    </div>
                </el-table-column>
        ...
        <el-alert :title="showStr"
                  v-if="cardboardInfo.status === 5 && cardboardInfo.forced_finish_number.length > 0"
                  show-icon
                  class="mt-sm"
                  type="warning"
                  :closable="false">
...
<script>
    ...
    export default {
        ...
        computed: {
            showStr(){
                let tStr = [];
                if(this.cardboardInfo.forced_finish_number.length > 0){
                    this.cardboardInfo.forced_finish_number.forEach(row => {
                        if(row.tracking_number){
                            tStr.push(row.tracking_number)
                        }else{
                            tStr.push(row.purchase_parcel_code)
                        }
                    });
                    return `运单号: ${tStr.join(',')}`
                }else{
                    return ''
                }
            }
