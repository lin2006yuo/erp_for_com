...
//                shipping: [],
        ...
        methods: {
            ...
            get_shipping_methods(warehouse){
                ...
                let find2 = this.priceingConfigs.child.find(row=>row.code==='pricingShippingEUB');//获取平邮运输方式选择
                let findCountry = {};
                if(find&&find.resources&&find.resources.data){
                    ...
                    this.$http(api_get_shipping_method,data).then(res => {
                        if(find&&find.resources){
                            ...
                            findData = this.arrangement_shipping(findData,res);
                        }
                        if(find2&&find2.resources){
                            let findData2 = find2.resources.data.find(res=>res.title==='运输方式：');
                            findData2 = this.arrangement_shipping(findData2,res);
                        }
                    }).catch(code => {
                        console.log(code);
                    })
            ...
            arrangement_shipping(findData,list){
                if(findData){
            ...
            submit() {
                ...
                console.log(this.priceingConfigs.child,'this.priceingConfigs.child');
                let action_value = this.priceingConfigs.child.filter(row=>{
                    return !!row.resources.data
                }).map(row =>{
                    return {
                        item_source:row.code,
                ...
                console.log(params.action_value,'params.action_value');

                //TODO:这里打补丁用来判断物流挂号费设置是否合理 fixed：4263
                for(let i = 0; i <  params.action_value.length; i++){
                    if(params.action_value[i].item_source&&(params.action_value[i].item_source === "pricingLogisticsRegistrationFeeSetting")){
