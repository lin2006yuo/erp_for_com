        ...
        <mailing-method ref="mailing">
            ...
            <div slot="button-list">
                ...
                <template v-if="searchData.status === 2 || searchData.status === 7">
                    <el-button size="mini" class="ml-sm" @click="query(true)">批量队列交货</el-button>
                    <el-button size="mini" class="ml-sm" @click="query(false)">批量交货</el-button>
...
<script>
    ...
    import {
        api_get_collection,
        api_get_menu,
        api_batch_collection,
        api_batch_out_collection,
        api_get_packagelist,
        api_batch_queue_collection,
        api_invalid_collection,
    ...
    export default {
        ...
        methods: {
            ...
            query(flag) {
                ...
                let message = `确认对选中的${this.selecteds.length}条集包单号进行确认${flag ? '加入队列' : ''}发货操作？`;
                let api = flag ? api_batch_queue_collection : api_batch_collection;
                this.$confirm(message, '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    this.$http(api, {id: this.selecteds}).then(res => {
                        this.$message({type: "success", message: res.message || res});
