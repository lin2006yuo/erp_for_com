...
export const print = function (bool, flag) {
                            ...
                            if (!!this.printData[item.value]) {
                                if (this.printData[item.value]) {
                                    if (item.checked) {
                                        ...
                                        if(this.custom.hasOwnProperty(item.value)) {
                                            label += this.custom[item.value](this.printData[item.value]);
                                        } else {
                ...
                if (row.value === '_sn' && bool) {
                    ...
                    if (!!this.printData[row.value]) {
                        if(this.custom.hasOwnProperty(row.value)) {
                            row.label = this.custom[row.value](this.printData[row.value]);
                        } else {
    ...
    for (let i = 0; i < this.printData.print_num; i++) {
        let identification = i === this.printData.print_num - 1 ? this.printData.identification : this.printData.identification + i;
        this.numberList.push(identification);
    }
    if (bool) {
        //将要打印的内容拼接起来
