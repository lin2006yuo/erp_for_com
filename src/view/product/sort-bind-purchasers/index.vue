<template>
   <page>
       <search :search-data="searchData" :clears="clears" @search="search" :purchasing="purchasing" :skuManager="skuManager" :secretary="secretary" class="mb-xs"></search>
...
<script>
    import {getUser,api_get_categories,api_get_log,getSkuManager} from '../../../api/sort-bind-purchasers';
    export default {
        page:{
            devinfo:{
                frontEnd:'陆城锫',
        ...
        methods: {
            ...
            get_data(){
                ...
                //获取sku查重文员
                this.$http(getSkuManager).then(res=>{
                    this.skuManager = res.map(row=>{
                        return {
                            label:row.realname,
                            value:row.id
                        }
                    });
                    // this.skuManager.unshift({id:'',realname:'全部'});
                }).catch(err=>{
                    this.$message({
                        type:'error',
            ...
            log_operate(val){
                ...
                this.$http(api_get_log,id).then(res=>{
                    ...
                    this.dialog_data.forEach(row=>{
                        ...
                        let name = t1[1].split(';')[1];
                        let t2 = t1.splice(t1.length-1,1);
                        let t3 = t2.join('=>');
                        let t4 = t3.split('=>');
                        let title = `将${name}${t4[0]}修改为${t4[1]}`;
            ...
            check(val){
                let name = val.name_path.replace(/-/g,'>>');
                this.copyPurchasing = JSON.parse(JSON.stringify(this.skuManager));
                this.copyPurchasing.forEach((row,index,arr)=>{
                    if(row.label.includes('全部')){
                        arr.splice(index,1)
                    }
                ...
                this.resId=val.id;
                this.operate_show=true;
                this.purchaser=val.sku_checking;
                this.purchaser_id=val.sku_checking_id;
                this.title=`修改分类: ${name} 绑定的SKU查重文员`;
            },
            init(){
                this.loading=true;
