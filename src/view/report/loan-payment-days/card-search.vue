        ...
        <search-card @search="search" :params="form" :clears="clears">
            ...
            <label-item label="账号简称：" class="ml-sm">
                <select-account
                    ...
                    type="account"
                    ref="selectAccount"
                    :param="{channel_id:form.channel_id}"
                ></select-account>
            ...
            <el-select v-model="form.time_type" class="inline width-xs ml-sm">
                <el-option v-for="(item) in timeTypes" :value="item.value" :key="item.value"
    ...
    export default {
        ...
        watch:{
            "form.channel_id"(val) {
                ...
                // this.get_site_account(val);
                // this.get_account(val)
            },
        },
        mounted(){
            this.channel_remote();
            this.get_site_account(this.form.channel_id);
        },
        methods: {
            search(){
                this.$emit('search')
            ...
            get_site_account(channel_id){
                ...
                this.$http(api_account_list,{channel_id:channel_id}).then(res=>{
                    if(res.site.length<=0){
                        ...
                        let res_site = res.site;
                        this.siteOptions = [{label:"全部",value:""},...res_site];
                    }

                    if(res.account.length<=0) {
                        this.accoutOptions = [{label:"",value:""}]
                    } else {
                        let res_account = res.account;
                        this.accoutOptions = [{label:"全部",value:""},...res_account];
                    }
                }).catch(code=>{
                    console.log(code);
                })
