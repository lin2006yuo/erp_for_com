    ...
    <page-dialog class="c-apply-stock-in" v-model="visible" @open="open" @close="close" title="申请借调库存">
        <el-form :model="form" label-width="80px" :show-message="true" :rules="rules" ref="applyForm">
            <el-row>
                ...
                <el-col :span="10">
                    <el-form-item class="inline" label="用途类型：" prop="type">
                        <el-select v-model="form.type" class="s-width-default">
                            <el-option v-for="channel in typeList"
                                       :value="channel.value"
                                       :label="channel.label"
                                       :disabled="channel.disabled"
                                       :key="channel.value"></el-option>
        ...
        data() {
            return {
                ...
                rules: {
                    ...
                    type: [
                        {type: 'number', required: true, message: '请选择用途类型', trigger: 'blur'},
                    ],
                    quantity: [
