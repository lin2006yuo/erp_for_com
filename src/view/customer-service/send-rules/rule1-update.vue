<template>
    <span :class="!!$slots.default?'':'rule1'">
        <b :class="$slots.default?'rule1-parent' : ''" :title="form.title">
            ...
            <template v-if="election_title">
                <input v-model="election" @change="electionChange($event)" type="checkbox">
...
<script>
    ...
    export default{
        ...
        methods:{
            ...
            electionChange(e){
                this.$emit('election-change',e.target.checked);
            }
        },
        computed:{
            condition(){
