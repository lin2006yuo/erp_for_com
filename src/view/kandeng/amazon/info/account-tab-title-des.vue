                        ...
                        <template v-if="item.select === 0">
                            <el-form-item v-if="item.key==='Title'"
                                ...
                                <el-button style="margin-left: 30px;" type="primary" size="mini"
                                           @click="initial_uppercase(row,i,index)">{{$t('ymx-detail.upperCase')}}
                                </el-button>
                            </el-form-item>
...
//                            res.field = clone(row);
            ...
            initial_uppercase(row,index,row_index){
                if(!row.field[index].value) return this.$message({message: this.$t('ymx-detail.enterTitle'), type:'warning'});
                if(row_index!==0){
                    this.row_uppercase(row,index);
                }else{
                    this.form.descript.forEach(res=>{
                        this.row_uppercase(res,index);
                    })
                }
            },
            row_uppercase(row,index){
