        ...
        <rainbow-header title="SKU设置" >
            ...
            <table class="template">
                <tr>
                    ...
                    <th width="50px">{{$t('ebay-list.operat')}}</th>
                </tr>
                <tbody>
                    <tr>
                        <th>
                        ...
                        <!--<th v-if="showRef">-->
                            ...
                            <!--&lt;!&ndash;<el-button type="primary"&ndash;&gt;-->
                                       ...
                                       <!--&lt;!&ndash;@click.native="make_application"&ndash;&gt;-->
                                       <!--&lt;!&ndash;class="inline"></el-button>&ndash;&gt;-->
                        <!--</th>-->
                        <!--修改价格-->
                        <th>
                            ...
                            <span  class="btn-icon" title="点击初始化刊登销售价数据" @click="get_standard_price()"></span>
                        </th>
                        <!--修改数量-->
                        <th>
            ...
            <div>
                ...
                <el-radio class="radio" v-model="form.list.assoc_order" :label="0">订单不关联商品(将转为"商品未知订单")</el-radio>
            </div>
...
<script>
    ...
    export default{
        ...
        methods:{
            get_standard_price(){
                let letGo =this.form.varians.every(res=>{ return !!res.v_price});
                if(letGo){
            ...
            get_v_price(){
                let skuList = this.form.varians.map(row => row.sku_id);
                let params = {
                    channel_id: 1,
                    account_id: this.param.default_account_id,
                    site_code: this.param.site_code,
                    sku_id: skuList,
                };
                this.$http(api_pricing_rules,params).then(res=>{
                    let errorMes = false;
                    this.form.varians.forEach(val => {
                        if(!!res[val.sku_id].total_price){
                            val.v_price = res[val.sku_id].total_price;
                            // this.reference_sale_price(val);
                        }else {
                            errorMes = true;
                        }
                    })
                    if(errorMes){
                        this.$message({message:'该产品暂无初始刊登定价数据',type:'error'})
                    }
                }).catch(code=>{
            ...
            delete_mappingspec(item){
                ...
                this.form.mappingspec.forEach(res=>{
                    if(res.channel_spec ===item.category_specific_name){
                        this.start_using(res,false)
                    }
                })
...
//                        }
            ...
            bundling_sales(row,index){
                ...
                // console.log(this.transferData,combine_sku,row)
            },
