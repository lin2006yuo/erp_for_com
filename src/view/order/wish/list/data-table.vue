...
<script>
    ...
    export default {
        ...
        methods: {
            order_export(params) {
                return this.$http(api_wish_orders_export, params).then(res => {
            ...
            get_fields() {
                this.$http(api_wish_order_export_fields).then(res => {
                    this.fields = res.map(row => ({
                        key: row.title,
                        title: row.remark
                    }));
                }).catch(code => {
            ...
            get_templates() {
                this.$http(api_goods_export_template, {type: 29}).then(res => {
                    res.forEach(row => {
                        row.value = row.name;
                    });
                    this.templates = res;
