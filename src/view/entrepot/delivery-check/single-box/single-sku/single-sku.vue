                ...
                <div class="box mt-sm">
                    <table class="sku-table" cellpadding="0" cellspacing="0">
                        <template>
                            ...
                            <tr>
                                <td style="text-align: right;">商品尺寸</td>
                                <td><span :class="{red: skuData.over_weight === 1, 'bold-font': skuData.over_weight === 1}"
                                          v-if="skuData.length || skuData.width || skuData.height"
                                >长：{{skuData.length}} cm&nbsp;宽：{{skuData.width}} cm&nbsp;高：{{skuData.height}} cm</span></td>
                            </tr>
                            <tr>
                                <td style="text-align: right;">商品单重</td>
                                <td><span >{{skuData.weight}}g</span></td>
                            </tr>
                            <tr>
    ...
    export default {
        ...
        methods: {
            ...
            submit() { // 扫描sku
                ...
                if (this.skuData.package_count === this.skuData.check_count || this.skuData.sku === '' || this.skuData.package_status === 2 || this.skuData.package_status === 3 || this.skuData.package_status === 4) {
                    ...
                    this.$http(api_single_sku, params).then(res => {
                        ...
                        this.skuData = res;
                        let shipping_type = res.properties.find(row => row.enabled === true);
                        this.$set(this.skuData, 'shipping_type', !!shipping_type ? shipping_type.name : '');
            ...
            http_print(id, res) {//http打印
                ...
                localWebSocket({
                    url: 'ws://127.0.0.1:10091/websocket',
                    open: () => {
                        local_websocket.connected();
