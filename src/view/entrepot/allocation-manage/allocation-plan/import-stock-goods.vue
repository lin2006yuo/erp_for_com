        ...
        <div class="stock-goods"
            ...
            <div class="stock-goods-content"
                ...
                <el-table class="scroll-bar"
                          ...
                          @select="save_code(stock)"
                          @select-all="save_code(stock)"
                          :data="stock.detail">
                    ...
                    <el-table-column label="可用锁定数量" width="100" inline-template>
                        <span>{{row.usable_allocated_qty}}</span>
                    </el-table-column>
                </el-table>
            </div>
        </div>
...
<script>
    import {api_stock_list_by_code} from "@/api/transfer-plan";
    ...
    export default {
        ...
        methods: {
                        ...
                        stock.list.forEach(row => {
                            ...
                            if (!!sku) {
                                sku.stocking_info.push({
                                    ...
                                    usable_allocated_qty: Number(row.usable_allocated_qty),
                                    qty: Number(row.allocated_qty),
                                    apply_id: row.ids,
                                    from_type: this.curType,
                                ...
                                if(this.curType === 2) this.$set(sku, 'channel_id', row.channel_id);
            ...
            save_code({code, id, from_type}) {
                this.curCode = code;
                this.curId = id;
                this.curType = from_type;
            },
            search_img(image) {
