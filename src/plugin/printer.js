export default {
    install(Vue, options) {
        options = options || {global: {url: 'http://127.0.0.1:10093'}};
            ...
            created() {
                this.$printersInit = () => {
                    ...
                    /*if (local_websocket && local_websocket.IsOpen) {
                        ...
                        local_websocket.sendRequest(param, (res) => {
                            if (res.status === 1) {
                                this.$store.dispatch('printer/globals', res.pint_list.map(row => {
                                    return {
                                        label: row,
                                        key: `${options.global.url}/${row}`,
                                        value: JSON.stringify({
                                            url: options.global.url,
                                            name: row
                                        }),
                                    }
                                }));
                            } else {
                                this.$message({type: 'error', message: '获取打印机列表失败'});
                            }
                        });
                    } else {
                        localWebSocket({
                            ...
                            error: () => {
                                console.info(`xxxxxx:printer error`, arguments);
