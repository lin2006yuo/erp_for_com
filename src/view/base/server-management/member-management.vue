<template>
    <page-dialog v-model="show"
        ...
        <el-row class="mb-xs mt-xs ml-sm">
            <label-buttons class="inline" label="状态：" :buttons="statsList" @select="select_type"></label-buttons>
        </el-row>
        <el-row class="mb-xs mt-xs">
            ...
            <el-button @click.native="add_push"
        ...
        <el-table class="scroll-bar"
            ...
            <el-table-column
                    ...
                    label="状态">
                <div>
                    <ui-tip :content="row.status" :width="98" v-copy></ui-tip>
                </div>
            </el-table-column>
            <el-table-column
                    inline-template
                    label="添加时间">
                <div>
                    <ui-tip :content="row.user.create_time_txt" :width="98" v-copy></ui-tip>
                </div>
            </el-table-column>
        </el-table>
...
<script>
    ...
    import {api_add_save_user_info,api_delete_user_info,api_delete_again_info} from '@/api/server-management'
    export default {
        ...
        methods: {
            select_type(val, item){
                ...
                this.$emit('submit',this.search_type);
            },
            add_inner(){
            ...
            add_push(){
                this.$confirm(`您将推送所选中的数据', 是否继续?`, '提示', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    let user_ids=this.outsideSelectData.map(row=>row.user.user_id);
                    this.$http(api_delete_again_info,this.serverId,{user_ids:user_ids}).then(res=>{
                        this.$message({type:'success', message:res.message||res});
                        this.$emit('submit',this.search_type)
                    }).catch( error =>{
                        error&&this.$message({type:'error', message:error.message||error});
