<template>
    <search-card :params="searchData" :clears="clears" @search="search">
        <label-item label="账号简称：">
            <!-- <el-select v-model="searchData.account_ids"
                       class="inline s-width-small"
                       filterable clearable>
                <el-option
                        v-for="res in accountShort"
                        :label="res.label"
                        :value="res.value"
            ...
            <!--</param-account>-->
            <super-select   v-model="searchData.account_ids"
                            :options="accountShort"
                            storageKey="wishSettingAccountOptions"
                            :placeholder="'请选择'"
                            class="s-width-small">
            </super-select>
        </label-item>
        <label-item class="ml-sm" label="销售员：">
            ...
            <super-select   v-model="searchData.seller_id"
                            :options="sellerList"
                            storageKey="wishSettingSellerOptions"
                            :placeholder="'请选择'"
                            class="s-width-small">
            </super-select>
        </label-item>
...
<script>
    import {api_get_wish_users,api_get_account_short} from '@/api/wish-cash-account'
    export default {
        data() {
            return {
                sellerList:[],
        ...
        methods: {
            get_account(){
                this.$http(api_get_account_short).then(res=>{
                    this.accountShort=res.data.map(row=>{
