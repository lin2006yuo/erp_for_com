...
import  {url_depart_status,url_depart_check,url_depart_del,url_depart_edit,url_depart_logs} from "../../../api/department"
const operates = [
    ...
    {name:'日志', api:url_depart_logs, parent:true, action(Class){
            this.journal(Class)
        }},
...
import {api_get_channel} from '../../../api/department';
export default {
    components:{
    ...
    render(h){
        ...
        let span=(<span></span>);
        return (
        <ui-table body-height={10} heads={[{label:'ID',width:5},{label:'部门名称',width:24},{label:'部门代码',width:8},{label:'部门类型',width:6},{label:'部门负责人',width:11},{label:'描述',width:13},{label:'状态',width:10},{label:'所属平台',width:10},{label:'菜单操作',width:13}]}>
            <tbody>
            {childs}
            </tbody>
        </ui-table>
