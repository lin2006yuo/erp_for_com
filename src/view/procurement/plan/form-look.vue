    ...
    <page-dialog size="full" v-model="show" :title="title" @open="open" class="p-form-look" @change="change_dialog"
        ...
        <div style="margin-top:5px" class="p-form-look-table">
            <el-table :data="filterGoodsLists"
                ...
                <el-table-column label="活动备货数" min-width="40" inline-template>
                    <div>
                        <ui-tip :content="row.ready_qty" :width="98"></ui-tip>
                    </div>
                </el-table-column>
        ...
        <div>
            <el-table
                ...
                <el-table-column label="贴标/套袋" inline-template>
                    <span>{{ lookData.label_deck|filterLabelDeck}}</span>
                </el-table-column>
                <el-table-column label="是否能退货" inline-template>
                    <span>{{ lookData.return_goods|filterReturnGoods}}</span>
                </el-table-column>
                <el-table-column label="退货天数" inline-template>
                    <span v-if="lookData.return_goods===1">{{ lookData.return_goods_data|filterReturnGoodsData }}</span>
                    <span v-else>--</span>
                </el-table-column>
                ...
                <el-table-column label="供应链金融" inline-template>
                    <span>{{lookData.supply_chain_finance_text}}</span>
                </el-table-column>
                <el-table-column label="发票类型" inline-template>
                    <span>{{lookData.invoice_text}}</span>
                </el-table-column>
                <el-table-column label="开票加收税点" inline-template>
                    <span>{{lookData.tax_rate_text | filterAmount}}</span>
                </el-table-column>
                ...
                <!--:colspan="selectMark==='purchase_order_overcharged' ? 13 :14"-->
            </el-table>
        </div>
        ...
        <card label="操作日志" class="mt-sm" v-show="loghidden">
            <el-table
                ...
                <el-table-column
                    ...
                    inline-template
                     min-width="595">
                    <span class="ellipsis" :title="row.operate_content">{{row.operate_content}}</span>
                </el-table-column>
            </el-table>
        </card>
        <!--常用备注   一直存在页面-->
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            btnclick() {
                console.log(this.filterSupplierBuyers,'filterSupplierBuyers');
        ...
        computed: {
            ...
            filterSupplierBuyers() {
                let shipFee = this.lookData.ship_fee_discount_json_1 || [];
                let ret = {};
                this.goodsList.forEach(goods => {
                    ...
                    ret[key] = ret[key] || {
                        ...
                        description: ship.description||this.lookData.description,
                        goods: []
                    };
