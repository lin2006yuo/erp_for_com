                    ...
                    <div v-loading="loading" element-loading-text="玩命加载中...">
                        ...
                        <div class="collapse" v-if="list.length>0">
                            <ui-collapse v-for="(item,item_i) in list"
                                         ...
                                         :class="`${ item.isCheck?'active':''} ${item.emails.find(email=>email.is_read === 0)?'bold-font':''} amazon-msg-list`">
                              ...
                              <div class="inline fr">
                                ...
                                <div>
                                    <div v-for="(message,m_i) in filterMessages(item)"
                                         ...
                                         :class="`${message.is_read===0?'bold-font':''} mb-xs`"
                                        ...
                                        <el-card :class="{'active-sm':message.isCheck}">
                                            ...
                                            <div class="flex">
                                                ...
                                                <div v-if="get_src(message)">
                                                    <template v-for="(attachment,a_i) in get_src(message)">
                                                        <el-popover
                                                            placement="right"
                                                            v-if="/\.png$|\.jpg$|\.jpeg$/i.test(attachment.path)"
                                                            trigger="hover">
                                                            <img v-lazy="attachment.path"
                                                                 width="200px"
                                                                 height="200px"
                                                                 :key="`${attachment.path}${a_i}`"
                                                                 style="border:1px solid #ccc;border-radius: 2px">
                                                            <span slot="reference">
                                                                <img v-lazy="attachment.path"
                                                                     width="60px"
                                                                     height="60px"
                                                                     @click="open_attachment(attachment.path,$event)"
                                                                     :key="`${attachment.path}${a_i}`"
                                                                     style="border:1px solid #ccc;border-radius: 2px"></span>
                                                        </el-popover>
                                                        <br v-if="a_i>0&&a_i%5===0"/>
                                                    </template>
                                                    <div v-for="(attachment,a_i) in get_src(message)">
                                                        <span v-if="/\.pdf$/i.test(attachment.path)"
                                                              class="operate"
                                                              :key="`${attachment.name}${a_i}`"
                                                              @click="open_attachment(attachment.path,$event)"
                            ...
                            <div v-if="detail.id" style="height: 100%;display: flex;flex-direction: column">
                                <div style=" background:rgba(173,227,255,.5);padding: 5px;border-radius: 5px"
                                    ...
                                    <div style="min-height: 30px">
                                        ...
                                        <span class="seller"
                                              v-else-if="/@marketplace.amazon/.test(detail.receiver)">卖</span>
                                        <span v-else><img src="../../../../assets/ymx.png"></span>
                                        <template v-if="detail.buyer_id">
                                            <span class="bold-font"
                                                  style="word-break: break-all;">{{detail.buyer_id}}</span><br>
                                        </template>
                                        <span class="bold-font"
                                              style="word-break: break-word;">{{detail.subject}}</span>
                                    </div>
                                    <div>
                                ...
                                <div class="max-heigh_single">
                                    <iframe style="width:100%;height:100%;" scrolling="yes" :srcdoc="detail.body"
                                            ref="singleFrame" v-if="detail.box_id === 1"></iframe>
                                    <div v-html="filterBody(detail.body)" v-else></div>
                                </div>
                            </div>
                                ...
                                <el-card v-for="(de,de_i) in details" :key="`${de.buyer_id}${de_i}`" class="mb-xs">
                                    <div style=" background:rgba(173,227,255,.5);padding: 5px;border-radius: 5px"
                                        ...
                                        <div style="min-height: 30px">
                                            ...
                                            <span class="seller"
                                                  v-else-if="/@marketplace.amazon/.test(de.receiver)">卖</span>
                                            <span v-else><img src="../../../../assets/ymx.png"></span>
                                            <template v-if="de.buyer_id">
                                                <span class="bold-font"
                                                      style="word-break: break-all;">{{de.buyer_id}}</span><br>
                                            </template>
                                            <span class="bold-font"
                                                  style="word-break: break-word;">{{de.subject}}</span>
                                        </div>
                                        <div>
                                            <span class="bold-font"
                                                  style="color:#3300FF">{{de.sender}}→:{{de.receiver}}</span>
                                        </div>
                                    </div>
                                    <div class="max-heigh">
                                        <iframe style="width:100%;height: 370px;overflow-x: hidden" scrolling="yes"
                                                :srcdoc="de.body" ref="mulFrame" v-if="de.box_id === 1"></iframe>
                                        <div v-html="filterBody(de.body)" v-else></div>
                                    </div>
                                </el-card>
                            </div>
                            <div class="mt-xs" style="height:30px;">
                ...
                <el-card class="mt-lg" v-resize="{height:20}">
                    <div>
                        ...
                        <el-row>
                            ...
                            <el-select v-model="more" placeholder="请选择"
                                       class="inline ml-xs" @input="change_temp" filterable>
                                <el-option
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            init_() {
                this.change_status();
                this.$page.param.resData.data.forEach(row => {
                    row.page = 1;
                });
            ...
            get_src(item) {
                if (Array.isArray(item.attachments) && item.attachments.length) {
                    return item.attachments.map(row => ({name: row.name, path: config.apiHost + row.path}))
                } else {
                    return false
                }
            },
            open_attachment(path,e) {
                e.stopPropagation();
                e.preventDefault();
                window.open(path);
            },
            ...
            load_more(item, type) {
                ...
                if (this.show_load(item)) {
                    this.$http(api_amazon_old_email, item.group_id, {
                        ...
                        item.page++;
                        // oldEmails = window.clone(item.emails);
            ...
            init() {
                ...
                this.$http(api_amazon_list, data).then(res => {
                    ...
                    this.list.forEach(row => {
                        ...
                        this.$set(row, 'isCheck', false);
                        this.$set(row, 'moreSize', 500);
                        this.$set(row, 'isHidden', false);
                        this.$set(row, 'isReply', false);
                    });
            ...
            look_detail(item, itemList) {
                ...
                this.temp_init();
                if (this.searchData.is_replied === 2) {
                    this.list.forEach(row => {
                        if (row.isReply) {
                            row.isHidden = true;
                        }
            ...
            replay() {
                if (this.searchData.is_replied === 2) {
                    let find = this.list.find(row => !!row.emails.find(item => item.id === this.detail.id));
                    if (!!find) {
                        find.isReply = true;
                    }
                }
                ...
                this.$http(api_amazon_reply, data).then(res => {
                    ...
                    if (this.searchData.is_replied === 2) {
                        let find = this.list.find(row => !!row.emails.find(item => item.id === data.reply_email_id));
                        if (!!find) {
                            find.isReply = true;
                        }
                    }
            ...
            filterBody(val) {
                if (typeof val === 'string') {
                    let replaceArr = val.match(/<pre[^>]*>([\s\S]*?)<\/pre>/gi);
                    if (!!replaceArr) {
                        replaceArr.forEach(str => {
                            val = val.replace(str, `<div>${/<pre[^>]*>([\s\S]*?)<\/pre>/.exec(str)[1]}</div>`);
                        });
                    }
                    return val
                }
