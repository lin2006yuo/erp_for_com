<template>
    <page class="smt-inbox">
        ...
        <el-row :gutter="8">
                            ...
                            <span slot="title">
                                <el-row class="inline lookClass" >
                                    <div class="inline">
                                        ...
                                        <span v-else class="inline" :class="{color:isColor(item.message_type) }" @click="look_platfrom(item)" >{{item.type_id}}</span>
                            ...
                            >
                                <td>
                                    <div >
                                        ...
                                        <ui-tip :content="list.order_no" :width="50" @cur-click="look(list,'look','table')" class="operate"></ui-tip>
                                    </div>
                                </td>
                                <td>{{list.country ? list.country : ''}}</td>
                                <td>{{list.currency_code ? list.currency_code : ''}}{{parseFloat(list.pay_amount).toFixed(2)}}</td>
                                <td>
                                    <el-popover placement="right"
                                        ...
                                        <div>
                                            <table class="template">
                                                ...
                                                <tr>
                                                    <td>邮寄方式编码:</td>
                                                    <td>{{list.logistics_type_code}}</td>
                                                </tr>
                                                <tr>
                                                    <td>邮寄方式名称:</td>
                                                    <td>{{list.logistics_service_name}}</td>
                                                </tr>
                                            </table>
                                        </div>
                                        <span slot="reference">
                               <span>
                                   <span>{{list.order_status ? list.order_status : ''}}</span>
                                    <i class="el-icon-document"></i>
                               </span>
                            </span>
                                    </el-popover></td>
                                <td>
                                    <ui-tip :content="list.evaluate ? list.evaluate: ''"></ui-tip>
                                </td>
                                <td>
                                    <ui-tip :content="list.issue ? list.issue : ''"></ui-tip>
                                </td>
                                <td><ui-tip :content="list.return ? list.return : ''"></ui-tip></td>
                            </tr>
                            <template v-if="list.show">
                                <tr >
                                    <th class="th-color">付款日期</th>
                                    <th class="th-color">发货日期</th>
                                    <th class="th-color">追踪号</th>
                                    <th class="th-color">物流商</th>
                                    <th class="th-color">SKU</th>
                                    <th class="th-color">数量</th>
                                    <th class="th-color">预计到达时间</th>
                                ...
                                <tr :key="`v${list.id}`" >
                                    <td>
                                        <el-popover placement="bottom"
                                                    trigger="hover"
                                                    :open-delay="500">
                                            <span >{{list.pay_time|fmsdatetime}}</span>
                                            <span slot="reference">{{(list.pay_time*1000)|fymd}}</span>
                                        </el-popover>
                                    </td>
                                    <td>
                                        <el-popover placement="bottom"
                                                    trigger="hover"
                                                    :open-delay="500">
                                            <span >{{list.shipping_time|fmsdatetime}}</span>
                                            <span slot="reference">{{(list.shipping_time*1000)|fymd}}</span>
                                        </el-popover>
                                    </td>
                                    <td>
                                        <ui-tip :content="list.shipping_number ? list.shipping_number : ''"></ui-tip>
                                    </td>
                                    <td>
                                        <ui-tip :content="list.shipping_name ?  list.shipping_name: ''"></ui-tip>
                                    </td>
                                    <td>
                                        <ui-tip :content="list.sku ? list.sku : ''"></ui-tip>
                                    </td>
                                    <td>
                                        <ui-tip :content="list.sku_quantity ? list.sku_quantity : ''"></ui-tip>
                                    </td>
                                    <td>
                                        <!--<el-popover placement="bottom"-->
                                                    <!--trigger="hover"-->
                                                    <!--:open-delay="500">-->
                                            <!--<span >{{list.arrival_time|fmsdatetime}}</span>-->
                                            <!--<span slot="reference">{{(list.arrival_time*1000)|fymd}}</span>-->
                                        <!--</el-popover>-->
                                        <ui-tip :content="list.arrival_time"></ui-tip>
                                    </td>
                                </tr>
                            </template>
                        </template>
            ...
            <el-col :span="14">
                <el-card class="aliexpress-msg-right-card ebay-right-card aliexpress-msg">
                    <div v-if="detail.length===0">暂无数据</div>
                    <div v-for="(data,data_i) in filterBody(detail)" :key="`${data.gmt_create}${data_i}`" @click="message_click(data)">
                        <el-card :class="['mb-xs',data.is_own===0?'buyer-background':'',data.isCheck?'right-active':'','pointer']">
                            ...
                            <div class="inline" v-if="data.file_path.length>0"
                                ...
                                <el-popover
                                    ...
                                    <span slot="reference">
                                         <img :src="img.sPath" width="55px" height="26px" style="cursor: -webkit-zoom-in;">
                                </span>
                                </el-popover>
                            </div>
                        </el-card>
                ...
                <el-card style="margin-top:5px" v-resize="{height:2}">
                    <div>
                        ...
                        <el-row>
                            ...
                            <el-select v-model="more" placeholder="请选择" class="inline" style="width:200px" @change="change_temp" filterable>
                                <el-option
        ...
        ></form-mdf>
        <order-form v-model="platformOrders" :form="platformForm" ></order-form>
...
<style lang="stylus">
    .smt-inbox{
        ...
        .aliexpress-msg-right-card,.ebay-right-card,.aliexpress-msg {
            .primary-cta-button {
        ...
        methods: {
            ...
            filterBody(val){
                if(typeof val === 'string'){
                    let replaceArr = val.match(/<a[^>]*>([\s\S]*?)<\/a>/gi);
                    if(!!replaceArr) {
                        replaceArr.forEach(str => {
                            val = val.replace(str, /<a[^>]*>([\s\S]*?)<\/a>/.exec(str)[1]);
                        });
                    }
                    return val
                }
            ...
            listFun_(){
                ...
                this.detail = [];
                this.tableData = [];
                this.list = this.$page.param.resData.data;
                this.total = this.$page.param.resData.count;
            ...
            look_platfrom (row) {
                ...
                this.platformOrders = false;
                if (row.message_type === 2) {
                    this.$http(aliexpress_check,row.type_id).then(res=>{
                        this.platformForm = res;
                        console.log(this.platformForm)
                    }).catch(code=>{
                        this.$message({message:code.message||code,type:'error'});
                    });
                    this.platformOrders = true;
                    let i = 0
                    console.log(i++)
                } else {
                    this.platformOrders = false;
