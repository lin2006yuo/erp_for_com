        ...
        <div v-show="locations[cf_index].is_remote" class="fee-box" v-for="(countryFee, cf_index) in remote" :key="`country_${cf_index}`">
            ...
            <div class="inline fee-info">
                <div class="fee-item" v-for="(districetFee, df_index) in countryFee.detail" :key="`district_${df_index}`">
                    ...
                    <span>首重：每 </span><el-input :disabled="!editable" v-number:fixed="0" v-model="districetFee.first_weight" class="inline input-width-xs"></el-input><span> g 收取 </span><el-input :disabled="!editable" v-number:fixed="2" class="inline input-width-xs" v-model="districetFee.first_fee"></el-input><span class="mr-lg"> 元</span>
                    <span>续重：每 </span><el-input :disabled="!editable" v-number:fixed="0" class="inline input-width-xs" v-model="districetFee.additional_weight"></el-input><span> g 收取 </span><el-input :disabled="!editable" v-number:fixed="2" class="inline input-width-xs" v-model="districetFee.additional_fee"></el-input><span class="mr-lg"> 元</span>
...
export default {
    ...
    methods: {
        ...
        city_tip(citylist) {
            return !citylist ? '' : citylist.reduce((pre, cur) => {
                return pre + cur.name + ', '
            }, '')
        },
        // select_nav() {
    ...
    watch: {
        locations: {
            handler() {
                //准备数据
                let remoteCache = {};
                this.remote.length !== 0 && this.remote.forEach(item => {
                    if(!remoteCache.hasOwnProperty(item.country_code)) {
                        remoteCache[item.country_code] = item;
                    }
                });
                this.remote.splice(0);
                this.locations.forEach((item, index) => {
                    if(remoteCache.hasOwnProperty(item.country_code)) {
                        this.remote.push(remoteCache[item.country_code]);
                    } else {
                        this.remote.push(
                            {
