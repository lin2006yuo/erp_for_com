    ...
    <page-dialog class="c-add-category" title="添加新用户"
        ...
        <div class="add-role-user">
                    ...
                    <el-row>
                        <el-col :span="spanWidth">
                            <el-form-item label="密码：" prop="password">
                                ...
                                <el-button class="inline ml-xs" type="primary" size="mini"
                                           @click.native="create_password">生成密码
                                </el-button>
                            </el-form-item>
                        </el-col>
                    </el-row>
                    ...
                    <el-row>
                        <el-col :span="spanWidth">
                            <el-form-item label="在职状态：" prop="job_status">
                                <el-select v-model="addForm.on_job" placeholder="请选择">
            ...
            <card label="角色">
                ...
                <el-row v-show="role_id.length" class="mt-md mb-md border">
                    <el-button v-for="(item, index) in roleList"
                               class="mb-sm sure-color"
                               :title="item.remark"
                               :class="{'ml-xs': index === 0}"
                               v-show="role_id.includes(item.id)" :key="index">
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            /** 删除选中的角色。 */
            deleteSelect(id) {
                let index = this.role_id.findIndex(item => item === id);
                this.role_id.splice(index, 1);
            },
            /** 添加角色 */
            addSelect() {
                this.selectIdList = clone(this.role_id);
                this.selectDialog = true;
            },
            /** 关闭确定选择的角色。 */
            select_role() {
                this.role_id = clone(this.selectIdList);
                this.addForm.role_id = this.role_id.join(",");
            },
