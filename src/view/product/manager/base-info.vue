        ...
        <!--<card label="产品信息">-->
            ...
            <el-form :model="baseData" label-width="140px" :rules="rules" ref="base">
                ...
                <el-form-item label="品牌1：" class="changeW" required  prop="brand_id">
                    ...
                    <!--</el-select>-->
                    <super-select v-model="baseData.brand_id"  v-if="editAble"
                                  @change="change_selects(baseData.brand_id,unitData[1].brand,'brand')"
                                  class="pri-width" storageKey="brandList"
                                  :options="unitData[1].brand"></super-select>
                ...
                <el-form-item label="开发员：">
                    ...
                    <!--</el-select>-->
                    <super-select v-model="baseData.developer_id" ref="developer"
                                  @change="change_selects(baseData.developer_id,development,'developer')"
                                  class="pri-width" v-if="editAble"
                                  storageKey="development"
                                  :options="development"></super-select>
                ...
                <el-form-item label="商品尺寸："
                    ...
                    <label>长：</label>
                    <span v-if="!editAble"
                          class="inline">{{baseData.net_depth}}</span>
                    <ui-limit-number
                            class="inline width-xs"
                            v-else v-model="baseData.net_depth"
                            :limit="1"></ui-limit-number>
                    <label>cm</label>
                    <label class="ml-xs">宽：</label>
                    <span v-if="!editAble"
                          class="inline">{{baseData.net_width}}</span>
                    <ui-limit-number
                            class="inline width-xs"
                            v-else v-model="baseData.net_width"
                            :limit="1"></ui-limit-number>
                    <label>cm</label>
                    <label class="ml-xs">高：</label>
                    <span v-if="!editAble"
                          class="inline">{{baseData.net_height}}</span>
                    <ui-limit-number
                            class="inline width-xs"
                            v-else v-model="baseData.net_height"
                            :limit="1"></ui-limit-number>
                    <label>cm</label>
                    <label class="ml-xs">体积重：</label>
                    <span v-if="!editAble"
                          class="inline">{{net_vlume}}</span>
                    <el-input v-else
                              class="inline"
                              v-model="net_vlume"
                ...
                <el-form-item label="包装材料：" required prop="packing_id">
                    ...
                    <!--</el-select>-->
                    <super-select v-model="baseData.packing_id" v-if="editAble"
                                  @change="change_selects(baseData.packing_id,unitData[3].packing,'package')"
                                  :options="unitData[3].packing"
                                  storageKey="packingList"
                                  class="pri-width" ></super-select>
                ...
                <el-form-item label="产品单位：" required prop="unit_id">
                    ...
                    <!--</el-select>-->
                    <super-select v-model="baseData.unit_id" v-if="editAble"
                                  @change="change_selects(baseData.unit_id,unitData[0].unit,'unit')"
                                  :options="unitData[0].unit"
                                  storageKey="unitList"
                                  class="pri-width" ></super-select>
                ...
                <el-form-item label="默认仓库：" required prop="warehouse_id" >
                    <!--<el-select class="pri-width"-->
                        ...
                        <!--<el-option v-for="unit in unitData[6].depot"-->
                                   ...
                                   <!--:value="unit.id"></el-option>-->
                    <!--</el-select>-->
                    <super-select v-model="baseData.warehouse_id" v-if="editAble"
                                  @change="change_selects(baseData.warehouse_id,unitData[6].depot,'warehouse')"
                                  :options="unitData[6].depot"
                                  storageKey="depotList"
                                  class="pri-width" ></super-select>
                ...
                <el-form-item label="是否空买："  required prop="pre_sale">
                    <el-select v-if="editAble"
                               class="pri-width"
                               ref="pre_sale"
                               v-mouse-side.mousewheel="()=>{$refs.pre_sale.visible = false}"
                               v-model="baseData.pre_sale">
                        <el-option v-for="item in pre_sale"
                                   :key="item.value"
                                   :label="item.label"
                                    :value="item.value"></el-option>
                    </el-select>
                    <span v-else>{{baseData.pre_sale==1?'是':'否'}}</span>
                </el-form-item>
...
//    import muiSelect from '../../../components/mui-select.vue';
    ...
    export default{
        data(){
            ...
            return{
                ...
                rules:{
                    ...
                    pre_sale:[ {required:true,message:'请选择是否空卖',trigger:'blur,change',type:"number"}],
                },
            }
        },
        mounted(){
        ...
        computed:{
            ...
            volume(){
                return (this.baseData.depth*this.baseData.width*this.baseData.height/8).toFixed(2);
            },
            net_vlume(){
                return (this.baseData.net_depth*this.baseData.net_width*this.baseData.net_height/8).toFixed(2);
            },
            selectLast(){
                return true;
            },
        ...
        methods:{
            ...
            //             this.brandList = this.unitData[1].brand.slice(1,50);
            //         }else{
            ...
            change_selects(id,selects,attr){
                ...
                if(!!find){
                    this.baseData[attr]=find.value;
                }
            },
            change_plat(item){
                let find=this.unitData[7].plat.find(row=>{
                    return row.value===item.value_id
                });
                if(!!find){
                    item.value=find.label;
                }
            },
            get_developer(){
                this.$http(api_development).then(res=>{
                    this.development=res.map(row=>{
                        return {
                            value:row.id,
                            label:row.realname
                        }
