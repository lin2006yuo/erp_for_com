<template>
    <page class="p-index-ali-publish over-flow-auto"  >
        ...
        <div class="page-fixed">
            ...
            <request-button class="p-wish-btn" :mintime="200" req-key="api_aliexpress_publish_submit" :disabled="disable||is_tort" @click.native="isTort">提交刊登</request-button>
    ...
    export default{
        ...
        mounted(){
            if(window.objlength(this.param)!==0){
                ...
                this.showTemp=!!this.param.show;    // 保存草稿箱按钮 显示或隐藏
        ...
        methods:{
            ...
            tort(){
                ...
                this.is_tort=true;
                for(let i of this.relevanceData.vars){
                    let bool=isExistIllegalWord(i.description, this.tort_arr);
                    let bol=isExistIllegalWord(i.title,this.tort_arr);
                    if(bool||bol){
                        this.tort_value={
                            name:bool?'详情':'标题',
            ...
            goods_description(){
                if(this.param.is_goods_tort===1){   // 刊登先判断是否侵权
                    this.tortShow = true;
                    this.tort_title = `${this.param.spu} 侵权记录`;
                    this.$set(this.tort_info, "id", this.param.id);
                }else{
                    this.publish('api_aliexpress_publish_submit');
                }
            },
            tort_submit(){
                this.tortShow = false;
            ...
            publish(reqKey){
                ...
                if(flag){
                    ...
                    if(this.param.task_id) data.task_id = this.param.task_id;
            ...
            //获得保存数据
            get_data(isVerifyParam,reqKey){     // isVerityParam true:需要验证 false:不验证
                let flag=true;
                let checkData=window.clone(this.relevanceData);
                // console.log("checkData===检查数据===========",checkData)
                if(isVerifyParam) {
                    checkData.vars.forEach((row) => {
                        ...
                        if(row.grossWeight<0.001||row.grossWeight>500){   // 产品包转参数 数值范围验证
                            flag = false;
                            this.$message({type:"error",message:"产品包装后的重量数值范围：0.001~500 "})
                            return flag;
                        }
                        row.attr_info.forEach(item => {
                            if(item.attribute_show_type_value==="input"){   // 验证属性列表不能存在中文标点字符
                                let reg =  /[，。、？！*·；：【】‘”……（）《》{}]/gi;
                                // let str = item.value.match(reg);
                                // if(str.length>0){
                                //     flag = false;
                                //     this.$message({type:"error",message:`${item.names_zh} 属性中存在中文字符 ${ str.join(" ") }`})
                                //     return flag;
                                // }
                                let str = "";
                                let result = reg.exec(item.value);
                                while(!!result){
                                    str = str+ ` ${result}`;
                                    result = reg.exec(item.value);
                                }
                                if(str.length>0){
                                    this.$message({type:"error",message:`${item.names_zh} 属性中存在中文字符 ${str}`})
                                }
                            }
                            if (!!item.required) {
                        ...
                        if(!row.imgs.length){
                            this.$reqKey(reqKey, false);
                            flag = false;
                            return this.$message({type: "warning", message: '刊登图片不能为空！'});
                        }
                        if(row.description === ""){
                            this.$reqKey(reqKey, false);
                            flag = false;
                            return this.$message({type: "warning", message: '内容描述不能为空！'});
                        }
                    });
                }
                ...
                data.vars=clonedata.vars.map((row,row_i)=> {
                    ...
                    row.imgs = row.imgs.map(row => row.path);      // 刊登图片数据
                    // row.market_images = row.market_images.map(val=>{    // 营销图片数据
                    //    return {
                    //        url:val.path,
                    //        image_type:1
                    //    }
            ...
            not_publish(val,account_id){
                this.loading=true;
                this.$http(api_aliexpress_base_info,{goods_id:val,task_id:this.param.task_id}).then(res=>{
                    res.vars=[];
                    res.goods_id=res.id;
                    delete res.id;
                    this.loading=false;
                    ...
                    this.trueId = 0;
                    console.log('this.relevanceData==================',this.relevanceData);
                }).catch(code=>{
                    this.$message({type:"error", message: code.message || code});
                })
            },
            ...
            edit_publish(val,id){
                ...
                let parm={id:val,task_id:this.param.task_id};
                if(id&&id!==""){
                    parm.category_id=id;
                }
                this.loading=true;
                this.$http(api_aliexpress_edit_product,parm).then(res=>{
                    ...
                    clones.categoryArray.forEach(it=>{
                    	it.category_id +="";    // 转换字符串类型
                    });
                    // clones.category_id=clones.category_relation[clones.category_relation.length-1].category_id||"";
                    if(!clones.category_id){
                        clones.category_id=clones.categoryArray[0].category_id||"";
                    }else{
                        clones.category_id = String(clones.category_id)
                    }
                    ...
                    Promise.all([template, ser_template, grounp,sales,brands,relation,custom,same_spu]).then(([temp, serv, gro,sale,brand,rela,customer,same_pro]) => {
                        ...
                        clones.brand = brand;
                        clones.same_spus = same_pro.map(val=>{
                            return {
                                label:val.goods_spu,
                                value:val.id
                            }
                        ...
                        //console.log('this.relevanceData==========编辑=========',this.relevanceData)
                        setTimeout(()=>{
                            this.loading=false;
                        },150)
                    })
        ...
        components:{
            ...
            "tort-record":require("../list/tort-record.vue").default,
