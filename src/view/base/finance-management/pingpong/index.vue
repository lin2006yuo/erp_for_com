<template>
    <page class="p-index">
        ...
        <data-table :table-data="tableData" :loading="loading"
                    ...
                    @submit="init"></data-table>
        <add-moudel v-model="addTable" :add-data="addData" @submit="init" :title="title" :creator-data="creatorData"></add-moudel>
...
<script>
    import {api_get_pingpong,api_get_pingpong_edit,api_user_staffs} from '@/api/pingpong'
    export default{
        data(){
            return{
                loading:false,
        ...
        methods:{
            init(){
                ...
                this.$http(api_get_pingpong,data).then(res=>{
            ...
            add(){
                ...
                this.title = `添加PingPong账户收款信息`;
                this.addTable = true;
            },
            look_edit(row,string){
                this.$http(api_get_pingpong_edit,row.id).then(res=>{
                    this.title = `${string}：${row.account_number} PingPong账户收款`;
                    this.editData = {
                        id:row.id,
