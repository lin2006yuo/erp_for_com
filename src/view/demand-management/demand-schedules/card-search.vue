    ...
    <div class="c-card-search p-package-tab-search">
        <search-card :params="form" :clears="clears" @search="search">
            <div class="mb-mini">
                ...
                <label-item label="需求负责人：" class="ml-sm">
                    <el-select v-model="form.leading_id"
                               filterable clearable
                               class="s-width-small"
                               v-sf.leading_id>
                        <el-option
                            v-for="res in leadingUserList"
                            :label="res.label"
                            :value="res.value"
                            :key="res.value"
                        ></el-option>
                ...
                <label-item label="功能模块：" class="ml-sm">
                    <el-select v-model="form.module"
                               class="s-width-small"
                               v-sf.module>
                        <el-option
                            v-for="res in moduleList"
                            :label="res.label"
                            :value="res.value"
                            :key="res.value"
                        ></el-option>
...
<script>
    import {api_get_progress__module,api_get_progress__principal} from '../../../api/demand-schedules'
    export default {
        data() {
            return {
                leadingUserList:[],
        ...
        methods: {
            ...
            get_principal(){
                this.$http(api_get_progress__principal).then(res=>{
                    res=res.map(row=>{
                        return{
                            label: row.realname,
                            value: row.user_id
                   ...
                   this.leadingUserList=res
                }).catch(code=>{
                    this.$message({type:'error',message:code.message||code});
                })
            },
            get_module(){
                this.$http(api_get_progress__module).then(res=>{
                    res=res.map(row=>{
                        return{
                            label: row.title,
                            value: row.id
