...
<script>
    import {api_user_dete } from '../../../api/role';
    ...
    export default{
        ...
        methods: {
            change_select(val){
                this.memData = this.memData.filter(row=>row.isNewAdd==true);
                val.forEach(async row=>{
                    let find = await this.$http(api_user_dete,{template_id:row});
                    let newList = find.data.filter(val=>!this.memData.find(vals=>vals.username===val.username));
                    this.memData = [...this.memData,...newList];
                })
            },
            peoplelitter(val){
                this.nowLitterValue.push({pag:this.nowLitterValue.length,value:val})
                val.forEach((el,index)=>{
                   let ind=this.memData.findIndex(r=>{
                      return r.username==el.username
                   })
                   if(ind==-1){
                     this.memData.push(val[index])
                   }
               })
            },
            clear_form(){
            ...
            init_params(){
                ...
                data.receive_ids = this.memData.map(row=>row.user_id||row.id);
            ...
            send(){
                ...
                // console.log(this.memData,params)
