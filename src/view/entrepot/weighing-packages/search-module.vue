        ...
        <el-card>
            <el-form ref="form" :model="searchData" label-width="80px">
                ...
                <el-form-item label="尺寸：">
                    <el-checkbox class="" slot="label" v-model="hasSize">尺寸：</el-checkbox>
                    <template v-if="hasSize">
                        <span>长</span>
                        <el-input v-model.number="size.length"
                                  @blur="size_change('length')"
                                  class="inline width-mini"></el-input>
                        <span class="mr-sm">cm</span>
                        <span>宽</span>
                        <el-input v-model.number="size.width"
                                  @blur="size_change('width')"
                                  class="inline width-mini"></el-input>
                        <span class="mr-sm">cm</span>
                        <span>高</span>
                        <el-input v-model.number="size.height"
                                  @blur="size_change('height')"
                                  class="inline width-mini"></el-input>
                        <span class="mr-sm">cm</span>
                    </template>
                </el-form-item>
    ...
    export default {
        ...
        data() {
            return {
                ...
                //如果上次请求还没有完成不进行下次请求
                isRead: false,//输入框只读设置
                hasWeight: true,//必须称重
                isSetWeight: this.value,// 面单输入框是否不可操作
                defaultDir: 2,//默认放向为右
                weighingStyle: 'usb',
                weighingList: [
                ...
                timer: null,
                hasSize: false,//必须填写尺寸
                size: {
                    length: 0,
                    width: 0,
                    height: 0,
                },
                sizeValidate: true,// 尺寸验证标识
            }
        },
        created() {
            let component = this;
        ...
        methods: {
            ...
            init() {
                ...
                let data = clone(this.paramsData);
                // 尺寸必填时验证并添加参数信息
                if(this.hasSize) {
                    Object.assign(data, this.size);
                    if(Object.keys(this.size).some(key => !this.size_validate(key))) {
                        return false;
                    }
                }
            ...
            // 尺寸改变
            size_change(key) {
                this.size_validate(key);
                this.size[key] = parseFloat(this.size[key]).toFixed(1);
            },
            size_validate(key) {
                if((this.size[key] === '' || Number(this.size[key]) <= 0) && this.hasSize) {
                    this.$message({type: 'warning', message: `请填写大于0的正确尺寸数字！`});
                    return false;
                }
                if(Number(this.size[key]) > Number(this.size.length)) {
                    this.$message({type: 'warning', message: `${key === 'width' ? '宽' : '高'}都不能大于长！`});
                    return false;
                }
                return true;
            },
            ...
            shipping_number_enter() {
                ...
                // 过滤面单号
                if (!this.get_params_data()) return;
                //一体秤逻辑
            ...
            submit_weight() {
                try {
                    ...
                    if (!this.paramsData.package_weight
                        ...
                        || Number(this.paramsData.package_weight) > 100000
                    ) {
                        this.$message({type: 'warning', message: '重量错误！'});
            ...
            pollingWeight() {
                ...
                let req = new Request('http://127.0.0.1:10093/setPackageinfo', {
                    method: 'post',
                    mode: 'cors',
            ...
            // 打印机轮询请求
            set_timer() {
                let req = new Request('http://127.0.0.1:10093/getWeighCache', {
                    method: 'get',
                    mode: 'cors',
                    headers: {"Content-Type": 'application/x-www-form-urlencoded'},
                });
                    ...
                    this.timer = setInterval(() => {
                        this.fetch_local(req).then(res => {
                            if (Number(res.status) === 1) {
                                ...
                                this.submit_weight(); // 称重称自带回车，不用这边触发
                            }
                        }).catch(code => {
