            ...
            <search-card class="mb-xs" @search="search" :params="searchData" :clears="clears">
                ...
                <label-item label="日期范围：" class="ml-sm">
                    <el-date-picker class="date inline"
                                    ...
                                    v-sf.date_from
                                    placeholder="开始时间"></el-date-picker><span>&nbsp;--&nbsp;</span>
                    <el-date-picker
                        class="date inline mr-sm"
                        placeholder="结束时间"
                        v-sf.date_to
                        :picker-options="picker_e"
                        v-model="searchData.date_to"></el-date-picker>
                </label-item>
            </search-card>
            <el-row class="mb-xs">
            ...
            data() {
                return {
                    searchData: {
                        ...
                        snType: 'name',
                        snText: '',
                        date_to: new Date(new Date().setDate(0)), //上个月最后一天
                        date_from: new Date(new Date(new Date().setDate(0)).setDate(1)), //上个月第一天
                        page: 1,
                        pageSize: 20
                    },
                    total: 0,
            ...
            methods: {
                ...
                init() {
                    ...
                    this.$http(this.api_init, params).then(res => {
                        ...
                        //缓存数据
                        store.setItem(`InventoryTabelData_${this.swith}`, res)
                    }).catch(error => {
                        this.loading = false
                ...
                //切换表类型
                btn_select(type,btn) {
                    console.log(this.$refs.componentTable.$el.querySelector('.el-table__body-wrapper'))
                    this.swith = btn.value
                    if(!store.getItem(`InventoryTabelData_${this.swith}`)) this.reset()
                    else {
                ...
                get_warehouse(callback) {
                    return this.$http(api_warehous_local).then(res => {
                        const stores = res.map(r => ({
                            label: r.name,
                            value: r.id
                        }))
                        callback(stores)
