<template>
    <print-dialog :title="`集结完成(${printData.title || printData.code})`"
        ...
        <div style="position: relative;margin:0 auto;overflow:hidden"
            ...
            <div style="line-height: 1.2em; font-size: 18px; font-weight: bold; margin-left:5px;">
                提货方式：{{printData.collector_type}}
            </div>
        ...
        methods: {
            async query() {
                ...
                try {
                    ...
                    if(this.funmart_ids.includes(this.printData.shipping_method_id)) {
                        this.funmart_print(res);
                    } else {
                        this.prints();
                    }
                } catch (code) {
