    ...
    <el-row class="p-ali-order-table">
        <div class="mt-sm mb-sm">
            ...
            <permission class="inline"
                        ...
                        req-key="url_post_aliexpress_order_sysc"
                        :route="apis.url_post_aliexpress_order_sysc"
                        @click="batch_orders_sysc">更新最迟发货时间
            </permission>
        </div>
        <!--数据table-->
        ...
        methods:{
            ...
            batch_orders_sysc(){
                if (this.selecteList.length <= 0) {
                    this.$message({
                        type: 'warning',
                        message: '请先勾选需要批量更新的数据！'
                    });
                    this.$reqKey('url_post_aliexpress_order_sysc', false);
                    return;
                }
                let data ={
                    data:this.selecteList.map(row => {
                        return {
                            account: row.account_id,
                            order_id: row.order_id
                        }
                    }),
                    update_uploaded_deadline:1
                };
                this.$http(api_post_aliexpress_order_sysc, data).then(res => {
                    this.$message({type: 'success', message: res.message || res});
                    this.$emit('sysc-success');
                }).catch(code => {
                    this.$message({type: 'error', message: code.message || code});
