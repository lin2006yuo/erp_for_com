<template>
    <ui-form :model="form" :rules="rules" ref="collectorInfo" label-width="120px">
        ...
        <ui-form-item label="提货方式：" prop="collector_type" localtion="right" :bubble="true"
                      class="inline">
            <el-select v-model="form.type" :disabled="isEdit" class="width-sm">
                <el-option v-for="item in collectorTypeList"
                           :label="item.label"
                           :value="item.value"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            init_collector_type() {
                this.$http(api_collector_type).then(res => {
                    this.collectorTypeList = res;
                }).catch(code => {
                    this.$message({type: 'error', message: code.message || code});
                })
            },
