<template>
    <div class="c-treess">
        <ul class="ul">
            <li  v-for="cate in  treeData"
                 class="li"
                 :class="cate.show?'li-show':''"
                 @click="click_cate(treeData,cate)">
                <label >{{cate.category_name_zh||cate.category_name}}</label>
                <i v-if="cate.category_isleaf===0" class="iconRight el-icon-d-arrow-right"></i>
            </li>
        </ul>
        <trees ref="details"
               :treeData="single.child"
               v-if="single.child&&single.child.length>0"
               class="class-tree"
               :accountId="accountId"
               @last="last"
    ...
    export default{
        ...
        methods:{
            ...
            click_cate_all(child,req,i){
                ...
                this.$http(api_aliexpress_classify,{category_id:req[i].category_id,account_id:this.accountId}).then(res=>{
            ...
            click_cate(parent,child){
                ...
                if(child.category_isleaf==0){
                    this.$emit("change-btn",true);  // 选中值不为末尾项，即存在下级分类树
                    this.$http(api_aliexpress_classify,{category_id:child.category_id,account_id:this.accountId}).then(res=>{
                        this.$set(child,"child",res);
                    }).catch(code=>{
                        console.log(code)
                    })
                }else {
                    this.$emit("change-btn",false); // 选中值为末尾项，即不存在下级分类树
