<template>
    <page  class="over-flow-auto" v-loading="loading">
        ...
        <div class="page-fixed">
            ...
            <el-button type="primary" size="mini" class="inline" @click.native="submit_publish" :disabled="is_tort" v-if="param.curPage==='not'">提交刊登</el-button>
    ...
    export default{
        ...
        methods:{
            ...
            * */
             ...
             */
            add_init_info(){
                return this.$http(api_shopee_getdata,{goods_id:this.param.goods_id,account_id:this.param.account_id}).then(res => {
                    if(res.vars.length<=0)return this.$message({type:'warning',message:'当前spu无vars数据，请更换spu'});
                    this.form.name = res.name;
                    this.form.spu = res.spu;
                    this.form.sales_status = res.sales_status;
                    this.form.transport_property = res.transport_property;
                    ...
                    this.form.tort_flag=res.tort_flag;
                    ...
                    res.vars.forEach(val=>{
                        switch (res.site_id){
                            ...
                            case 5:
                            case 6:
                                val.weight = (val.weight*1000+0.02*1000)/1000;
                                val.unit='kg';
                                break;
                            case 7:
                                val.weight=((val.weight*1000+0.02*1000)/1000)*1000;
                                val.unit='g';
                            default:
                                break;
                        }
            ...
            add_data(val){
                val.forEach(row=>{
                    ...
                    console.log(this.form.vars,'form.vars.push')
                });
                this.$nextTick(()=>{
                    let length = this.form.vars.length;
                    if(length<=0)return;
                    ...
                    if(templateData){
                        this.form.vars.forEach(row=>{
                            if(row.account_code!==this.curAccount){
                                ...
                                if(row.site_id!=templateData.site_id){
                                    row.variant=variant;
                                }
                            }
                        })
            ...
            fanseRight(){//侵权记录继续
                ...
                //点击继续提交刊登
                this.$http(api_shopee_add,this.tort_flagFor).then(res => {
            ...
            goods_description(){
                this.tort_flagFor = this.arrange_data();
                if(!!this.form.tort_flag){
                    if(!this.tableData.length){
                        this.$http(api_get_goods_description, this.param.goods_id, {page: 1, page_size: 50})
                            .then(res => {
                    ...
                    if(row.variant.length>0){
                        row.variant.forEach(res=>{
                            if(res.name.length>20){
                                canSubmit = false;
                                return this.$message({type:'warning',message:'商品规格属性名不能超过20个字符，请修改！'})
                            }
                        })
                    }
                });
            ...
            * */
            ...
            arrange_data(){
                ...
                cloneForm.vars.forEach(row=>{
                    ...
                    if(!!row.curPage&&row.curPage==='not'&&row.radioData===1){
