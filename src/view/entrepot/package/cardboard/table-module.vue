...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            batch_print() {//批量打印
                let ids = this.selectedData.filter(row => row.status >= 2).map(row => row.id);
            ...
            batch_delete() {//批量删除
                let ids = this.selectedData.map(row => row.id);
            ...
            batch_finish() {//批量打板完成
                let ids = this.selectedData.filter(row => row.status === 1).map(row => row.id);
