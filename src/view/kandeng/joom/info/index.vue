    ...
    <page>
        <page class="over-flow-auto">
            ...
            <div class="page-fixed">
                ...
                <el-button size="mini" type="primary" @click.native="carry_over" :disabled="isDis||is_tort">刊登到平台</el-button>
                <el-button size="mini" @click="cancel">取消</el-button>
            </div>
        </page>
        <tort-table :title="title" :isloading.sync="isloading" v-model="show" :tableData="trot_data" @is_ok="public_channel" @handleCurrentChange="trot_handleCurrentChange" :count="count">
            <label>该SPU存在侵权记录，是否继续？</label>
            <el-button type="primary" @click="public_channel" size="mini">继 续</el-button>
...
<script>
    ...
    import {api_get_goods_description,api_infringement} from '../../../../api/product-library'
    ...
    export default{
        ...
        methods: {
            trot_handleCurrentChange(val){
                this.isloading=true
                this.$http(api_get_goods_description,this.publish.goods_id,{page:val,page_size:20}).then(res=>{
                ...
                if(this.param.isAdd){// 如果是未刊登 -》 刊登
                    if(this.param.shop_id !== '' && this.param.account_id !== ''){
                        let find = this.accounts.find(account=>{
                            return account.id === Number(this.param.account_id)
                        });
...
//            表单验证
            variant_verification(){
                ...
                for(let i = 0; i< vars.length; i++){
                    ...
                    for(let i=0; i<variant.length; i++){
                        ...
                        if(Number(row.declared_value).toString()==='NaN'||this.to_str(row.declared_value) === '0' || this.to_str(row.declared_value).length === 0){
                            bool = false;
                            console.log(row.declared_value);
                            this.$message({
                                type:'info',
                                message:item.tab_label+"报关价必填且不能为0"
                            });
                            return false
                        }
                        // if(Number(row.declared_value)<=0){
                        //     bool = false;
                        //     this.$message({
                        //         type:'info',
                        //         message:item.tab_label+"报关价不能小于等于0"
                        //     });
                        //     return false
                        // }
            ...
            // stort_confirm(){
            //     this.isloading=true
            //     this.$http(api_get_goods_description,this.publish.goods_id,{page:1,page_size:20}).then(res=>{
            ...
            tort(){
                ...
                this.is_tort=true;
                for(let i of this.publish.publish_vars){
                    let bool=isExistIllegalWord(i.description, this.tort_arr);
                    let bol=isExistIllegalWord(i.name,this.tort_arr);
                    if(bool||bol){
                        this.is_tort=false;
                        this.is_tort=false;
                        this.tort_value={
                            name:bool?'详情':'标题',
                            account_code:i.account_name,
                            bool,
                            bol,
                        };
...
//            刊登到平台
            carry_over(){
                if(!this.variant_verification()){//this.variant_verification()
                    return
                }else if(this.tort()){
                    return
                } else{
