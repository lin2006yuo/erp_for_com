    ...
    <page class="p-index">
        ...
        <label-item label="请选择第三方仓库：" class="mb-xs mt-sm">
            <super-select v-model="warehouse_id"
                          ...
                          storage-key="placeorderThirdWarehouse"
                          class="inline s-width-default"
                          @change="change_warehouse"></super-select>
        ...
        <data-table ref="dataTable"
                    ...
                    :order-vova-btn="orderVovaBtn"
                    :upload-vova-btn="uploadVovaBtn"
                    ...
                    @order-vova="order_vova"
                    @upload-vova="upload_vova"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            upload_vova() {
                if(this.selectedItems.length <= 0) {
                    this.$reqKey('uploadVova', false);
                    this.$message({type: 'error', message: '请选择要批量下单的的数据'});
                    return
                }
                this.$reqKey('uploadVova', true);
                const ids = this.selectedItems.map(row => row.id).join(',');
                this.$http(api_placeorder_vova, {ids}).then(res => {
                    if(res.message === '上传成功') {
            ...
            order_vova() {
                if(this.selectedItems.length <= 0) {
                    this.$reqKey('orderVova', false);
                    this.$message({type: 'error', message: '请选择要获取跟踪号的数据'});
                    return
                }
                this.$reqKey('orderVova', true);
                const ids = this.selectedItems.map(row => row.id).join(',');
                this.$http(api_placeorder_vova_track, {ids}).then(res => {
                    if(res.message !== '不是vova平台不能获取跟踪号') {
                        this.$message({type: 'success', message: res.message});
                        this.$reqKey('orderVova', false)
                    }
                }).catch(error => {
        ...
        computed: {
            ...
            orderVovaBtn() {
                return this.uploadStatus === 0 && this.form.channel_id === 12
            },
            uploadVovaBtn() {
                return this.uploadStatus === 0 && this.form.channel_id === 12
            },
