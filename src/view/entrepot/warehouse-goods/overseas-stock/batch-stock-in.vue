<template>
    <page-dialog v-model="visible" size="large" @open="open" @close="close" title="提交借调申请">
        ...
        <el-table :data="tableData"
            ...
            <el-table-column label="用途类型" inline-template>
                <div>
                    <el-select v-model="row.type" class="s-width-default">
        ...
        methods: {
            ...
            validate() {
                ...
                if (this.to_channel_id === '') {
                    this.$message({type: 'warning', message: '请选择调入平台'});
                    return false;
                }
                if (this.reason === '') {
                    this.$message({type: 'warning', message: '请填写借调原因'});
                    return false;
                }
                if(this.tableData.length === 0) {
                    this.$message({type: 'warning', message: '请添加需要借调的商品'});
                    return false;
                }
                ...
                return !this.tableData.some((row,index) => {
                    ...
                    if(isNaN(row.quantity) || row.quantity < 1 || row.quantity > Number(row.available_quantity) || row.audit_id === '') {
                        if(row.audit_id === '') {
                            this.$message({type: 'warning', message:`第${index+1}行没有选择审核人!`});
                        } else {
                            this.$message({type: 'warning', message: `第${index+1}行调拨数量为无效数据!`});
                        }
                        return true;
                    }
                    if(row.type === '') {
                        this.$message({type: 'warning', message: `第${index+1}行没有选择用途类型！`});
                        return true;
                    }
                    return false;
                });
