...
<script>
    ...
    import {
        ...
        api_ali_address, logistics_enabled,api_logistics_fee,api_account_list
    ...
    export default {
        ...
        methods: {
            ...
            get_information: function (id) {
                ...
                this.$http(api_logistics_fee,id).then(res=>{
            ...
            //    添加站点信息
                this.channelData.channels.forEach(item=>{
                    this.$http(api_account_list,{channel_id:item.channel_id}).then(res=>{
                        res.site.forEach(row=>{
                            row.enabled=false;
                            row.min="";
                            row.max="";
                            row.currency="";
                            row.site_code=row.value;
                        })
                        if (res.site.length>0){
                            item.site_list = [...res.site]
                        }else {
                            item.site_list = []
                        }
                    })
