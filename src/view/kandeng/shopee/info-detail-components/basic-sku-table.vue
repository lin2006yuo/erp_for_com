    ...
    <div class="c-basic-sku-table">
        <table class="template" v-if="form.variant&&form.variant.length>0">
            ...
            <tr v-for="(row,index) in form.variant">
                ...
                <td>
                    <el-input v-model="row.name" @blur="blur_name(row)"></el-input>
                </td>
...
<script>
    ...
    export default {
        ...
        methods:{
            blur_name(row){
                if(row.name.length>20) return this.$message({type:'warning',message:'商品规格属性名不能超过20个字符，请修改！'})
            },
            blur_change(name,val){
                if(!val)return;
                if(val.length>20) return this.$message({type:'warning',message:'商品规格属性名不能超过20个字符，请修改！'})
