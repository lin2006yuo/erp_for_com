<template>
    <div class="aliexpress-wishPublish-list">
        <search-card @search="search" :params="listParams" :clears="clears" :is-search="isSearch">
            ...
            <div class="inline width-sm" >
                <super-select v-sf.account_id
                              v-model="listParams.account_id"
                              @change="change_account"
                              storageKey="aliexpress-account-id"
                              :adjustStructure="(data)=>{return{data:data.map(val=>({label:val.label,value:val.id})),count:data.length}}"
                              :remoteRequestHttp="httpAccountId">
                </super-select>
            </div>
            ...
            <div class="inline width-sm" >
                <super-select v-model="listParams.developer_id"
                              storageKey="aliexpress-developer-id"
        ...
        <ui-table
            ...
            <tr v-for="(row, index) in products" :key="row.goods_id">
                ...
                <td style="position:relative">
                    <div class="icon-wrap">
                        <i v-if="row.is_goods_tort===1" class="pointer tort-icon" @click="showTort(row)">侵权</i>
                        <i v-if="row.is_goods_discount===1" class="pointer tort-icon fall-in-price" @click="showFallInPrice(row)">跌价</i>
                    </div>
        ...
        <fall-in-price v-model="fallInPriceShow" :title="fallInPriceTitle" :fallInPriceInfo="fallInPriceInfo"></fall-in-price>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods: {
            showFallInPrice(row){
                this.fallInPriceShow = true;
                this.fallInPriceTitle = `${row.spu}补贴信息`;
                this.$set(this.fallInPriceInfo,"id",row.goods_id);
            },
            showTort(row){
                this.tortShow = true;
                this.tort_title = `${row.spu} 侵权记录`;
                this.$set(this.tort_info,"id",row.id)
            },
            httpAccountId(){
                return http('get|aliexpress-accounts')
            },
            ...
            get_developer() {
                this.$http(api_get_user_development_staffs).then(res => {
                    res.forEach(val=>{
                        this.$set(val,"label",val.realname);
            ...
            publish(row) {
                let params = {
                    ...
                    isCopy: false,  //  复制
                    isTime: true,   //  允许定时
                    show: true,     // 报存草稿箱按钮 显示或隐藏
                    default_account_id: this.listParams.account_id,
                    is_goods_tort:row.is_goods_tort,
                    spu:row.spu
                };//---查看标示id为1
        ...
        components: {
            ...
            "fall-in-price":require("./fall-in-price.vue").default,
        }
    }
</script>
