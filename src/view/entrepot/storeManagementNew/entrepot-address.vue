                ...
                <ui-form :model="form" :rules="rules" ref="form" label-width="120px" localtion="right">
                    ...
                    <el-row>
                        ...
                        <ui-form-item label="仓库：" class="inline">
                            <super-select v-model="form.entrepot"
                                          v-if="form.type !== 5"
                                          :disabled="isDisabled"
                                          @change="entrepot_change"
                                          storageKey="entrepotManageWarehouseList"
                                          :options="thirdWarehouseList"></super-select>
                            <!--<select-remote v-model="form.entrepot"
                                           ...
                                           style="width: 153px;"
                            ></select-remote>-->
                            <scroll-select v-model="warehouse"
                                           v-else
                                           style="width:153px" class="inline"
                                           textAlign="left"
                                           :size="20"
                                           remote="get|warehouse/getWarehousesByType"
                                           :fix-params="fix_params_warehouse"
                                           :fixResult="fix_result_warehouse">
                            </scroll-select>
                        </ui-form-item>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 仓库类型改变请求仓库列表
            warehouse_type_change() {
                let type = this.form.type;
                if(type === 5){
                    return false
                }
                this.$http(store_type_two, type).then(res => {
                    this.thirdWarehouseList = res.map(row => {
            ...
            // fba仓库参数
            fix_params_warehouse({page, pageSize, keyword}) {
                return {
                    page: page,
                    pageSize: pageSize,
                    entrepot_name: keyword || '',
                    type: this.form.type,
                };
            },
            fix_result_warehouse(res) {
                return {
        ...
        computed: {
            ...
            warehouse: {
                get() {
                    return {
                        value: this.form.entrepot,
                        label: this.form.entrepot_name,
                    };
                },
                set(val) {
                    this.form.entrepot = val.value;
                    this.form.entrepot_name = val.label;
                }
            },
            pcc: {
                get() {
