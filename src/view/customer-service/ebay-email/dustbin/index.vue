                    ...
                    <div v-loading="loading" element-loading-text="玩命加载中...">
                        ...
                        <div class="collapse" v-resize="{height:76}" v-if="emailList.length>0">
                            <ui-collapse v-for="(item,item_i) in emailList"
                                ...
                                <span slot="title" style="line-height:26px;  cursor: pointer;" :class="`${item.is_read===0?'bold-font':''}`">
                                    <el-row class="inline">
                ...
                <el-card class="rightCard ebay-right-card" v-resize="{height:34}">
                    <div>
                            <div v-if="detail.id" style="height: 100%;display: flex;flex-direction: column">
                                ...
                                <div class="max-heigh_single filter-body" v-resize="{height:23}">
                                    <iframe style="width:100%;min-height: 97%;overflow-x: hidden" scrolling="yes" :srcdoc="filterBody(detail.body)"
    ...
    export default {
        ...
        methods: {
            ...
            get_unread(val){
                ...
                if (val){
                    data.start_date= Number(val.start_date)>0?datef('YYYY-MM-dd', val.start_date/1000):'';
                    data.end_date= Number(val.end_date)>0?datef('YYYY-MM-dd', val.end_date/1000):'';
                }
            ...
            init_params(){
                ...
                data.start_date =data.start_date > 0 ? datef('YYYY-MM-dd', data.start_date/1000) : "";
                data.end_date =data.end_date > 0 ? datef('YYYY-MM-dd', data.end_date/1000) : "";
