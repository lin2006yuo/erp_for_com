...
<script>
    ...
    export default{
        ...
        methods:{
            ...
            getModelList(){
                this.$http(api_common_modeList,{type:"cate"}).then(res=>{
            ...
            edit(val){
                if(!!val.queue_count) return this.$message({type:"warning",message:'该定时规则正在执行中，不予修改！'})
            ...
            del(data,arr){
                if(!!arr.queue_count) return this.$message({type:"warning",message:'该定时规则正在执行中，不予删除！'});
