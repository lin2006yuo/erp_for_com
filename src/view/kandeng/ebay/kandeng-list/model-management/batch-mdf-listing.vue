        ...
        <page-dialog v-model="dialog" @open="open"
            ...
            <div slot="footer">
                <request-button class="inline" req-key="batchMdfListing" :mintime="300"
                                @click="submit" :disabled="loading">{{$t('common.confirm')}}</request-button>
                <el-button size="mini" class="inline" @click.native="dialog = false" :disabled="loading">{{$t('common.cancel')}}</el-button>
            </div>
        </page-dialog>
    </div>
</template>
