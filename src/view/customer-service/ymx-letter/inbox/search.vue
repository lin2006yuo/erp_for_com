<template>
    <div class="p-search-amazon-inbox">
        <search-card :params="searchData" :clears="clears" @search="search" @enter="search">
            <el-row>
                <label-buttons label="邮件分类：" :buttons="handles" @select="select_handles" class="inline mr-sm"></label-buttons>
                <label-buttons label="回复状态：" :buttons="priority" :default-index="1" @select="select_priority"
                               class="inline  mr-sm"></label-buttons>
                <label-buttons label="需优先回复邮件：" :buttons="boxs" @select="select_boxs" class="inline  mr-sm"></label-buttons>
                <label-buttons label="读取状态：" :buttons="reads" @select="select_read" class="inline"></label-buttons>
            </el-row>
            <el-row>
                <select-more-button
                    label="账号："
            ...
            <div class="inline">
                <el-select v-model="searchData.option_field" placeholder="请选择" class="inline s-width-default"
                           v-sf.option_field>
                    <el-option
                        :key="item.value"
                ...
                <el-input v-model="searchData.option_value" :placeholder="`请输入${changeLabel}...`"
                          class="inline s-width-large" v-sf.option_value></el-input>
                <label class="ml-sm inline">客服账号：</label>
                <!-- <el-select v-model="searchData.customer_id" placeholder="请选择" class="inline s-width-middle" filterable
                           v-sf.customer_id>
                    <el-option
                ...
                <super-select 
                    ...
                    v-sf.account_id
                    storageKey="api_customer_ymx_letter_inbox"
                    :placeholder="all"
                    :options="options"
                    >
                </super-select>
                <label class="ml-sm inline">标签类型：</label>
                <el-select v-model="searchData.flag_id" placeholder="请选择" class="inline s-width-middle" clearable
                           filterable v-sf.flag_id>
                    <el-option
                        :key="item.value"
                ...
                <el-select v-model="searchData.site" placeholder="请选择" class="inline  mr-sm s-width-middle" clearable
                ...
                <el-select v-model="searchData.time_field" placeholder="请选择" class="inline s-width-default"
                           v-sf.time_field>
                    <el-option
                        :key="item.value"
                ...
                <el-date-picker type="date" placeholder="开始时间" v-model="searchData.time_start"
        ...
        methods: {
            get_site() {
                this.$http(api_amazon_emails_site).then(res => {
                    this.siteList = [{label: '全部', value: ''}, ...res.map(site => ({
                        label: site.label,
                        value: site.label
                    }))];
                }).catch(code => {
