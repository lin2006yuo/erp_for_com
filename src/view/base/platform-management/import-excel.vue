<template>
    <page-dialog class="p-import-excel" title="excel导入" v-model="visible" @open="open" size="large" :close-on-click-modal="false">
        <div>
            <div class="file-btn">
                请选择要导入的Excel文件
                <input type="file" class="input-font-size-platfrom" ref="file" multiple @change="file_change">
            </div>
    ...
    export default {
        ...
        methods: {
            ...
            submit(file, data) {
                ...
                this.apiFunction(item).then((res)=> {
                    if (res.status === 200) {
                        this.fullscreenLoading = false;
                        if(res.data.message.length>0||res.data.error.length>0){
                            this.files[index].result = "导入成功";
                                // res.data.message;
                            this.visible=false;
                            /** 新增了发送事件的方法参数。*/
                            this.$emit('files-update', res.data);
                        }else{
            ...
            download_file(){
                let url = config.apiHost + "downfile?code=channel_account&test";
