    ...
    <div>
        ...
        <div class="p-head-border-color">
            <el-form :label-width="labelWidth" :model="form" :rules="rules_1" ref="base_info_1">
                <el-form-item :label="`${$t('ymx-detail.tempRef')}：`" v-if="curPage==='not'&&!showSite">
                ...
                <el-form-item :label="`${$t('ymx-detail.classifiTemp')}：`" required
                    ...
                    <super-select
                        v-model="form.category_template_id"
                        class="inline width-base"
                        @select-option="first_change"
                        :options="form.categoryTemplatList"
                        :adjustStructure="category_template_result"
                        :placeholder="$t('ymx-detail.select')"
                        :storageKey='`${form.code}account_tab_base_category_template`'
                    ></super-select>
                ...
                <template v-for="(item,index) in form.basic">
                        <el-form-item v-if="is_show(item)"
                            ...
                            <super-select
                                ...
                                v-if="item.key==='warehouse_id'"
                                class="inline width-base"
                                :options="amazonWarehouseList"
                                :placeholder="'请选择'"
                                storageKey='account_tab_base_warehouse'
                            ></super-select>
                        </el-form-item>
                        <template v-if="item.select === 0">
                            ...
                            <el-form-item v-if="item.key==='IsVirtualSend'"  :label="`${get_name(item.name)}：`">
                                <el-radio v-model="item.value" :label="item.option[1]" class="inline" >是</el-radio>
                                <el-radio v-model="item.value" :label="item.option[0]" class="inline" >否</el-radio>
                            </el-form-item>
                            <template v-else>
                                <el-form-item :label="`${get_name(item.name)}：`"
                                              v-if="item.key!=='Timer'&&item.key!=='Spu'&&item.key!=='IsVirtualSend'&&item.key!=='Department'"
                                              :prop="`basic[${index}].value`"
                                              :rules="{required: item.require === 1, message:$t('ymx-detail.required'), trigger:'blur'}">
                                    <el-input v-model="item.value"
                                              class="inline width-base"></el-input>
    ...
    export default{
        ...
        methods: {
            ...
            get_name(value){
                switch (value) {
                    ...
                    case "平台父SKU":
                        ...
                        break;
                    case "是否虚拟仓发货":
                        return this.$t('ymx-detail.virtualWarehouseDelivery');
                        break;
                    default:
                        return value;
                }
            ...
            async reload_category_templat(){
                ...
                let config = {key:this.form.siteId,data:res};
            ...
            async reload_product_templat(){
                ...
                await this.get_product_attribute(productType, this.form.siteId).then(id => {
                    /*初始化产品默认模板属性
                    * */
                    if (!id||id instanceof Boolean) return this.$message({type: "warning", message: `【${row.code}】${this.$t('ymx-detail.account')}，暂无默认的产品模板，请添加！`});
                    this.$set(this.form,'product_template_id',id);
                    this.get_edit_data(productType, this.form.siteId, id).then(res => {
                        if (res.product&&res.product.attrs.length>0) {
                            ...
                            this.$emit('reload-product-templat',this.form);
                            setTimeout(()=>{
            ...
            quote_model(){
                if(!this.form.doc_id)return this.$message({type:'warning',message:'暂无可引用的范本数据，请先到【范本管理】页面，为当前SPU添加范本！'});
            ...
            * */
            get_category_list(){
                ...
                this.get_product_attribute(type,this.form.siteId).then(res=>{
                    if(!res||res instanceof Boolean)return;
            ...
            * */
            get_attr_product(){
                ...
                this.get_product_attribute(productType,this.form.siteId).then(id=>{
                    /*初始化产品默认模板属性
                    * */
                    if(!id||id instanceof Boolean)return this.$message({type:"warning",message: this.$t('ymx-detail.setDefault')});
        ...
        computed: {
            ...
            amazonWarehouseList(){
                    return this.form.amazonWarehouseList.map(row=>{
                        return {
                            value:row.id,
                            label:row.name
                        }
