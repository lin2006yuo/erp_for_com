            ...
            <div class="tabs-content-pane card">
                <el-tabs type="card" :active-name="action" ref="tabs" @tab-click="tab_click">
                    ...
                    <el-tab-pane key="Infringement-record"
                                 ...
                                 label="侵权记录"
                                 class="roll-height">
                        <Infringement-record :spu="spu" @edit="edit" :pageSize="this.pageSize"  :currentPage="this.currentPage" :goodsId="this.goodsId" :edit-able="show[9]" ></Infringement-record>
                    </el-tab-pane>
                </el-tabs>
            </div>
        </div>
...
//                    })
            ...
            * */
            ...
            description_init(){
                ...
                this.$http(publish_description,this.goodsId).then(res=>{
                    this.descData=res.map(row=>{
                        row.newTags="";
                        this.$set(row,'description2',this.transform_br(row.description));
                        return row
                    });
            ...
            transform_br(str){
                let reg = /\r/ig;
                let content = str.replace(reg,'<br>');
                return content;
            },
            //上传产品描述
            desc_updata(data){
            ...
            save_qc(){
              this.$http(publish_updata_quality,this.goodsId,{qc:this.qualityData[0]}).then(res=>{
                  this.$message({type:"success",message:res.message})
              })  .then(e=>{
                  this.$message({type:"error",message:e||e.message})
              });
            },
            //图片
            img_init(){
            ...
            unit_init(){
                this.$http(publish_edit_unit).then(res=>{
                    this.unitData[0].unit=res.map(row=>{
                        return {
                            label:row.name,
                            value:row.id
                        }
                ...
                this.$http(publish_edit_packing).then(res=>{
                    this.unitData[3].packing=res.map(row=>{
                        return {
                            label:row.name,
                            value:row.id
                        }
                ...
                this.$http(publish_edit_brand).then(res=>{
                    this.unitData[1].brand=res.map(row=>{
                        return {
                            label:row.name,
                            value:row.id
                        }
                ...
                this.$http(publish_edit_depot).then(res=>{
                    this.unitData[6].depot=res.map(row=>{
                        return {
                            label:row.name,
                            value:row.id
                        }
