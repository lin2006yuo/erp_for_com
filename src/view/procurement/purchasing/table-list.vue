    ...
    <el-row class="p-purchasing">
        ...
        <ui-table :has-data="dataTable.length>0"
                  ...
                  :heads="[
                        ...
                        {label:'sku状态',width:4},
                        {label:'仓库',width:4},
                        {label:'安全交期',width:6,isSort:true,type:'desc'},
                        {label:'可用库存',width:6,isSort:true,type:'desc'},
                        {label:'活动备货数',width:6,isSort:true,type:'desc'},
                        {label:'在途库存',width:6,isSort:true,type:'desc'},
                        {label:'日均销量',width:6,isSort:true,type:'desc'},
                        {label:'订单未配货',width:5},
                        {label:'缺货数量',width:6,isSort:true,type:'desc'},
                        {label:'建议采购',width:6,isSort:true,type:'desc'},
                        {label:'最小起订量',width:4},
            ...
            <template v-if="data.dataList.length>0" v-for="(data, index) in dataTable">
                ...
                <tr>
                    ...
                    <td :colspan="18" class="td-relative">
                ...
                this.dataTable.forEach(data=>{
                    data.dataList.forEach(item=>{
                        if(item.isCheck&&!item.isDiable){
                            ...
                            datas1.push(dataTable1);
                            let arrSku = datas1.filter(row => {
                                return Number(row.purchase_price) ===0
                            }).map(k => k.sku);
                            Object.assign(skuList, arrSku);
                ...
                if(listId.length<=0){
                    ...
                    this.$http(api_updata_before_create_plan,{data_json:datas}).then(res=>{
                        ...
                        if(!flag){
                            ...
                            this.$confirm(`${str} 采购单价为0, 是否继续采购 ?`, '提示', {
