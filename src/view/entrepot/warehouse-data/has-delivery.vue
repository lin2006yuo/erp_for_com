    ...
    export default {
        ...
        methods: {
            batch_report() {
                ...
                return this.$http(api_get_shipped_export, params, header).then(res => {
                    if(res.join_queue === 1) {
                        this.visible = true;
                        this.$message({
                            type: 'success',
                        ...
                        downloadFile({
                            ...
                            suffix: '.xls',
                            success: () => { this.$message({type: "success", message: '导出成功' || res}); return Promise.resolve() },
                            error: () => { this.$message({type: "error", message: '导出失败' || res}); return Promise.reject() }
                        });
                    }
                }).catch(err => {
