<template>
    <page class="p-index-member" ref="page">
        <search :searchData="searchData" @search="search" :channel="channel"></search>
        ...
        <ui-table :has-data="tableData.length>0"
            ...
            <template v-for="(item, i) in tableData">
                <tr v-for="(trs ,index) in item.info" @click="row_high_light(i)" :class="{active:item.isHighLight}">
                    ...
                    <td :rowspan=" item.info.length" v-if="index===0">
                        <div>
                            <permission tag="span" :route="apis.url_member_edit" class="operate"
                                        @click="edit(item, index)">编辑
                            </permission>
                            |&nbsp;<permission tag="span" :route="apis.url_member_del" class="operate"
                                               @click="del_one(item)">删除
                        </permission>
                            |&nbsp;<permission tag="span" :route="apis.url_member_ship_log" class="operate"
                                               @click="show_log(item,index)">日志
                        </permission>
                            <permission tag="span" :route="apis.url_channel_user_account" @click="user_account(item)">|&nbsp;<span
                                class="operate">成员管理</span></permission>
                        </div>
                    </td>
                </tr>
            </template>
        ...
        <user-table v-model="memDialog"
                    ...
                    :seller-id="sellerId"></user-table>

        <export-dialog v-model="dialogVisible"></export-dialog>
    ...
    export default {
        ...
        methods: {
            ...
            getSales() {
                ...
                this.$http(api_sales_list, params).then(res => {
            ...
            add() {
                ...
                this.title = "添加";
                this.devData = [];
                this.editLoading = false;  // 对话框loading条。
                this.memVisable = true;
            },
            edit(row, index) {
                this.editIndex = index;
                let res = clone(row);
                delete res.channel_name;
                delete res.account_name;
                delete res.customer_name;
                delete res.create_time;
                delete res.isCheck;
                delete res.isHighLight;
                ...
                this.listEdit = false;
                obj.type = `channel${res.channel_id}`;
                this.editLoading = true;
                this.$http(api_platform_account, {channel_id: res.channel_id}).then(ress => {
                    // this.load({type: `channel${res.channel_id}`, channels: ress.account});   // 数据在时时变化。不能放入vuex。
                    obj.param = ress.data.map(item => {
                        return {label: item.code, value: item.id};
                    });
                    obj.param.unshift({
                        label: res.code, value: res.account_id
                    });
                    this.editLoading = false;
                    let newObj = {...obj, ...res};
                    // 确定是不是joom平台。
                    this.sureChannel(newObj);
                    this.devData = [newObj];
                    // 如果平台的数据是joom平台的话。
                    this.memVisable = true;
                    this.batchEdit = false;
            ...
            /** 确定是不是joom平台，如果是joom平台的话，仓库选择只有全部。*/
            sureChannel(obj) {
                if (obj.channel_id === 7) {
                    obj.isJoom = true;
                    obj.warehouse = [{isCheck: false, label: '全部', value: 0}];
                }
            },
            del_all() {
                let data = [];
            ...
            export_all() {
                ...
                if (data.account_code) {
                    data.account_id = data.account_code.split(" ");
                }
            ...
            async getChannelAccounts(channel_id) {
                if (!this.channelsAccounts) {
                    this.channelsAccounts = {};
                }
                if (this.channelsAccounts[channel_id]) {
                    return this.channelsAccounts[channel_id];
                }
                return await this.$http(api_platform_account, {channel_id: channel_id}).then(ress => {
                    let list = ress.data.map(item => {
                        return {label: item.code, value: item.id};
                    });
                    this.channelsAccounts[channel_id] = list;
            ...
            async batch_edit() {
                ...
                this.batchEdit = true;
                this.editLoading = true;
                this.devData = await Promise.all(arr.map(async row => {
                    let param = await this.getChannelAccounts(row.channel_id);
                    param.unshift({label: row.code, value: row.account_id});
                    ...
                    return {
                        ...
                        type: row.type,
                        param: param,
                        warehouse: row.channel_id === 7 ? [{isCheck: false, label: '全部', value: 0}] : row.warehouse
                    }
                }));
                this.editLoading = false;
            },
