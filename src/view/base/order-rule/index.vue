<template>
    <page>
        <search-card @search="search" @enter="search" :params="searchData" :clears="clears" :enable="startCompare">
            ...
            <label-items label="仓库：" class="inline">
                <super-select
                    v-model="searchData.warehosue_id"
                    :options="warehouseList"
                    filterable
                    clearable
                    storage-key="warehouseRules"
                >
                </super-select>
            </label-items>
            ...
            <label-items v-if="[1,2].includes(platform)" label="站点：" class="inline ml-sm">
                ...
                <!--</el-select>-->
                <super-select
                    v-model="searchData.platformSite"
                    :options="sites"
                    filterable
                    clearable
                    storage-key="rulesSitePlatform"
                  >
                </super-select>
            </label-items>
            <label-items v-if="platform" label="账号：" class="inline ml-sm">
                ...
                <!--</select-filter>-->
                <super-select
                    v-model="searchData.platformAccount"
                    :options="accounts"
                    filterable
                    clearable
                    storage-key="rulesAccoutsName"
                    placeholder="请输入账号"
                   >
                </super-select>
...
<script>
    ...
    export default{
        ...
        methods:{
            ...
            default_rule(){
                this.$http(api_acquire_default_rule,{channel_id :this.platform}).then(res=>{
        ...
        computed: {
            ...
            disabledRule(){
                if(this.autoAction===2 && (this.platform===1 || this.platform === 2 || this.platform === 3 || this.platform === 4)){
                    return false;
                }else {
                    return true;
                }
