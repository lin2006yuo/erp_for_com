<template>
    <div class="rel-goods">
        ...
        <el-tabs type="border-card" @tab-click="tab_click" style="width:100%" :active-name="currentTab"  :closable="closable"
            ...
            <el-tab-pane :label="item.account_code"
...
//                    is_plan_publish:"",
        ...
        methods: {
            ...
            init_data(goods_id,spu){//在info/index页面使用
                ...
                this.get_account(spu).then(res=>{
                    if(this.relevanceForm&&this.relevanceForm.default_account_id!==""){
                        ...
                        if(find){
                            arr.push(find);
                            this.keep_channel(arr); // 初始化时调用，获取初始数据
                        }
                    }
                });
            },
            ...
            max_weight(item,val){
                let list = item.listing_info.map(value=>{
                    return value.weight
                });
                item.grossWeight = Math.max(...list)/1000;
            },
            ...
            get_imgs(val){
            	...
            	if(!!find){
            		...
            		// this.$set(find,"is_market_image",val.is_market_image);
            		// this.$set(find,"market_images",val.market_images);
                }
            },
            // 复制状态下修改分类
            copy(val){
            ...
            copy_accout(val){
                ...
                val.forEach((row,row_i)=>{
                    ...
                    Promise.all([template, ser_template, grounp,sales,relation,custom,same_spu]).then(([temp, serv, gro,sale,rela,customer,same_pro]) => {
                        ...
                        temps.same_spus = same_pro.map(val=>{
                            return {
                                label:val.goods_spu,
                                value:val.id
                            }
            ...
            keep_channel(val){
                ...
                val.forEach(row => {
                    ...
                    Promise.all([permission, template, ser_template, grounp,sales,relation,custom,same_spu]).then(([per, temp, serv, gro,sale,rela,customer,same_pro]) => {
                        ...
                        data.sales=sale;
                        data.same_spus = same_pro.map(val=>{
                            return {
                                label:val.goods_spu,
                                value:val.id
                            }
                        ...
                        data.sku_attr_info=[];
                        data.delivery=[];
                        data.virtual_send = this.relevanceForm.virtual_send;   // 初始 虚拟仓发货
                        data.warehouse_id = this.relevanceForm.warehouse_id;    // 初始 发货仓库
...
//                                                ali.custom_pic = "";
                                    ...
                                    let length = this.relevanceForm.vars.length;
                                    this.relevanceForm.vars.push(data);
                                    //console.log('this.relevanceForm=====新增=================',this.relevanceForm)
                                    this.currentTab = this.relevanceForm.vars[length].account_code || "";
                                })
                            }
                        }else{
