    ...
    <page>
        <search-card @search="search" @enter="search" :params="searchData" :clears="clears">
            ...
            <super-select
                ...
                v-sf.accountId
                :options="amazonAccount"
                :clearable="true"
                :storageKey="'publish_list_accountId'"
            ></super-select>
            ...
            <super-select
                ...
                v-sf.create
                :options="createArr"
                :storageKey="'publish_list_create'"
            ></super-select>
        ...
        <div>
            ...
            <export-pre-publication v-model="downVisible" @down-excel="down_excel"></export-pre-publication>
        </div>
    </page>
</template>
...
<script>
    ...
    export default{
        ...
        methods: {
            ...
            get_account(){
                this.$http(api_amazon_account).then(res => {
                    res.data.forEach(item=>{
                        this.amazonAccount.push({label:item.account_name,value:item.id});
                    })
                }).catch(code => {
                    this.$message({
                        type: "error",
            ...
            get_user(){//
                this.$http(api_get_user).then(res => {
                    this.createArr = res.userList.map(item=>{
                        return {
                            label:item.username,
                            value:item.id
                        }
            ...
            down_excel(curAccount){
                let data = {
                    account: [curAccount.value,curAccount.label],
                ...
                this.$reqKey('preExport',false);
                this.$message({type:'success',message:'导出中，请稍等...'})
            },
            search(){
                this.init();
            },
        ...
        components: {
            ...
            exportPrePublication:require('./export-pre-publication').default,
        }
    }
</script>
