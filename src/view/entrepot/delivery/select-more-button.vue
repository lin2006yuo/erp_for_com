...
<script>
    ...
    export default {
            ...
            button_select(item) {
                if (!this.moreSelect) {
                    ...
                    if (item.type === 'default') {
                        item.type = 'primary';
                        this.filterButtonLists.forEach(row => {
                            if (row.value !== item.value && row.type === 'primary') {
                                row.type = 'default'
                            }
                        });
                    }
        ...
        computed: {
            ...
            filterButtonLists() {
                return this.buttonList.filter(row => row.showSearch).map(row => {
                    let find = this.selectedList.find(selected => selected.value === row.value);
                    if (find) {
                        this.$set(row, 'type', 'primary');
                    } else {
                        this.$set(row, 'type', 'default');
                    }
                    return row
                });
            },
