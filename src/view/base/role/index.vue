        ...
        <el-table
            ...
            <el-table-column inline-template label="操作" width="300">
                <div v-if="row.id != 1">
                    ...
                    |
                    <permission tag="span" :route="apis.url_role_delete" class="operate" @click="show_log(row)">日志</permission>
                    |
...
<script>
    ...
    import {api_role_list,api_role_get,api_role_delete,api_change_status,api_user_list,api_member_list,api_copy_role,api_get_log,
...
//            ---------------------------------------------------   日志
            show_log(row){
                console.log(row);
                this.title=`角色：${row.name} 的操作日志`;
                this.$http(api_get_log,row.id)
                    .then(res=>{
