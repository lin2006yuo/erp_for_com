<template>
    <div>
        ...
        <el-form v-else ref="form" v-for="(item,index) in form"
            ...
            <el-row>
                <el-col :span="12">
                    <el-form-item label="计抛重量：">
                        <div class="inline" style="vertical-align: bottom;">{{item.bubble_weight | retainInt}}</div>
                        <label class="inline" style="vertical-align: bottom;">g</label>
                    </el-form-item>
                </el-col>
                <el-col :span="12">
                    <el-form-item label="物流商运费：">
                        <div class="inline" style="vertical-align: bottom;">{{item.providers_currency_code}}&nbsp;{{item.providers_fee |retainDecimals}}</div>
                    </el-form-item>
                </el-col>
            </el-row>
            <el-row>
                <el-col :span="12">
                    <el-form-item label="物流商实际称重：">
                        <div class="inline" style="vertical-align: bottom;">{{item.providers_actual_weight | retainInt}}</div>
                        <label class="inline" style="vertical-align: bottom;">g</label>
                    </el-form-item>
                </el-col>
                <el-col :span="12">
...
//            获取仓库
            ...
            init_shipping(item){
                if(item.warehouse_id!==0){
                    if(!this.shippingData[`${item.warehouse_id}${item.country_code}${item.id}`]) {
                        ...
                        console.log(this.shipList,'11111');
                        this.$set(item, "shippingList", this.shipList);
                        this.$set(item,"newShippingList",list);
                    }
                }
            ...
            submit_params(isTrue){
                ...
                let cur = this.form.map(row=>{
                    let weight='';
                    if (row.shipping_id) {
                        row.shippingList.filter(item=>{
                            if (row.shipping_id===item.value){
                                weight=item.weight
                            }
                        })
                    }
