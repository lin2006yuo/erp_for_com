    ...
    export default {
        data() {
            return {
                ...
                custom: {
                    product_name: (val) => {
                        let style = this.templateData.temp_data.find(row => row.value === 'product_name').style;
                        if (!this.span) {
                            this.create_span();
                        }
                        let query = this.get_span_replace(val, parseInt(style.width));
                        if (query === false) {
                            return val;
                        } else {
                            let replace = val.slice(query.index, query.sum);
        ...
        methods: {
            ...
            get_span_replace(val, limit) {
                ...
                let width = this.span.offsetWidth;
                let diff = Number(width) - Number(limit);
                let diffNum, index;
                if (diff > 0) {
                    diffNum = Math.ceil(diff / Number(width) * length) + 3;
                    index = Math.floor((length - diffNum) / 2);
                    return {
                        index,
                        sum: diffNum + index,
                    }
                ...
                if (val) {
                    this.$nextTick(() => {
                        if (this.batchData.default_temp_id) {
                            this.$refs.initial_value[0] && this.$refs.initial_value[0].inputSelect();
                        }
                    });
                    this.batchData.print_data.forEach(row => {
