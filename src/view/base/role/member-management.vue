    ...
    <page-dialog class="p-memeber-management" :title="`给角色：${titleName} 添加成员`"
        ...
        <el-row :gutter="8">
            <el-col :span="memberList">
                <el-card class="mb-xs">
                    <el-row>
                        <el-col :span="16" class="search-row-member">  
                            <el-select v-model="searchFlag" class="select-name" >
                                <el-option v-for="item in searchOptions" :key="item.value" :value='item.value' :label='item.label'></el-option>
                            </el-select>
                            <el-input  icon="search" v-model="filterMember"   placeholder="搜索用户"></el-input>
                            <!-- <el-button size="mini" type='primary' @click.native="searchName">搜索</el-button>                            -->
                        </el-col>
        ...
        computed:{
            ...
            members(){
                if (this.searchFlag === 1) {
                   return this.memForm.data.filter(row=> !!~(row.realname.indexOf(this.filterMember)));                   
                } else {
                    return this.memForm.data.filter(row=> !!~(row.username.indexOf(this.filterMember)));
                }                
            },
            memberListName(){
