    ...
    <div class="c-data-table">
        ...
        <ui-table
            ...
            :heads="[
                ...
                {label:'账号简称',width:10},
                {label:'销售员',width:10},
                {label:'已收藏',width:8,isSort:true,order_type:'likes'},
                {label:'已售量',width:8,isSort:true,order_type:'sales'},
                {label:'上传时间',width:10,isSort:true,order_type:'publish_create_time'},
                {label:'更新时间',width:10},
                {label:'操作',size:120}
                ]">
            <tbody>
            <template v-for="(data, index) in tableData">
                <tr :class="[data.show ? 'active' : '',data.heighLight?'active-color':'',data.listing_status===5?'redborder':'']" @click="addClass(index)"
                    ...
                    <td class="relative">
                        <!--本地spu-->
                        <span @click="bind_spu(data,index)" class="operate" v-if="data.spu===''">
                             <i class="el-icon-information"
                                style="position: absolute;top:50%;margin-top:-6px;left: 30px"
                                :title="$t('ebay-list.clickLink')"></i>
                            <ui-tip content="未关联" style="padding-left: 12px"></ui-tip>
                        </span>
                        <ui-tip :content="data.spu" v-else></ui-tip>
                    </td>
                    <td><ui-tip :content="data.name"
                                :is-operate="true"
                                @cur-click="cur_click(data)"
                                :width="74"></ui-tip></td>
                    ...
                    <td><times :time="data.publish_create_time" :width="80"></times></td>
                    <td><times :time="data.publish_update_time"></times></td>
                    <td>
                        <trends-select
                            class="inline"
                ...
                <template v-if="data.show">
                    ...
                    <tr v-for="(item, item_i) in data.variants" :key="item.id"  class="variant">
                        ...
                        <td colspan="1"><ui-tip :content="!item.local_sku?'未关联':item.local_sku" :width="98"></ui-tip></td>
        ...
        <bind-spu v-model="bindVisible"  :channel-sku="channelSku" :local-sku="localSku" type="shopee" @refresh="valids_shopee"></bind-spu>
    </div>
</template>
<style lang="stylus">
...
<script>
    ...
    import  {api_publish_shopee_variant,api_batch_setting,api_batch_del_item,
	...
	export default{
        ...
        methods:{
            ...
            get_variations(data){
                data.varians = [];
                this.$http(api_publish_shopee_variant,data.id).then(res=>{
                    data.varians = res;
                }).catch(code=>{
            ...
            show_detail(data){
             ...
             bind_spu(data, index){//关联本地spu
                 this.index = index;
                 this.tableData_item = data;
                 this.$http(api_publish_shopee_variant, data.id).then(res=>{
                     this.channelSku = res.map(row=>{
                         return {
                             key:[{name:'颜色',field:'name'},{name:'价格',field:'price'}],
                             id:row.id||0,
                             pid:row.pid,
