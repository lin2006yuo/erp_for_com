    ...
    <page class="p-index">
        <label-item label="请选择第三方仓库：" class="mb-xs mt-sm">
            <super-select v-model="warehouse_id"
                          ...
                          class="inline"
                          :options="warehouses"
                          storageKey="thirdWarehouse"></super-select>
            ...
            <div class="mb-mini el-row">
                ...
                <label-item label="" class="inline ml-sm">
                    ...
                    <label-item class="ml-sm" label="开发员：">
                        <super-select v-model="paramsData.developer_id"
                                      class="s-width-default"
                                      storageKey="warehouseDeveloper"></super-select>
                        <!--<param-account-->
                                <!--class="s-width-default"-->
        ...
        <goods-classify ref="classifyPart" :leftControllerTitle="leftControllerTitle">
            ...
            <div slot="button-list">
                ...
                <permission tag="ElButton"
                            :route="apis.url_sync_third_price"
                            class="ml-sm"
                            size="mini"
                            type="default"
                            v-show="isShowSyncBtn"
                            @click.native="sync_third_price">同步第三方仓价格</permission>
            </div>
            <!-- 商品列表树 -->
                        ...
                        <tr v-for="channel in row.channel_detail">
                            ...
                            <td>
                                <el-popover placement="bottom"
                                            ref="stock_quantity"
                                            @show="show_lock_stock(row.sku_id, channel.channel_id)"
                                            trigger="click"
                                            width="500">
                                    <el-table :data="stockLockDetail"
                                              max-height="250"
                                              class="scroll-bar">
                                        <el-table-column width="125" label="借调单号" prop="code"></el-table-column>
                                        <el-table-column width="125" label="借调人" prop="creator"></el-table-column>
                                        <el-table-column width="125" label="可借调数量" prop="can_lend_qty"></el-table-column>
                                        <el-table-column width="125" label="调拨占有数" prop="occupy_qty"></el-table-column>
                                    </el-table>
                                    <span slot="reference" class="operate">{{channel.lend_lock_quantity}}</span>
                                </el-popover>
                            </td>
                            <!--可用库存-->
                            <td>
...
<script>
    ...
    import {api_lock_detail} from "@/api/warehouse-goods-channel";
    import {
        api_third_stock,
        api_change_alert,
        api_get_apply,
        ...
        api_sync_third_price,
        url_sync_third_price,
    ...
    export default {
        ...
        methods: {
            //同步第三方仓价格
            sync_third_price(){
                let skus = this.tables.lists.filter(row => row.isCheck).map(row => row.thirdparty_goods_sku);
                let data = {
                    warehouse_id: this.warehouse_id,
                    third_skus: skus
                };
                this.$http(api_sync_third_price, data).then(res => {
            ...
            get_staffs(page, pageSize, query){
                let url = "get|user/development/staffs";
                return http(url)
            },
            hidden_left() {
            ...
            warehouses_init() {
                this.$http(api_get_warehouse_default, {sign: 'third'}).then(res => {
                    this.warehouses = res.map((item) => {
                        return {
                            ...
                            is_update_price: item.is_update_price
                        };
                    });
            ...
            // 调拨锁定库存
            show_lock_stock(sku_id, channel_id) {
                let data = {
                    sku_id: sku_id,
                ...
                this.$http(api_lock_detail, data).then(res => {
        ...
        computed: {
            ...
            isShowSyncBtn(){
                if (this.warehouses.length !== 0) {
                    return Number(this.warehouses.find(row => row.value === this.warehouse_id).is_update_price) === 1;
                }
                return false;
            },
            skuIdData() {
