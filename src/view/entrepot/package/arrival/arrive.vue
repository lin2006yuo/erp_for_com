    ...
    <div>
        <el-card class="mb-sm">
            <div class="mb-xs arrive-box">
                <label-item label="拆板卡板号：">
                    <el-input ref="box_arrive" placeholder="请扫描卡板号" v-model="box_id" :disabled="!canArriveBox"
                              @focus="select_box" @keyup.enter.native="box_arrive"
                              class="inline width-super"></el-input>
                    <el-button type="primary" @click.native="change_box" class="ml-sm" size="mini">
                        更换卡板号
                    </el-button>
                    <span v-if="!canArriveBox" class="ml-sm font-18 bold-font operate-color">
                        卡板&nbsp;{{box_id}}&nbsp;拆板拆包中</span>
                </label-item>
            </div>
            <el-radio class="radio" v-model="searchData.s_bill_type" label="2">搜运单号
            </el-radio>
            <el-radio class="radio" v-model="searchData.s_bill_type" label="1">搜采购单ID
            </el-radio>
            <el-input v-model="searchData.s_bill_number" placeholder="请输入内容" class="inline mr-sm width-super ml-sm"
                      @focus="number_focus" @keyup.enter.native="s_bill_enter($event)" autofocus ref="search_input">
            </el-input>
            <permission tag="ElButton" class="inline" :route="apis.url_parcel_search" type="primary"
                        @click.native="search_btn(searchData)" size="mini">查询
            </permission>
        ...
        <div class="mb-sm ml-lg" v-if="submitShow" style="overflow: hidden">
            <div class="fl">
                ...
                <trends-select class="inline" @trigger="p_action($event)" :selects="printallOptions" type="primary">
                </trends-select>
            </div>
            <div class="fr">
                <label>使用打印模板：</label>
                <el-select v-model="default_temp_id" placeholder="请选择" class="mr-sm inline" @change="change_print">
        ...
        <el-row class="el-row-reset-card" :style="cardStyle">
            <el-form v-loading="loading" element-loading-text="玩命加载中">
                <el-form-item>
                    <el-card v-for="(item, index) in arriveData" class="mb-xs" :key="item.id">
                        <span class="mr-lg bold-font font-18 red">{{item.warehouse_name}}</span>
                        <span class="bold-font">采购单ID：</span>
                        <span class="mr-sm color-success">{{item.purchase_order_id}}</span>
                        <span class="bold-font">采购单号：</span>
                        <span class="mr-sm color-success">{{`PO${item.purchase_order_id}`}}</span>
                        ...
                        <span class="color-success">{{item.purchase_user_name}}</span>
                        <span class="mr-lg ml-lg bold-font font-18 red" v-show="item.support_label">供应商已贴标</span>
                        <el-button class="inline ml-sm mb-xs" type="primary" size="mini"
                                   @click="abnormal_feedback(item)">拆包异常反馈
                        </el-button>
                        <el-table :data="item.sku_list" class="arrival-table scroll-bar" ref="table"
                                  :row-key="set_row_key"
                                  @selection-change="handle_selection_change(index, $event, item)"
                                  highlightCurrentRow max-height="500" style="width: 100%;">
                            <el-table-column type="selection" :selectable="selectable" width="30">
                            </el-table-column>
                            <el-table-column min-width="70" label="图片">
                                <template slot-scope="scope">
                                    <el-popover placement="right" trigger="hover">
                                        <img :src="scope.row.thumb | imgSrcFilter" width="200px" height="200px">
                                        <span slot="reference">
                                            <img v-lazy="scope.row.thumb" @click="search_img(scope.row.thumb)"
                                                 v-if="scope.row.thumb" height="60px" width="60px" style="border:none">
                                        </span>
                                    </el-popover>
                                </template>
                            </el-table-column>
                            <el-table-column min-width="120" label="SKU/SKU别名">
                                <template slot-scope="scope">
                                    <el-row>
                                        <ui-tip v-if="$hasPermission(apis.url_sku)" :content="scope.row.sku"
                                                @cur-click="show_goods_detail(scope.row)" :is-operate="true"
                                                :width="98">
                                        </ui-tip>
                                        <ui-tip v-else :content="scope.row.sku" :width="98"></ui-tip>
                                        <el-button v-if="(scope.row.check === 0) && $hasPermission(apis.url_get_sku)"
                                                   type="primary" @click="re_check(scope.row)" size="mini">复核
                                        </el-button>
                                    </el-row>
                                    <ui-tip :content="`${scope.row.sku_alias || '--'}`"></ui-tip>
                                </template>
                            </el-table-column>
                            <el-table-column min-width="200" inline-template label="名称">
                                <div>
                                    {{row.sku_name}}
                                </div>
                            </el-table-column>
                            ...
                            <el-table-column
                                    inline-template
                                    label="单价">
                                <div :class="+row.price >= 100 ? ['red', 'bold'] : ''">
                                    {{row.price.toFixed(3)}}{{item.currency_code}}
                                </div>
                            </el-table-column>
                            <el-table-column min-width="70" inline-template label="货位">
                                <span>{{row.warehouse_cargo_code || `--`}}</span>
                            </el-table-column>
                            <el-table-column label="缺货数量" min-width="60" inline-template>
                                <span>{{row.shortage_quantity}}</span>
                            </el-table-column>
                            <el-table-column prop="qty" min-width="60" label="采购数量">
                            </el-table-column>
                            <el-table-column inline-template min-width="60" label="已接收数">
                                <div>
                                    <ui-tip :content="row.expected_int_qty | isZero" :width="98"></ui-tip>
                                </div>
                            </el-table-column>
                            <el-table-column label="本次良品" min-width="100" class="t-center input-number required-sign">
                                <template slot-scope="scope">
                                    <el-input v-model="scope.row.accepted_goods_qty"
                                              ...
                                              :ref="`accept${scope.row.num}`"
                                              :disabled="scope.row.status === 1 || item.is_allow_receipt === 0"
                                              style="width: 100%" type="number" @focus="goods_focus(scope.row)"
                            ...
                            <el-table-column label="本次次品" inline-template class="t-center ">
                                <el-input v-model="row.defective_product_qty" :min="0" style="width: 100%" type="number"
                                          disabled>
                                </el-input>
                            </el-table-column>
                            <el-table-column inline-template label="备注">
                                <div>
                                    <el-input v-model="row.note" @change="trim_note(row)" style="width: 100%">
                                    </el-input>
                                </div>
                            </el-table-column>
                            ...
                            <el-table-column label="操作" min-width="120">
                                <template slot-scope="scope">
                                    <trends-select class="inline" @trigger="r_action(scope.row,item,$event)"
                                                   :selects="options(scope.row)" type="primary">
                                    </trends-select>
                                </template>
                            </el-table-column>
                        </el-table>
                ...
                <div v-if="showSave && submitShow" class="fr">
                    <permission tag="ElButton" :route="apis.url_parcel_receipt" type="primary" class="mr-md"
                                :disabled="btnDisabled" @click.native="form_submit(false)" size="mini">保存
                    </permission>
                    <permission tag="ElButton" :route="apis.url_parcel_receipt" type="primary" class="mr-md"
                                :disabled="btnDisabled" @click.native="form_submit(true)" size="mini">保存并打印
                    </permission>
                </div>
        ...
        <!--</el-card>-->
        <search-dialog v-model="dialogTableVisible" @refresh="search_btn" :purchase-code="purchaseCode"
                       :purchase-note="purchaseNote">
        </search-dialog>
        <review-dialog v-model="review_visible" :goods_detail="goods_detail" @init="search_again">
        </review-dialog>
        <goods-detail v-model="goods_visible" :goods_detail="goods_detail">
        </goods-detail>
        <preview v-model="printVisible" :table-data="tableData" :print-data="printData" :relevance="relevance"
                 :printer="printer" :preview-show="previewShow" ref="preview">
        </preview>
        <batch_preview v-model="batch_printVisible" :batch-show="batchShow" :relevance="relevance" :printer="printer"
                       :batch-data="batchData" ref="batch_preview">
        </batch_preview>
        <blowup-image v-model="imgDialog" :img-path="imgPath" :title="`查看大图`"></blowup-image>
        <abnormal-feedback v-model="feedbackVisible" @search="search_btn(searchData)" :feedback-info="feedbackInfo">
        </abnormal-feedback>
    </div>
</template>
<style lang="stylus">
...
<script>
    ...
    export default {
                    ...
                    p_action() {
                        ...
                        for (let i = 0; i < this.select_sku.length; i++) {
                            if (this.select_sku[i]) {
                                for (let j = 0; j < this.select_sku[i].length; j++) {
                                    ...
                                    post.purchase_order_id = this.arriveData[i].purchase_order_id;
                                    ...
                                    // }
                                    post.label_type = 1;
                                    post.purchase_parcel_code = this.arriveData[i].purchase_parcel_code;
                        ...
                        this.$http(api_print_label, {
                            ...
                            this.$refs.batch_preview.get_template(1, {
                                warehouse_id: this.arriveData[0].warehouse_id
                            });
        ...
        methods: {
            ...
            options(row) {
                return row.operateButtons.filter(operate => {
                    return this.$hasPermission(operate.apis);
                })
            },
            get_single_params(data, list) {
                let skus = [];
                ...
                skus.push({
                    ...
                    is_allow_receipt: list.is_allow_receipt,
                    note: data.note
                });
                let info = {
                ...
                return receiptParcelInfo
            },
            // 1: 打印标签 / 2: 保存并打印 / 3: 保存
            r_action(data, list, ev) {
                const receiptParcelInfo = this.get_single_params(data, list)
                switch (ev.value) {
                    case 1:
                        this.tableData = [];
                        let post = {
                            ...
                            label_type: 1,
                            purchase_parcel_code: list.purchase_parcel_code,
                            shortage_quantity: data.shortage_quantity //TODO: 缺货数量后端数据源被注释，暂时不传
                        };
                        // if(post.num <= 0){
            ...
            form_submit(isPrint) {
                let receiptParcelInfo = [];
                let i = 0;
                this.arriveData.forEach(item => {
                    item.sku_list.forEach(row => {
                        if (!!Number(row.accepted_goods_qty || !!Number(row.defective_product_qty))) {
                            let param = this.get_single_params(row, item)[0];
                            let curInfo = receiptParcelInfo.find(row => row.purchase_order_id === param
                                .purchase_order_id);
                            curInfo ? (curInfo.skuInfo = curInfo.skuInfo.concat(param.skuInfo)) :
                                receiptParcelInfo.push(param);
                        }
                    })
                });
                    ...
                    this.$http(api_parcel_receipt, {
                        ...
                        if (res.status === 1) {
                            ...
                            if (isPrint) {
                                ...
                                this.arriveData.forEach(item => {
                                    item.sku_list.forEach(row => {
                                        if (!!Number(item.accepted_goods_qty) || !!
                                            Number(item.defective_product_qty)) {
                                            let post = {
                                                sku_id: row.sku_id,
                                                ...
                                                label_type: 1,
                                                shortage_quantity: row
                                                    .shortage_quantity,
                                                purchase_parcel_code: item
                                                    .purchase_parcel_code,
                                            };
                                            printList.push(post);
                                        }
                                ...
                                this.$http(api_print_label, {
                                    ...
                                    this.batch_printVisible = true;
                                    this.batchData.default_temp_id = clone(this
                                        .default_temp_id);
                                    this.$nextTick(() => {
                ...
                this.$http(api_parcel_search, search).then(res => {
                    ...
                    if (res.status === 1) {
                        if (res.parcel_number > 1) {
                            this.purchaseNote = [];
                            this.purchaseNote = res.parcel_row_list.map(row => {
                                return Object.assign(row, {
                                    label: row.tracking_number,
                                    value: row.purchase_parcel_code
                                });
                            });
                            this.purchaseCode = search.s_bill_number;
                            this.dialogTableVisible = true;
                        } else {
                            if (!this.box_check(res.box_status, res.is_parcel_box)) {
                                return;
                            }
                            // 数据初始化
                            this.init(res);
            ...
            // 数据初始化
            init(res) {
                this.arriveData = res.list;
                let i = 0;
                this.arriveData.forEach(item => {
                    item.sku_list.forEach(row => {
                        this.$set(row, "tag_note", "");
                        this.$set(row, "quality_type", "2");
                        this.$set(row, "num", i);
                        i++;
                        this.$set(row, 'operateButtons', clone(this.operateButtons));
                        if (row.status === 1 || item.is_allow_receipt === 0) {
                            row.operateButtons[0].disabled = true;
                            row.operateButtons[2].disabled = true;
                        } else {
                            row.operateButtons[0].disabled = false;
                            row.operateButtons[2].disabled = false;
                        }
            ...
            default_focus() {
                let i = 0;
                let key = `accept${i}`;
                let curInput = this.$refs[key][0];
                while (curInput) {
                    if (!(curInput.$refs.input.disabled)) {
                        curInput.inputSelect();
                        break;
                    }
                    i++;
                    key = `accept${i}`;
                    curInput = this.$refs[key][0];
                }
            },
            search_again() {
            ...
            key_updown(ev, row) {
                ...
                switch (ev.keyCode) {
                    case 34:
                        this.select_jump(row.num, flag, key, curInput, 'next');
                    case 33:
                        this.select_jump(row.num, flag, key, curInput, 'prev');
                    default:
                        return;
                }
            },
            // 快捷键光标跳转
            select_jump(start, flag, key, curInput, direction) {
                do {
                    if (flag !== row.num && !curInput.$refs.input.disabled) {
                        curInput.inputSelect();
                        break;
                    }
                    direction === 'next' ? flag++ : flag--;
                    key = `accept${flag}`;
                    curInput = this.$refs[key][0];
                } while (curInput)
            },
            ...
            goods_focus(row) {
                let key = `accept${row.num}`;
                this.$refs[key][0].inputSelect();
            },
            get_template(id) {
                let data = {
                    warehouse_id: this.arriveData[0].warehouse_id,
                ...
                this.$http(api_box_transfer, this.box_id).then(res => {
                    switch (res.box_status) {
                        ...
                        case 4:
                            this.$message({
                                type: 'error',
                                message: this.boxMessage[res.box_status]
                            });
                            this.box_id = '';
                            this.select_box();
                            break;
                ...
                if (this.box_id === '') {
                    switch (box_status) {
                        ...
                        case 2:
                            this.$message({
                                type: 'error',
                                message: this.trackingMessage[box_status]
                            });
                            this.searchData.s_bill_number = '';
                            this.select_box();
                            return false;
                    ...
                    if (current_box === false) {
                        switch (box_status) {
                            ...
                            case 2:
                                this.$message({
                                    type: 'error',
                                    message: this.trackingBoxMessage[box_status]
                                });
                                this.searchData.s_bill_number = '';
                                this.number_focus();
                                return false;
        ...
        computed: {
            ...
            showSave() {
                return this.arriveData.some(item => item.is_allow_receipt !== 0)
            }
        },
        watch: {
            print_all(val) {
                this.$nextTick(() => {
                    this.arriveData.length &&
                    this.arriveData.forEach((item, index) => {
                        item.sku_list.forEach(row => {
                            this.$refs.table[index].toggleRowSelection(row, val);
                        })
                    });
