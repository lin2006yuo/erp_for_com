        ...
        <el-table
            ...
            <el-table-column
                ...
                <div>
                    ...
                    <span v-if="item.value2 !== undefined">/{{row[item.value2]}}</span>
                </div>
            </el-table-column>
        </el-table>
        <el-pagination
        ...
        methods: {
            async init(){
                try {
                    ...
                    this.tableData = this.add_pack_sku_sum(res.data);
            ...
            //添加sku打包数量字段
            add_pack_sku_sum(tableData) {
                return tableData.map(row => {
                    const sku_pack_sum = +row.single_single_packing_quantity + +row.more_sku_quantity + +row.remark_sku_quantity + +row.single_more_sku_quantity + +row.large_single_single_packing_quantity + +row.large_more_sku_quantity + +row.large_remark_sku_quantity + +row.large_single_more_sku_quantity//添加+号强制转化为number类型
                    const newRow = {...row, sku_pack_sum}
                    return newRow
                })
            }
        },
