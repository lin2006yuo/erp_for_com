        ...
        <div class="p-titleColour-attribute">
            ...
            <table class="template">
                <tr>
                    ...
                    <th width="105px" class="required-sign">销售价($)</th>
                    ...
                    <th width="105px" class="required-sign">运费($)</th>
                    <th width="80px">操作</th>
                </tr>
                <tbody>
                    <tr>
                        ...
                        <!--销售价-->
                        <th>
                            <ui-limit-number class="input-width min-width"
                                             v-model="price"
                                             @blur="blur_change('price',price)"
                                             :limit="2"></ui-limit-number>
                            <span @click="is_get_standard_price('price')" class="btn-icon" title="点击初始化刊登销售价" ></span>
                        </th>
                        <!--数量-->
                        <th>
                        ...
                        <th>
                            <ui-limit-number class="input-width min-width"
                                             ...
                                             :limit="2"></ui-limit-number>
                            <span @click="is_get_standard_price('shipping')" class="btn-icon" title="点击初始化刊登运费" ></span>
...
//            },
            ...
            is_get_standard_price(val){
                let bool=true;
                for (let i of this.tableData) {
                    if(i[val]!=''){
                        bool=false;
                        this.$confirm(`此操作将重置所有SKU的${val=='price'?'销售价格':'运费'}`, '提示', {
                            confirmButtonText: '确定',
                            cancelButtonText: '取消',
                            type: 'warning'
                        }).then((res) => {
                            this.get_standard_price(val);
                        }).catch(code=>{
                            return this.$message({
                                type:'info',
                                message:"已取消"
                ...
                bool&&this.get_standard_price(val);
            },
            get_standard_price(val){
                const params = {
                    channel_id:this.channelId,
                    account_id:this.accountId,
                    sku_id:this.tableData.map(row=>row.sku_id).filter(row=>!!row),
                };
                this.$http(api_ref_price, params).then(res=>{
                    switch (val) {
                        case 'price':
                            for(let key in res ){
                                const find = this.tableData.find(ref=>{
                                    return Number(ref.sku_id) === Number(key);
                                });
                                if(!res[key].total_price){
                                    return this.$message({type:'info',message:'找不到销售价规定'})
                                }
                                find.price=res[key].total_price;
                            }
                            break;
                        case 'shipping': for(let key in res ){
                            const find = this.tableData.find(ref=>{
                                return Number(ref.sku_id) === Number(key);
                            });
                            if(!res[key].shipping_fee){
                                return this.$message({type:'info',message:'找不到运费规定'})
                            }
                            find.shipping=res[key].shipping_fee;
                        }
                            break;
                    }
