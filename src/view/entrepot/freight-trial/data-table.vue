<template>
    <div class="c-data-table">
        ...
        >
            ...
            <el-table-column label="抛比" inline-template :width="120" >
                <div>{{row.volumn_weight | filterPrice}}</div>
            </el-table-column>
            <el-table-column label="抛重" inline-template :width="120" >
                <div>{{row.throw_weight | filterPrice}}</div>
            </el-table-column>
            <el-table-column label="计费重量" inline-template :width="120" >
                <div>{{row.weight | filterPrice}}</div>
            </el-table-column>
            <el-table-column label="运费" inline-template :width="100" v-if="show">
                <div>{{row.before_amount | filterPrice}}</div>
            </el-table-column>
            <el-table-column label="挂号费" inline-template :width="100" v-if="show">
                <div>{{row.registered_fee | filterPrice}}</div>
            </el-table-column>
            <el-table-column label="处理费" inline-template :width="100" v-if="show">
            ...
            <el-table-column label="当前币种折后总价" inline-template v-if="show">
                <div class="red bold-font">
                    <span>{{row.currency_code}}</span>
                    <span class="font-18">{{row.amount | filterPrice}}</span>
                </div>
            </el-table-column>
            <el-table-column label="物流总运费（人民币）" inline-template sortable>
                <div class="red bold-font">
                    <span>RMB</span>
