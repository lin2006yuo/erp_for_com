...
<!--            </el-select>-->
                <super-select   v-model="searchData.channel_id"
                                :options="channel"
                                storageKey="ymxSettingChannelOptions"
            ...
            <label-item label="账号简称：" class="ml-sm">
                ...
                <super-select   v-model="searchData.account_id"
                                ...
                                storageKey="ymxSettingAccountOptions"
                                :placeholder="'请选择平台'"
                                class="s-width-middle"
                                v-sf.account_id>
                </super-select>
        ...
        methods: {
            change_channel(channel_id){
                ...
                this.$http(api_email_account,{channel_id:channel_id}).then(res=>{
                    ...
                    this.accoutOptions=this.accoutOptions.map(item=>{
                        return {label:item.label,value:item.value};
                    })
                    console.log("accoutOptions",this.accoutOptions);
