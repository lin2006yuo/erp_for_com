            ...
            <el-form v-for="row in data" :key="row.id" :model="row" label-width="120px">
                ...
                <el-form-item label="客服：">
                    ...
                    <super-select
                            ...
                            v-model="row.customer_id"
                            :storage-key="`memberShipCustomer`"
...
<script>
    ...
    import {api_member_update,api_member_del,api_add_member,api_sales_dialog} from '@/api/member';
    export default{
        data(){
            return {
                rowId:"",
...
//                    0-所有 1-本地 2-海外
        ...
        methods:{
            sales_sales(){
                this.$http(api_sales_dialog,'sales').then(res=>{
                    this.sellerData = res.map(row=>{
                        return {label:row.realname,value:row.id};
                    });
                })
            },
            sales_customer(){
                this.$http(api_sales_dialog,'customer').then(res=>{
                    this.customerData = res.map(row=>{
                        return {label:row.realname,value:row.id};
                    });
                })
            },
            ...
            save(){
                ...
                if(this.data[0].id){
                    ...
                    // }else {
                    //     this.$message({type: 'warning', message: '销售员为必填项'});
                    // }
