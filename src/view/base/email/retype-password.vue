    ...
    <div class="p-retype">
        <page-dialog title="显示密码" v-model="show" size="small" @open="open" @close="close" :close-on-cilck-modal="false">
            ...
            <div slot="footer" class="dialog-footer">
                <request-button req-key='retypePass' @click="keep">确认</request-button>
...
<script>
    import {api_get_password} from '@/api/email'
    export default{
        data(){
            return{
                show:this.value,
        ...
        methods:{
            ...
            keep(){
                if(this.login_password) {
                    this.show = false;
                    this.$http(api_get_password,this.retypeData.id,{password:this.login_password}).then(res => {
                        this.$emit('get-password',res,this.retypeData.type);
                    }).catch(code=>{
