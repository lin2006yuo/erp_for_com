<template>
    ...
    <page-dialog v-model="editVisible" size="full"
        ...
        <el-tabs ref="tabs" @tab-click="tab_click" :active-name="action">
            ...
            <!--运费折扣-->
            <el-tab-pane name="discount" class="scroll" label="其他" key="discount">
        ...
        <div slot="footer" class="dialog-footer">
            <div >
                <el-button v-if="editable" size="mini" type="primary" @click="keep">确定</el-button>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            keep_eff() {
                ...
                switch (this.effData.arriveable_type) {
                    ...
                    case 2:
                        ...
                        //偏远费校验
                        /*let validate = this.remote_validate(data.details);
                        if(validate){
                            return this.$message({type: 'warning', message: `第${validate.errIndex}组偏远费设置有误`})
                        };*/
                        break;
                }
            ...
            remote_validate(details) {
                let errIndex = 0;
                if(details.some((row,index) => {
                    // 定位报错组别
                    errIndex++;
                    if(row.is_remote) {
                        return row.remote.some(item => {
                            let location = row.locations.find(location => location.country_code === item.country_code);
                            if(location && location.is_remote) {
                               return item.detail.some(remote => {
                                    return remote.city.length === 0 ||
                                    remote.zip_range.length === 0 ||
                                    Object.keys(remote).some(key => key !== 'id' && (remote[key] === '' || remote[key] === 0));
                                })
                            }
                            return false;
                        });
