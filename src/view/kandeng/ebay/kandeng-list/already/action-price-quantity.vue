<template>
    <page-dialog v-model="show" size="large" @open="open"
        ...
        <table class="template">
            ...
            <tr v-for="row in showTables" :key="row.id">
                <td>
                ...
                <td>
                    <ui-tip :content="row.local_sku" :width="95"></ui-tip>
                </td>
                <td>
                    <ui-tip :content="row.link_sku" :width="95"></ui-tip>
                </td>
        ...
        <div slot="footer" class="dialog-footer">
            ...
            <request-button @click="keep" req-key="actionPrice" v-else>{{$t('common.confirm')}}</request-button>
                    ...
                    this.selectIds.forEach(row=>{
                        ...
                        if(row.varians&&row.varians.length!==0){
                            row.varians.forEach(sku => {
                                let variansSpu = this.showTables.find(res=>{
                                    return res.spu===row.spu&&res.listing_id===row.id;
                                });
                                let obj = {
                                    ...
                                    isCheck:false,
                                    spu:!!variansSpu?' ':row.spu,
                                    local_sku:sku.v_sku,
                                    link_sku:sku.combine_sku,
                            ...
                            let find=this.showTables.find(it=>{
                                return !!it.sku&&it.sku ===row.listing_sku;
                            });
                            !find&&this.showTables.push(obj);
                        }
                    })
