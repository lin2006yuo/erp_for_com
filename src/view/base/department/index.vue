<template>
    <page class=" department">
        <search-card class="mb-xs"
                     ...
                     @search="init"
                     @enter="init">
            <label-item label="部门标签：" class="mr-sm">
                <super-select   v-model="dataParams.tag_id"
                                :options="tagOptions"
                                storageKey="departmentTagOptions"
        ...
        <journal-data :title="title"
                      v-model="logsData"
                      :lists="lists"></journal-data>
    </page>
</template>
<style lang="stylus">
    .department {
...
//                this.$forceUpdate();
            ...
            journal(row){
                this.lists = [];
                this.logsData = true;
                this.title = `部门：${row.name} 的操作日志`;
                this.$http(depart_logs,row.id).then(res=>{
                    this.lists = res;
                }).catch(code => {
                    this.$message({type: "error", message: code.message || code})
                })
...
//                        row.status=!row.status
            ...
            get_tag_options(){
                console.log("获取标签列表");
                this.$http(api_search_tag,{pageSize:1000,status:1})
                    .then(res=>{
