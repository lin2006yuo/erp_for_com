    ...
    <page class="p-index">
        <label-item label="请选择亚马逊FBA仓：" class="mb-xs mt-sm">
            <super-select v-model="warehouse_id"
                          ...
                          class="inline"
                          :options="warehouses"
                          storageKey="amazonFbaWarehouse"></super-select>
        ...
        <goods-classify ref="classifyPart" :leftControllerTitle="leftControllerTitle">
            ...
            <div slot="button-list">
                ...
                <permission tag="ElButton" :route="apis.url_get_stock_cycle" class="ml-sm" size="mini" type="default"
                            @click.native="$refs.fba_table.get_stock_cycle">设置备货周期
                </permission>
                <permission tag="trendsSelect"
                            class="inline ml-sm"
                            type="primary"
                            :route="apis.url_post_export_fba_stock"
                            :selects="partAllOptions"
                            @trigger="export_data"></permission>
                <permission tag="ElButton" :route="apis.url_post_import_sku" class="ml-sm" size="mini" type="primary"
                            @click.native="import_sku">导入SKU
                </permission>
            </div>
            <!-- 商品列表树 -->
        ...
        <import-dialog v-model="importVisible"
                       @init="init"
                       :warehouse_id="searchData.warehouse_id"
                       :import-info="importInfo"></import-dialog>
    </page>
</template>
...
<script>
    ...
    import {url_change_alert, url_get_apply, url_get_lend, url_get_logs, url_get_allot, url_get_stock_cycle} from '@/api/overseas-stock';
    import {url_get_details} from '@/api/entrepot-overseas';

    import {api_warehouse_info, api_stock_list, api_post_export_fba_stock, url_post_export_fba_stock, api_post_import_sku, url_post_import_sku} from '@/api/fba-stock.js';
    ...
    export default {
        ...
        methods: {
            export_data(val) {
                if (val.value === 1 && this.$refs.fba_table.selectedIds.length <= 0) return this.$message({
                    type: "warning",
                    message: "请先选择需要导出的数据"
                });
            ...
            creat_excel() {
                ...
                switch (this.export_type) {
                    case 1://部分
                        data = {
                            ids: this.$refs.fba_table.selectedIds,
            ...
            order_export(params) {
                return this.$http(api_post_export_fba_stock, params).then(res => {
                    if (res.join_queue === 1) {
            ...
            warehouse_info() {//request => 亚马逊FBA仓
                ...
                this.$http(api_warehouse_info, params).then(res => {
        ...
        components: {
            ...
            importDialog: require('./import-dialog').default
        }
    }
</script>
