...
<script>
    import {api_servers_pwd} from '@/api/server-management'
    export default{
        data(){
            return{
                show:this.value,
        ...
        methods:{
            ...
            keep(){
                if(this.login_password) {
                    ...
                    this.$http(api_servers_pwd,this.retypeData.id,data).then(res => {
                        let password = ''
                        if(this.retypeData.type==='all'){
                            password = res.data.password;
                        }else {
                            password = res.data.proxy_user_password
                        }
                        this.$emit('get-password',password,this.retypeData.type);
                        this.show = false;
                    }).catch(code=>{
