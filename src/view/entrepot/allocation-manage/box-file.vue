<template>
    <page-dialog class="c-allocation-attachment"
                 tag="附件上传后不可更改"
                 v-model="visible" @open="open" @close="close"
                 :title="trendAction.title">

        <div>
            <h3 class="t-left">商品条码附件(仅销售)：</h3>
            <!--<el-row v-if="barcode_list.length === 0">
                <el-button type="primary"
                           v-if="trendAction.status === 0"
                           size="mini" @click="upload_file('barcode_list')">
                    上传商品条码
                </el-button>
            </el-row>-->
            <el-row v-if="barcode_list.length !== 0">
                <el-col :span="20">
                    <div v-for="(file, index) in barcode_list"
                         :key="index">
                        <el-tag class="mr-xs mb-xs"
                                type="primary">{{file.name}}
                        </el-tag>
                        <!--<permission tag="ElButton"
                                    class="barcode-delete"
                                    type="danger"
                                    size="mini"
                                    icon="close"
                                    @click="box_close(3, file)"
                                    :route="apis.url_delete_allocation_attachment"></permission>-->
                    </div>
                </el-col>
                <!--<el-col :span="4">
                    <el-button type="primary" size="mini"
                               v-if="trendAction.status === 0"
                               @click="upload_file('barcode_list')">上传商品条码
                    </el-button>
                </el-col>-->
                <el-col :span="4">
                    <el-button class="ml-sm" type="primary" size="mini" @click="barcode_file">导出</el-button>
                </el-col>
            </el-row>
        </div>
        <div>
            ...
            <el-row v-else>
                ...
                <el-col :span="4">
                    <el-button class="ml-sm" type="primary" size="mini" @click="download_file">导出</el-button>
                </el-col>
            </el-row>
        </div>
        <div>
            <h3 class="t-left">物流面单(仅仓库)：</h3>
            <el-row v-if="shipping_label.length !== 0">
                <el-col :span="20">
                    <el-tag class="mr-xs mb-xs"
                            type="primary"
                            v-for="(file, index) in shipping_label"
                            :key="index">{{file.name}}
                    </el-tag>
                </el-col>
                <!--<el-col :span="4">
                ...
                <el-col :span="4">
                    <el-button class="ml-sm" type="primary" size="mini" @click="download_shipping">导出</el-button>
                </el-col>
            </el-row>
        </div>
        <div>
            <h3 class="t-left">箱唛附件(仅销售)：</h3>
            <el-button v-if="box_title.length === 0"
                       type="primary" size="middle"
                       @click="upload_title('box_title')">上传附件
            </el-button>
            <el-row v-else>
                <el-col :span="20">
                    <el-tag class="mr-xs mb-xs"
                ...
                <el-col :span="4">
                    <el-button class="ml-sm" type="primary" size="mini" @click="download_titles">导出</el-button>
                </el-col>
            </el-row>
        </div>
        <div>
            ...
            <el-row v-else>
                ...
                <el-col :span="4">
                    <el-button class="ml-sm" type="primary" size="mini" @click="download_other">导出</el-button>
                </el-col>
            </el-row>
        </div>
        <div slot="footer">
        ...
        <import-file v-model="importVisible"
...
<style lang="stylus">
    .c-allocation-attachment {
        .barcode-delete.el-button--mini {
            padding: 3px;
            border-radius: 50%;
        }
    }
...
<script>
    ...
    import {
        api_get_allocation_attachment,
        api_delete_allocation_attachment,
        url_delete_allocation_attachment
    ...
    export default {
        ...
        methods: {
            ...
            init() {
                this.$http(api_get_allocation_attachment, this.trendAction.id).then(res => {
                    ...
                    this.shipping_label = res.shipping_label_attachment;
                }).catch(code => {
            ...
            upload(type, data) {
                ...
                this.shipping_label = data.shipping_label_attachment;
            ...
            download_shipping() {
                let url = `${config.apiHost}allocation/attachment`;
                this.shipping_label.forEach(title => {
                    downloadFile({
                        url: url,
                        get: {full_name: title.path},
                        fileName: title.name,
        ...
        components: {
            ...
            importFile: require('./allocation-picking/import-file.vue').default,
        }
    }
</script>