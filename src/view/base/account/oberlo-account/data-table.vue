<template>
    <div class="c-data-table">
        <el-table v-resize="{height:41}"
            ...
            <el-table-column label="Oberlo状态" align="center" inline-template>
            ...
            <el-table-column label="操作" inline-template align="center" width="260">
                <div>
                    <permission
                            tag="span"
                            :route="apis.url_get_oberlo"
                            class="operate"
                            @click="look_over(row)">查看
                    </permission>
                    <permission tag="span"
                                :route="apis.url_oberlo_update">&nbsp;|&nbsp;
                    </permission>
                    <permission
                            tag="span"
                            :route="apis.url_oberlo_update"
                            class="operate"
                            @click="edit_update(row)">编辑
                    </permission>
                    <permission
                            tag="span"
                            :route="apis.url_oberlo_authorize">&nbsp;|&nbsp;
                    </permission>
                    <permission
                            tag="span"
                            :route="apis.url_oberlo_authorize"
                    ...
                    <permission
                        tag="span"
                        :route="apis.url_oberlo_account_log">|</permission>
                    <permission tag="span"
                                :route="apis.url_oberlo_account_log"
                                class="operate"
...
<script>
    import {url_get_oberlo,url_oberlo_update,url_oberlo_authorize,url_oberlo_account_log,api_change_status} from '@/api/oberlo-account';
    export default {
        permission: {
            url_get_oberlo,
            url_oberlo_update,
            url_oberlo_authorize,
            url_oberlo_account_log
        },
        data() {
            return {
                status: ["启用", "停用"],
        ...
        methods:{
            ...
            look_over(row){//---查看
                this.title = `查看`;
                this.$emit('look-over',row,this.title);
            },
            edit_update(row){//---编辑
                this.title = `编辑`;
                this.$emit('look-over',row,this.title);
