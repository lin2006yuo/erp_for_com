...
let routeMaps = require('@/routes/maps').default
let pack = require('../../routes/pack').default
...
export default {
    ...
    state: {
        ...
        navMax: 10,
        leftOpen: true,
        triggerMenu: getItem('triggerMenu', 'hover'),
    ...
    mutations: {
        ...
        async nav_open(state, router) {
            ...
            route.query = {};
            if (splitPath[1]) {
                route.query = string2query(splitPath[1]);
            }
    ...
    getters: {
        ...
        navs_cur(state) {
            return (cur) =>{
                cur = cur === undefined ? state.navIndex : cur;
                return state.navItems[cur];
            }
        },
        navs_index(state) {
