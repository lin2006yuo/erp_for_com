<template>
    <div class="c-data-table">
        <el-table v-resize="{height:41}"
            ...
            <el-table-column label="Fummart状态" align="center" inline-template>
                <div :class="parseInt(row.is_invalid)===0?'loseStatus':'sucStatus'">
                    {{row.is_invalid|filterStatus}}
                </div>
            </el-table-column>
            ...
            <el-table-column label="操作" inline-template align="center" width="260">
                <div>
                    <permission
                            tag="span"
                            :route="apis.url_get_funmart"
                            class="operate"
                            @click="look_over(row)">查看
                    </permission>
                    <permission tag="span"
                                :route="apis.url_funmart_account">&nbsp;|&nbsp;
                    </permission>
                    <permission
                            tag="span"
                            :route="apis.url_funmart_account"
                    ...
                    <permission
                            tag="span"
                            :route="apis.url_funmart_authorize">&nbsp;|&nbsp;
                    </permission>
                    <permission
                            tag="span"
                            :route="apis.url_funmart_authorize"
                    ...
                    <permission tag="span"
                                :route="apis.url_funmart_account_log">&nbsp;|&nbsp;
                    </permission>
                    <permission
                        tag="span"
                        :route="apis.url_funmart_account_log"
...
<script>
    import {url_funmart_account,url_funmart_authorize,url_get_funmart,api_funmart_status,url_funmart_account_log} from '@/api/funmart-account';
    export default {
        permission: {
            url_get_funmart,
            url_funmart_account,
            url_funmart_authorize,
            url_funmart_account_log,
        },
        data() {
            return {
                status: ["启用", "停用"],
        ...
        methods:{
            row_key(row) {
                return row && row.base_account_id;
            },
            ...
            changStatus(row) {//-----------------------修改启用或禁用事件
                ...
                this.$confirm('您将' + statusTitle + '，确认此操作吗？', '提示', {
                    ...
                    this.$http(api_funmart_status, params).then(res => {
