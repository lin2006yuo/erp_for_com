<template>
    <div class="mt-sm mb-sm">
        <div class="inline">
            <el-button type="primary" size="mini" @click="delete_batches">{{$t('ymx-list.deleteBatches')}}</el-button>
            ...
            <el-button type="primary" size="mini" @click="derive">{{$t('ymx-list.derive')}}</el-button>
        </div>
        <el-select class="width-lg inline" v-model="selectButton" :placeholder="$t('ymx-list.moreEdit')">
            <el-option v-for="item in buttonArr" :key="item.value" :label="item.label" :value="item.value" @click.native="batch_handle(item)"></el-option>
        </el-select>
...
import {api_amazon_listing_grab} from '@/api/heel-sale-complaint-management'
    ...
    export default{
        ...
        methods: {
            ...
            //导出
            derive(){
                this.$emit('listing-export');
            },
            //批量删除
            delete_batches(){
                this.$emit('delete-batches');
            }
        },
        computed: {
            judge(){
