        ...
        <search-card @search="search" :params="searchData" :clears="clears">
            <div class="mb-mini">
                ...
                <div class="inline width-sm">
                    <super-select v-sf.account_id
                                  v-model="searchData.salesperson_id"
                                  storageKey="aliexpress-salesperson-id"
                                  :adjustStructure="adjustSalespersonId"
                                  :remoteRequestHttp="httpSalespersonId">
                    </super-select>
                </div>
            ...
            <label-item label="是否虚拟仓发货：">
                <el-select
                    v-model="searchData.virtual_send"
                    class="inline width-sm">
                    <el-option
                        v-for="item in virtual_warehouse_list"
                        :label="item.label"
                        :value="item.value"
                        :key="item.value">
                    </el-option>
            ...
            <label-item label="侵权平台：" class="ml-xs">
                <el-select
...
<script>
    ...
    import {api_get_channel,} from '@/api/order-local.js';
        ...
        data(){
            return {
                ...
                /*tortList:[
                    ...
                    {label:"速卖通平台",value:4},
                    {label:"CD平台",value:5},
                    {label:"Lazada平台",value:6},
                    {label:"Joom平台",value:7},
                    {label:"MyMall平台",value:8},
                    {label:"Shopee平台",value:9},
                ]*/
                tortList:[]
            }
        },
        ...
        methods:{
            ...
            get_channel(){
                this.$http(api_get_channel,{}).then(res=>{
                        this.tortList=[{label:this.$t('ymx-list.pleaseChoose'),value:""},{label:this.$t('ymx-list.all'),value:0},...res]
                    }).catch(err=>{
                        this.$message({type:"error",message:err.message||err});
                    })
            },
            adjustStructure(data){
                return {
                    data:data.map(val=>({label:val.label,value:val.id})),
                    count:data.length||0
                }
            ...
            adjustSalespersonId(data){
                return {
                    data:data.map(val=>({label:val.realname,value:val.id})),
                    count:data.length||0
                }

            },
            httpSalespersonId(){
                return http(Url2("get|user/:type/staffs",{type:'sales'}),{content:''})
            },
            //获取销售员
            sale_fix_result(res){
                return  res.map(row=>{
