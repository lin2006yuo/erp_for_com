        ...
        <rainbow-header title="物流设置" background-color="#ABCD05">
            <div class="fr mt-mini mr-sm" slot="header-right">
                <super-select class="inline width-lg"
                              ...
                              storage-key="ebayTransListDetail">
                </super-select>
...
<script>
    import {api_get_transform,api_get_trans,api_save_common_trans,api_save_combList,api_get_countrys,api_get_combList,api_batch_sync,
        api_get_vat_info,api_edit_combList,api_save_common_model} from '@/api/ebay-kandeng-public-module';
            ...
            change_trans(val){
                ...
                let params = {
                 ...
                 this.$http(api_edit_combList,params).then(res=>{
                     ...
                     this.form.set.international_shipping =[];
                     let trans = res.trans.filter(row=>{return parseInt(row.inter)===1});
                     let transin = res.trans.filter(row=>{return parseInt(row.inter)===0});
...
//                         this.form.set.international_shipping = cur1;
            ...
            set_international_shipping(row){
                ...
                if(row.location==='Worldwide'||row.location[0]==='Worldwide'){
                    obj.selectAll = true;
...
//                if(Array.length<=1)return this.$message({type:"warning",message:"现有数据不允许删除"});
            ...
            get_shipping(){
                ...
                this.$http(api_get_transform,{site:this.form.list.site}).then(res=>{
                    ...
                    this.outtransportList=this.outtransportList.map(res=>{
                        return {
                            label:res.description,
                            value:res.shipping_service
                        }
...
//            --------------------------------------------------------------- 不送达地区
            ...
            select_model(val){
                ...
                this.$http(api_edit_combList,params).then(res=>{
                    let cur = (res!=='null'&&!!res.exclude)?res.exclude:'[]';
