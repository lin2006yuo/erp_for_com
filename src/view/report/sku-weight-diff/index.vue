        ...
        export default{
            ...
            methods:{
                ...
                deal_time(searchData){
                    ...
                    let cur = curString.split('\n').filter(row => !!row);
                    data.snText = cur && cur.length > 0 ? cur.map(row => row.trim()) : data.snText;
                    return data;
                },
                selectionChange(val) {
                    this.selectList = val;                    
                ...
                init () {
                    let data = this.deal_time(this.searchData);
                    if(data.snText.length && data.snText.length > 300) return this.$message({ type: 'warning', message: '批量搜索只支持300条数据！' })
                    this.loading = true;
                    if (data.is_auto_update_weight === '') {
                        delete data.is_auto_update_weight
                    }
