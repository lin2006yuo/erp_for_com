        ...
        <table class="template" v-for="(item, index) in formData">
            <tr>
                <td style="width:300px;text-align: right">
                    <span v-if="item.record_number">{{item.record_number}}: </span>
                    <span><a class="link" :href="item.channel_item_link" target="_blank">{{item.channel_item_id}}</a></span>
                </td>
                <td>
                    <div class="inline fl t-left" v-if="channel_name==='ebay'&&item.comment_type">
                        <p class="inline t-left ml-sm">
                            <img src="../../../assets/good.png" :title="`好评：${item.comment_type.comment_text}`" v-if="item.comment_type.comment_type===1">
                            <img src="../../../assets/centre.png" :title="`中评：${item.comment_type.comment_text}`" v-else-if="item.comment_type.comment_type===2">
                            <img src="../../../assets/bad.png" :title="`差评：${item.comment_type.comment_text}`" v-else-if="item.comment_type.comment_type===3">
                            <img src="../../../assets/stay.png" v-else-if="item.comment_type.comment_type===0" title="等待买家评价">
                        </p>
                        <p class="inline t-left ml-sm">
                            <img src="../../../assets/wait.png" v-if="item.comment_type.status===0" title="未评价">
                            <img src="../../../assets/yet.png" :title="item.comment_type.leaved_comment_text" v-else>
                        </p>
                    </div>
                    <div class="inline fl t-left" v-if="channel_name==='aliExpress'&&item.comment_type">
                        <p class="inline t-left ml-sm">
                            <img src="../../../assets/good.png" :title="`好评：${item.comment_type.buyer_feedback}`" v-if="item.comment_type.buyer_evaluation==='4'||item.comment_type.buyer_evaluation==='5'">
                            <img src="../../../assets/centre.png" :title="`中评：${item.comment_type.buyer_feedback}`" v-if="item.comment_type.buyer_evaluation==='3'">
                            <img src="../../../assets/bad.png" :title="`差评：${item.comment_type.buyer_feedback}`" v-if="item.comment_type.buyer_evaluation==='1'||item.comment_type.buyer_evaluation==='2'">
                            <img src="../../../assets/stay.png" v-if="item.comment_type.buyer_evaluation==='0'" title="等待买家评价">
                        </p>
                        <p class="inline t-left ml-sm">
                            <img src="../../../assets/yet.png" v-if="item.comment_type.status===2" :title="`已回评：${item.comment_type.evaluate_content}`">
                            <img src="../../../assets/wait.png" v-else :title="item.comment_type.status|filterCommentTitle">
                        </p>
                    </div>
