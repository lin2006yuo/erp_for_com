            ...
            <el-form :model="singleData" :rules="rules" ref="forms" label-width="159px">
                ...
                <el-form-item label="引用类似产品：" >
                    <!--<el-select v-model="spuCate"
                               ...
                               @input="(val)=>{change_same_spu(val)}"
                               placeholder="输入搜索更多..."
                               style="width: 200px;">
                        <el-option
                            v-for="(item,i) in spu_cate_options"
                            :key="i"
                            :label="item.goods_spu"
                            :value="item.id">
                        </el-option>
                    </el-select>-->
                    <super-select v-model="spuCate"
                                  class="inline"
                                  :storageKey="`aliexpress-product-code-${singleData.account_id}`"
                                  @change="(val)=>{change_same_spu(val)}"
                                  style="width:200px"
                                  :options="singleData.same_spus">
                    </super-select>
                ...
                <el-form-item label="产品分类：" required prop="category_id">
                    ...
                    <!--选择平台分类产品-->
                    <category-dialog v-model="categoryShow" :site=false channel="aliexpress" :accountId="singleData.account_id" @submit="submit"></category-dialog>
                </el-form-item>
                <div v-if="singleData.attr_info&&singleData.attr_info.length>0" class="attrs mb-sm">
                    ...
                    <el-form  label-width="160px" >
                        ...
                        <el-form-item  :label="`${item.names_zh}：`"
                            ...
                            <el-select v-model="item.value"
                                       ...
                                       class="inline"
                                       style="width:200px"
                                       v-if="item.attribute_show_type_value==='list_box'"
                                       @change="attr_changes(item)">
                                <el-option
                                    :key="i"
                            ...
                            <el-input v-model="item.value" style="width:200px"
                                      :maxlength="70"
                                      class="inline"
                                      v-if="item.attribute_show_type_value==='input'"
                                      @blur="validateData(item)">
                            </el-input>
                            <span v-if="item.attribute_show_type_value==='input'">（{{item.value|filterLength}}/70）</span>
                    ...
                    <div class="ml-lg">
                        ...
                        <el-row  v-for="(item,attIndex) in  singleData.attr_info"
                            ...
                            <el-input v-model="item.name" :maxlength="40" style="width:200px"  class="inline"  ></el-input>
                            <div class="inline"  style="width:60px"><span>（{{item.name|filterLength}}/40）</span></div>
                            <el-input v-model="item.value" :maxlength="70" style="width:200px"  class="inline"  ></el-input>
                            <div class="inline" style="width:60px"><span>（{{item.value|filterLength}}/70）</span></div>
                            <div class="inline"><el-button type="text" size="mini" @click="del_attr(attIndex)">删除</el-button></div>
                        </el-row>
                    </div>
                ...
                <el-form-item label="发货仓库："  >
                    <!--<el-select v-model="singleData.warehouse_id"
                               placeholder="输入搜索更多..."
                               class="inline"
                               clearable
                               style="width: 200px;"
                        ...
                        <el-option
                            v-for="(item,i) in warehouse_options"
                            :key="item.id"
                            :label="item.name"
                            :value="item.id">
                        </el-option>
                    </el-select>-->
                    <super-select v-model="singleData.warehouse_id"
                                  class="inline"
                                  storageKey="aliexpress-warehouse-code"
                                  style="width:200px"
                ...
                <el-form-item label="是否虚拟仓发货：">
                    <el-radio-group v-model="singleData.virtual_send">
...
<script>
    ...
    export default{
        ...
        created(){
            // this.spu_cate_options=this.singleData.same_spus.slice(0,30);
        },
        mounted(){
            this.$http(api_aliexpress_depot).then(res=>{
                this.depot=res;
                // this.warehouse_options = res.slice(0,30);
                res.forEach(val=>{
                    this.$set(val,"label",val.name);
                    this.$set(val,"value",val.id);
                });
                this.warehouse_options = res;
                if(!this.singleData.warehouse_id){
                    this.singleData.warehouse_id=res[0].id;     // 发货仓库不存在时去默认第一个值
                }
            }).catch(error=>{
        ...
        methods: {
            validateData(item){    // 验证属性列表不能存在中文标点字符
                let reg =  /[，。、？！*·；：【】‘”……（）《》{}]/gi;
            ...
            remoteMethodSpu(query){
                if (query !== '') {
                    setTimeout(() => {
                        this.spu_cate_options = this.singleData.same_spus.filter(item => {
                            return item.goods_spu.toLowerCase().indexOf(query.toLowerCase()) > -1;
                        });
                    }, 200);
                } else {
                    this.spu_cate_options = this.singleData.same_spus.slice(0,30);
            ...
            remoteMethod(query){
                if(typeof query ==="string"){
                    if(query!==""){
                        setTimeout(()=>{
                            this.warehouse_options = this.depot.filter(item=>{
                                return item.name.toLowerCase().indexOf(query.toLowerCase()) > -1;
                            })
                        },200)
                    }else{
                        this.warehouse_options = this.depot.slice(0,30);
                    }
                }
            ...
            get_spu_attr(val){
                ...
                this.$http(api_get_same_spu_attr, data).then(sku=> {
                    if(!sku) { return;}
                    this.organize_data(sku,true);   // 处理接口数据
                }).catch(code=>{
...
//                                ali.custom_pic = "";
                ...
                this.$set(this.singleData,'is_market_image',sku.is_market_image);
                this.$set(this.singleData,"market_images",sku.market_images || []);
            ...
            add_attr(){
                ...
                if(i<=19){
                	...
                	this.$message({type:"warning", message: "最多添加20个属性值"});
                }
            },
            // 删除自定义属性
            del_attr(index){
        ...
        filters: {
            ...
            filterLength(val){
                if(!val&&val!==0){
                    ...
                    return　String(val).split("").length
                }
            }
        },
        watch: {
