<!--
    说明: 封装super-select组件，提供获取数据的能力，根据传入类型，获取不同的数据，可缓存数据，
          目前支持的类型：
          |type                       |name
          |---------------------------|-------------------------
          |developer                  |开发员
          ...
          |account                    |账号简称*
          |site                       |站点*
          |warehouse                  |仓库 中山仓 --- 冠通W0058
          |warehouse2                 |仓库 中山仓 --- jumia香港代发仓
          |warehouse_all              |仓库 所有
          |warehouse_gucang           |仓库 谷仓仓库
    ...
    使用：
      ...
      扩展：
          在switch语句中新增一条case即可（注意：api返回的数据必须为一个数组，否则修改get_list方法），
          case返回一个对象
            {
                api: 调用数据的接口
...
<script>
    ...
    import {api_arehouse_gucang} from '@/api/overseas-package-return' //获取仓库 谷仓仓库
    export default {
        data() {
            return {
                options: []
        ...
        computed: {
           server() {
               switch(this.type) {
                   ...
                   //获取仓库 谷仓仓库
                   case 'warehouse_gucang':
                       return {
                           api: api_arehouse_gucang,
                           handle: (i) => ({
                               value: i.id,
                               label: i.name
                           }),
                           key: 'ra-warehouse-gucang'
                       }
               }
           },
            innerDisabled() {
                switch (this.type) {
                    case 'account':
                        if(this.options && this.options.length) return this.disabled
                        else return true
                    case 'site':
                        if(this.options && this.options.length) return this.disabled
                        else return true
            ...
            innerPlaceholder() {
                switch (this.type) {
                    case 'account':
                        if(this.options && this.options.length) return this.placeholder
                        else {
                            if(this.param.channel_id) {
                                return '该平台下暂无站点'
                            } else {
                                return '请先选择平台'
                            }
