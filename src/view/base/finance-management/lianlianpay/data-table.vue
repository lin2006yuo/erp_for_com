    ...
    <div class="c-data-table">
        <el-table v-resize="{height:41}"
            ...
            <el-table-column
                    ...
                    label="收款账号">
                <div>
                    <ui-tip :content="row.lianlian_account" :width="98" v-copy></ui-tip>
                </div>
            </el-table-column>
            <el-table-column
                    prop="lianlian_name"
            ...
            <el-table-column label="操作" inline-template align="center" width="150">
                <div>
                    <permission
                            tag="span"
                            :route="apis.url_get_lianlianpay_edit"
                            class="operate"
                    ...
                    <permission
                            tag="span"
                            :route="apis.url_lianlianpay_save"
                            class="operate"
...
<script>
    import {url_get_lianlianpay_edit,url_lianlianpay_save,api_lianlianpay_status} from '@/api/lianlianpay';
    export default {
        permission: {
            url_get_lianlianpay_edit,
            url_lianlianpay_save,
        },
        data() {
            return {
                status: ["启用", "停用"],
        ...
        methods:{
            changStatus(row) {//-----------------------修改启用或禁用事件
                ...
                this.$confirm('您将' + statusTitle + '，确认此操作吗？', '提示', {
                    ...
                    this.$http(api_lianlianpay_status,row.id,status).then(res => {
