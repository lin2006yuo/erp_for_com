<template>
    <page class="p-index.vue">
        ...
        <el-row class="mb-xs mt-xs">
            ...
            <permission tag="ElButton" type="primary" size="mini" class="ml-sm"
                        :route="apis.url_get_twice_picker"
                        @click="twice_operator('more')">
                多品分拣人设置
            </permission>
            <permission tag="ElButton" type="primary" size="mini" class="ml-sm"
                        :route="apis.url_get_twice_picker"
                        @click="twice_operator('remark')">
                含备注分拣人设置
            </permission>
        </el-row>
        <data-table
        ...
        <add-edit v-model="dialogShow"
                  :app-type-data="appTypeData"
                  :edit-id="editId"
                  ...
                  :title="title"></add-edit>
        <twice-operator v-model="twiceVisible" :operator-info="operatorInfo" :action="action"></twice-operator>
    </page>
</template>
<style lang="stylus">
...
<script>
    ...
    import {
        warehouse_area,
        add_warehouse_area,
        api_edit_warehouse_area,
        url_add_warehouse_area,
        url_get_twice_picker,
    ...
    export default {
        ...
        methods: {
            ...
            init() {
                ...
                this.$http(warehouse_area, params).then(res => {
                    ...
                    this.loading = false;
                    this.$refs.search.firstLoading = false
                }).catch(code => {
                    this.$message({
                        type: 'error',
                        message: code.message || code
            ...
            twice_operator(data) {
                this.operatorInfo = {
                    warehouse_name: this.warehouses.find(row => row.value === this.searchData.warehouse_id).label,
                    warehouse_id: this.searchData.warehouse_id,
                };
                if(data === 'more'){
                    this.action = {
            ...
            edit_operator(val, id) {
                ...
                this.warehouseData.category_ids = val.category_lists.map(res => res.category_id);
                this.warehouseData.category_lists = val.category_lists.map(res => res.category_name.replace('>', '>>'));
                this.title = `编辑仓库分区: ${val.name} 管理员信息`;
            },
            add_data(val) {
                let data = {
                    code: '',
        ...
        components: {
            ...
            twiceOperator: require('./twice-operator.vue').default,
        }
    }
</script>
