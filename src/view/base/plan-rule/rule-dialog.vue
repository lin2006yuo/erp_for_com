<template>
    <page-dialog v-model="show" size="large" @open="open" :title="`${title}${name}`"  @change="change_dialog" @close="close">
            ...
            <el-form>
                <el-row>
                    ...
                    <el-form-item class="inline ml-sm">
                        <label class="mr-mini inline">采购阶段：</label>
                        <el-select v-model="rule_type" class="inline width-lg" @change="change_stage">
                ...
                <el-row>
                    <el-form-item prop="start_time" class="inline">
...
<script>
    ...
    export default{
        ...
        methods:{
            change_stage(val) {
                if(this.mdfid ===0) {
                    this.$http(api_rule_items, {rule_type: val}).then(res=>{
                        this.rules = res;
                    });
                }
            },
            ...
            open(){
                this.rule_type = this.new_rule_type;
                if(this.mdfid!==0){
                    this.approversOpts = [];
...
//                            this.showSelect = true;
            ...
            validators,
            init(){
                this.$http(api_rule_items, {rule_type: this.new_rule_type}).then(res=>{
                    this.rules = res;
                });
            },
            clear(){
