    ...
    <div class="p-parcel-returned-manage">
        <div class="inline mb-xs ml-sm">
            ...
            <permission tag="ElButton"
                        :route="apis.url_package_return_import"
                        class="inline ml-sm"
                        type="primary"
                        size="mini"
                        @click.native="importShow = true">批量导入</permission>
        </div>
        ...
        <!--批量导入弹框-->
        <import-dialog v-model="importShow" @batch-import="batch_import"></import-dialog>
        <look-allocation :edit-data="lookData"
                         v-model="lookVisible"
...
<script>
    ...
    import {
        ...
        api_package_return_import,
        api_package_return_save_import,
        api_package_return_download_template,
        url_package_return_import,
...
//        	批量导出、全部导出
            ...
            parcel_information(row) {
                if(row.return_type === '调拨退回'){
                    this.trendAction = {
                        title: `查看调拨计划(${row.package_number})`,
                    ...
                    let data = {
                        order_code: row.package_number
                    };
                    this.$http(api_look_allocation, data).then(res => {
                        this.trendAction.id = res.id;
                        this.lookData = res;
        ...
        components: {
            ...
            lookAllocation: require('../allocation-manage/allocation-plan/look-allocation').default
        },
    }
</script>
