<template>
    <el-row class="p-attribute-set-wish">
        ...
        <div class="p-titleColour-attribute">
            ...
            <el-row v-if="tableData.sku_attr_info.length===0">
                <p style="color:#ff0000">注意：所选产品分类仅支持单属性产品，仅能添加一个sku</p>
            </el-row>
            <!--SKU列表-->
            <el-row>
                ...
                <tr style="background: #ccc;">
                    ...
                    <template v-for="attr in tableData.sku_attr_info" v-if="attr.is_checked">
                        <td>
                            ...
                            <el-input v-model="attr.change_value"
                                      @input="(value)=>{change_customize(value)}"
                                      @blur="(value)=>{validateData(value,attr)}"
                                      class="inline width-sm"
                                      v-if="attr.customized_name&&attr.customized_name===1"
                                      :maxlength="20">
                            </el-input>
                            <span v-if="attr.customized_name&&attr.customized_name===1">({{!!attr.change_value?attr.change_value.length:0}}/20)</span>
                    ...
                    <template v-if="!hasDelivery">
                        <td>
                            <ui-limit-number v-model="price"
                                             ...
                                             class="inline"
                                             :limit="2"
                                             @blur="change_all_price"></ui-limit-number>
                            <span class="btn-icon" @click="init_rule_price"></span>
                        </td>
                        <td>
                            <integer-input v-model="count" :min="0"
                                           class="inline"
                ...
                <tr v-for="(row,row_i) in tableData.listing_info" :key="row_i+`${row_i.id?row_i.id:''}`">
                    ...
                    <template v-for="(attr,attr_i) in tableData.sku_attr_info">
                        <td v-if="attr.is_checked">
                            ...
                            <span v-if="attr.customized_name&&attr.customized_name===1">({{!!get_custom_name(row,attr)?String(get_custom_name(row,attr)).length:0}}/20)</span>
...
//            },
            ...
            change_customize(newVal){
                let data = this.commont_change();   // 获取勾选的sku 数组
            ...
            validateData(e,attr){
                ...
                if(str.length>0){
                    this.$message({type:"error",message:`${attr.names_zh} 属性中存在中文字符 ${str}`})
                }
            },
            ...
            /*手动获取刊登定价规则*/
            init_rule_price(){
                let find = this.tableData.listing_info.find(row=>!!Number(row.retail_price));
                if(find){ //列表中价格一列有数据时,在覆盖之前给提示
                    this.$confirm(`此操作将覆盖已填写的零售价信息，是否确定继续?`,'提示',{
                        confirmButtonText:"确定",
                        cancelButtonText:"取消",
                        type:"warning"
                    }).then(()=>{
            ...
            get_rule_price(){
                let skuIdList = this.tableData.listing_info.map(item=>item.goods_sku_id);
                let params = {
                    channel_id:4,
                    account_id:this.tableData.account_id,
                    site_code:0,
                ...
                this.$http(api_pricing_rules,params).then(res=>{
                    /*无法匹配规则时给提示*/
                    if(Object.values(res).find(row=>row.hasOwnProperty('error')))return this.$message({type:'error',message:'无法匹配规则'});
                    this.tableData.listing_info = this.tableData.listing_info.map(row=>{
                        this.$set(row,'retail_price',this.arrangement_price(res,row.goods_sku_id));
                        return row;
                    });
                    this.$message({type:'success',message:'刊登定价获取成功'})
                }).catch(code=>{
                    this.$message({type:'error',message:code.message||code})
                })
            },
            /*整理定价规则数据，并返回
            * */
            arrangement_price(PricingRules,skuId){
                let totalPrice = '';
