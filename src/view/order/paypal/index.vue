...
<script>
    ...
    import {orderPaypal_list,api_post_paypal_orders_sync,api_paypal_transaction_status,url_post_paypal_orders_sync} from '../../../api/order-paypal'
    ...
    export default{
        ...
        methods: {
            ...
            get_buttons_data(){
                this.$http(api_paypal_transaction_status).then(res=>{
                    for(let key in res){
                        this.buttonsData.push({name:res[key],status:key})
                    }
                    this.buttonsData.unshift({name:"全部",status:''});
                })
            },
            sync_paypal_order(){
                ...
                this.$http(api_post_paypal_orders_sync,{sync_param:JSON.stringify(this.selectList.map(row=>{return {account:row.paypal_account_id,txn_id:row.txn_id}}))}).then(res=>{
            ...
            search_list(){
                ...
                if(this.searchData.date_e){
                    data.date_e = datef("YYYY-MM-dd HH:mm:ss",this.searchData.date_e/1000)
                }else{
                    data.date_e=''
                }
                this.loading=true;
