<template>
    ...
    <div class="c-table-data">
        <div class="relative">
            ...
            <!--使用request-button提供的request接口，识别请求成功后取消loading-->
            <permission tag="request-button"
                        :request="change_packages_number"
                        req-key="api_batch_package_number"
                        class="inline ml-sm"
                        :timeout="0"
                        :route="apis.url_batch_package_number">更换包裹号
            </permission>
            <permission tag="request-button"
                        :request="sync_logistics"
                        req-key="api_sync_logistics"
                        class="inline ml-sm"
                        v-show="syncBtn.sign_btn"
                        :timeout="0"
                        :route="apis.url_sync_logistics">同步物流商发货
            </permission>
            <permission tag="request-button"
                        :request="sync_sign"
                        req-key="api_sync_sign"
                        class="inline ml-sm"
                        :timeout="0"
                        v-show="syncBtn.logistics_btn"
                        :route="apis.url_sync_sign">同步签收状态
            </permission>
            <color-tip></color-tip>
            <div class="inline fr">
        ...
        <preview-label v-model="declareVisible" :label-info="declareInfo"></preview-label>
    </div>
</template>
<style lang="stylus">
    .c-table-data {
...
<script>
    ...
    import {
        ...
        url_sync_sign,
        api_sync_sign,
        url_batch_package_number,
        api_batch_package_number,
        api_declare_view,
        url_declare_view,
    } from '@/api/packages';
    ...
    export default {
        ...
        methods: {
            ...
            m_option(row) {
                let buttons = [
                    {label: '打印面单', value: 1, apis: url_post_print, disabled: true},  // type=1为 包裹列表
                ...
                buttons.push({label: '测试打印自制面单', value: 5, apis: url_test_custom_print, disabled: false});

                Number(row.invoice_can_print) === 0 ? buttons.push({
                        label: '查看发票',
                        value: 3,
                        apis: url_print_invoice,
                        disabled: true
                    }) :
                    buttons.push({
                        label: '查看发票',
                ...
                if (row.is_can_declare) {
                    buttons.push({
                        label: '预览报关面单',
                        value: 6,
                        apis: url_declare_view,
                        disabled: false
                    });
                }
            ...
            sync_logistics() {
                if (this.selectList.length === 0) {
                    this.$reqKey(`api_sync_logistics`, false);
                    this.$message({
                        type: 'warning', message: '请选择要同步的包裹'
                    })
                    return
                }
                //收集选中的id
                const package_ids = this.selectList.map(v => {
                    return v.id
                });
                return this.$http(api_sync_logistics, {package_ids}).then(res => {
                    if (res.message === '更新成功') {
                        this.$message({type: 'success', message: '同步物流状态成功'});
                        return Promise.resolve()
                    } else {
                        return Promise.reject('同步物流状态失败')
                    }
                })
            },
            //同步签收状态
            sync_sign() {
                if (this.selectList.length === 0) {
                    this.$reqKey(`api_sync_sign`, false);
                    this.$message({
                        type: 'warning', message: '请选择要同步的包裹'
                    });
                    return
                ...
                return this.$http(api_sync_sign, {package_ids}).then(res => {
                    if (res.message === '更新成功') {
                        this.$message({type: 'success', message: '同步签收状态成功'});
                        return Promise.resolve()
                    } else {
                        return Promise.reject('同步签收状态失败')
                    }
                })
            },
            // 批量更换包裹号
            change_packages_number() {
                if (this.selectList.length === 0) {
                    this.$reqKey(`api_sync_sign`, false);
                    this.$message({
                        type: 'warning', message: '请选择要更换的包裹号'
                    });
                    return
                }
                const package_ids = this.selectList.map(v => {
                ...
                return this.$http(api_batch_package_number, {package_ids}).then(res => {
                    if (res.message === '更新成功') {
            ...
            preview_label(row) {
                ...
                let content = '';
                this.$http(api_declare_view, row.id, data).then(res => {
                    this.declareInfo = {
                        content: res.content,
                        title: `预览报关面单：(${row.number})`,
                        file_name: `${row.order_number}.png`,
                    };
        ...
        props: {
            ...
            syncBtn: {
                type: Object,
                default: {logistics_btn: false, sign_btn: false}
            }
        },
        components: {
            ...
            previewLabel: require('./preview-label.vue').default,
        }
    }
</script>
