...
<script>
    export default {
        ...
        methods:{
            ...
            get_select_links(){
                console.log('get_select_links');
                this.selectLinks.forEach(item=>{
                    if(item.value!==0){
                        if(item.value === 27){
                            let find = this.buttonList.find(row=>{
                                let first_letter = row.name && row.name.slice(0,1)||row.label && row.label.slice(0,1)||row.code && row.code.slice(0,1);
            ...
            queryMore(){
                this.buttonList.forEach(row=>{
                    ...
                    find&&(this.$set(row,'showSearch',true));
                });
                this.query();
            },
            selectCheck(item){
