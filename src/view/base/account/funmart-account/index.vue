<template>
    <page class="p-index">
        ...
        <permission tag="ElButton"
                    ...
                    class="mt-xs"
                    :route="apis.url_funmart_batch_set">批量设置</permission>
        <data-table
...
<script>
    import {api_funmart_account,api_funmart_account_log,url_funmart_batch_set,api_funmart_batch_set} from '@/api/funmart-account';
    export default{
        permission:{
            url_funmart_batch_set
        },
        refresh(){
        ...
        methods:{
            ...
            setClich(val){
                ...
                this.$http(api_funmart_batch_set,params).then(res=>{
                    ...
                    setTimeout(() => {
                        this.$reqKey('funmart_downloadAdd',false);
                    }, 300)
                })
            },
            ...
            init(status){
                ...
                this.loading = true;
                this.$http(api_funmart_account,data).then(row=>{
            ...
            look_over(row){//---查看
                this.title = `查看账号 ${row.account_name} 信息`;
                this.listData = {
                    code: row.code,
                    id: row.id,
            ...
            edit(row){
                this.title = `编辑账号 ${row.account_name} 信息`;
                this.listData = {
                    code: row.code,
                    id: row.id,
                    account_name:row.account_name,
            ...
            log_form(row){//---日志
                this.$http(api_funmart_account_log,row.id).then(res => {
