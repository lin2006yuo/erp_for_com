    ...
    <page-dialog v-model="show" @open="clear" size="large" title="修改可售数量" :close-on-click-modal="false">
         <div>
            ...
            <p>
             ...
             <div>
                 <label class="has-inline text-right" style="width: 120px;"> 本地SKU：</label>
                 <order-input v-model="skuValue"
                              style="width:30%"
...
<script>
    import {
        api_edit_ae_stock,api_sku_code_search
        ...
        methods:{
            ...
            set_stock_num(){
                ...
                this.selectArr.forEach(it=>{
                    this.showTables.forEach(val=>{
                        if(it.sku===val.sku){
                            this.$set(val,"stock",Number(this.setStock))
                        }
                    });
                });
                return true;
...
//                let flag=this.set_stock_num();
                ...
                this.$http(api_edit_ae_stock,params).then(res=>{
                    ...
                    if(!!this.setStock){
                        this.$emit('change-data', Number(this.setStock));
                    }
                    this.show=false;
                }).catch(code=>{
                    this.$message({
            ...
            searchSku(){
                let skuValue = window.clone(this.skuValue);
                let arr = skuValue.replace(/[\n\s\r]/mg,',').split(',').filter(val=>!!val);
                let product_ids_arr=this.showTables.filter(row=>row.isCheck).map(val=>{return val.product_id});
                let data = {
                    product_ids:String([... new Set(product_ids_arr)]),
                    sku:String(arr)
                };
                this.$http(api_sku_code_search,data).then(res=>{
                    res.forEach(val=>{
                        this.$set(val,"isCheck",true);
                        this.$set(val,"stock",val.old_stock);
                    });
                    this.showTables = res;
                }).catch(error=>{
