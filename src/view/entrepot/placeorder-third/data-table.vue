    ...
    <el-row class="shipment-data-table">
        <mailing-method ref="mailing" @left-show-change="$listeners['left-show-change']">
            ...
            <div slot="button-list">
                ...
                <el-button @click="reback" size="mini" class="ml-sm" type="" :disabled="uploadStatus === 1 || uploadStatus === 2">释放包裹</el-button>
                ...
                <request-button
                    v-show="orderVovaBtn"
                    @click.native="upload_vova"
                    req-key="uploadVova"
                    :timeout="0"
                >批量vova线上下单</request-button>
                <request-button
                    v-show="uploadVovaBtn"
                    @click.native="order_vova"
                    req-key="orderVova"
                    :timeout="0"
                >批量获取跟踪号</request-button>
            </div>
            <!-- 头部右侧-->
            ...
            <el-table slot="goods-tabel"
                ...
                <el-table-column v-if="uploadStatus!==0 || this.channelId === 12" label="跟踪号" prop="shipping_number"></el-table-column>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            get_templates() {
                this.$http(api_goods_export_template, {type: 155}).then(res => {
                    res.forEach(row => {
                        row.value = row.name;
                    });
                    this.templates = res;
            ...
            create_excel(param) {
                ...
                switch (this.export_type) {
                    case 0://部分
                        ...
                        Object.assign(data,this.$parent.init_params());
            ...
            order_vova() {
                this.$emit('order-vova')
            },
            upload_vova() {
                this.$emit('upload-vova')
            }
        },
        computed: {
            treeCount() {
