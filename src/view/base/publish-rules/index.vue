...
<script>
    import {api_get_pricing_list, api_getchannel, api_account_list, api_pricing_sort, api_get_user, api_get_option} from '../../../api/publish-rules'
    export default {
        page:{
            devinfo:{
                frontEnd:'张明亮,吴楚光',
        ...
        methods: {
            remoteRequestHttp() {
                ...
                this.$http(api_get_option,params).then(res => {
                    this.options = res.data.map(item => {
                        return {
                            label: item.company_name || item.realname,
                            value: item.id
                        }
                    });
                });
