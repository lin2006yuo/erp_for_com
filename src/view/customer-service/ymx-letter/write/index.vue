...
<!--                    </el-select>-->
                    <super-select
                        ...
                        v-model="sendData.customer_id"
                        :disabled="!sendData.account_id"
                        :placeholder="all"
                        storageKey="api_amazon_write_customer_ymx_letter"
                        :options="customers"
                    >
                    </super-select>
                </td>
    ...
    export default{
        ...
        mounted(){
            if(window.objlength(this.$page.param)!==0){
                this.load(this.$page.param.ordersNumber);
            }
        },
        methods: {
            get_team(){
            ...
            change_account(){
                ...
                if(!!this.sendData.account_id) {
                    this.$http(api_amazon_write_customer, {account_id: this.sendData.account_id}).then(res => {
                        ...
                        this.customers = res.data.map((res)=> {
                            return {
                                label: `${res.realname}`,
                                value: res.customer_id
                            }
                        })
                        // this.customers.unshift({label:'全部', username:'All', value:''})
                    }).catch(code => {
                        this.$message({type: "error", message: code.message || code})
                    })
                }
