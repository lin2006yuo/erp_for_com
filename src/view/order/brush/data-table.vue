...
<script>
    ...
    import {api_order_product,api_get_speed,api_cancel_invalid,api_virtual_tracking_synchronize} from '../../../api/order-local';
    export default{
        data(){
            return{
                mdfid:0,
            ...
            action(data,options){
                ...
                switch(options.value){
                    ...
                    case 3:
                        ...
                        this.oldChannelId=data.channel_id;
                        this.demand_virtual_tracking(data.id,data);
                        break;
                    case 4:
...
//            取消作废
            ...
            /*查询虚拟跟踪号,是否有保存*/
            demand_virtual_tracking(id){
                this.$http(api_virtual_tracking_synchronize,id).then(res=>{
                    this.brushForm.synchronize_shipping_id=res.data.synchronize_shipping_id?res.data.synchronize_shipping_id:'';
                    this.brushForm.synchronize_tracking_number=res.data.synchronize_tracking_number?res.data.synchronize_tracking_number:'';
                    if (!res.data.synchronize_shipping_time) {
                        this.brushForm.synchronize_shipping_time=new Date();
                    }else {
                        this.brushForm.synchronize_shipping_time=new Date(res.data.synchronize_shipping_time*1000);
                    }
