<template>
    <page-dialog class="p-import-excel" title="Excel导入移库" v-model="visible" size="large" :close-on-click-modal="false">
        <div>
            <div class="file-btn">
                选取文件
...
<script>
    import {api_post_import_shift} from '@/api/warehouse-cargo-shift';
    ...
    export default {
        ...
        methods: {
            ...
            submit(file, data) {
                ...
                let api = api_post_import_shift;
                this.$http(api, data).then(res => {
                    this.fullscreenLoading = false;
                    if (res.failure === 0) {
                        this.files[index].result = `导入文件成功!`;
                        this.$message({type: 'success', message: `导入成功，成功${res.success}个，失败${res.failure}个`});
                        this.visible = false;
                    } else {
                        if (res.failure > 0) {
                            this.$message({type: 'error', message: `导入有失败，成功${res.success}个，失败${res.failure}个`});
                            this.files[index].result = "导入有失败！";
                            this.files[index].errors = res.remark;
                        }
                    }
                }).catch(code => {
                    this.fullscreenLoading = false;
            ...
            file_change() {
                ...
                Object.keys(files).forEach(key => {
                    ...
                    if ((/\.(?!(xlsx$|xls$))/.test(file.name))) {
                        ...
                        this.files = [{file: file, result: '', errors: []}];
                    }
                });
                //手动清掉input value
