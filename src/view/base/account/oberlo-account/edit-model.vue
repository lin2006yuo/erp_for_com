<template>
    <page-dialog :title="title" v-model="show" size="small" @open="open" @close="close" :close-on-cilck-modal="false">
        <el-form :model="listData"
            ...
            <el-form-item label="抓取Oberlo订单功能：">
                <el-col :span="14">
            ...
            <el-form-item label="同步发货状态到Oberlo功能：">
                <el-col :span="14">
...
<script>
    import {api_oberlo_update} from '@/api/oberlo-account';
    export default{
        data(){
            return{
                show:this.value,
                rules:{
                    ...
                    name:[
                        {required: true, message: '账户名称为必填项', trigger: 'change' }
                    ],
                    site:[
        ...
        methods:{
            ...
            keep(){
                this.$refs.listData.validate(valid=>{
                    if(valid){
                        this.$http(api_oberlo_update,this.listData).then(res=>{
