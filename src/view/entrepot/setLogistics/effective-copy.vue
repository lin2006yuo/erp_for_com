                    ...
                    <rainbow-header title="全球可达(相同费用)" background-color="#EB5405">
                        ...
                        <el-form :model="item" label-width="90px">
                            ...
                            <el-form-item label="起重：" v-if="item.is_volumn_weight">
                                ...
                                <span>单边长：</span><el-input :disabled="!editable" v-number:fixed="2" v-model="effData.volume_length" class="inline input-width"></el-input><span> cm</span>&nbsp;
                                ...
                                <label-item label="计抛方式：" class="ml-sm">
                                    <div class="relative throw_type width-lg">
                                        ...
                                        <el-popover placement="right"
                                            ...
                                            <div>
                                                <p class="bold-font">记抛方式的公式：</p>
                                                <p>实际重量A,体积重量B</p>
                                                <p>计半抛：(A+B)/2</p>
                                                <p>免三分之一抛：（B-A)/3*2+A</p>
                                                <p>超过实重30%的：B- A*30%</p>
                                                <p class="red">只能三选一，如果3个都不选，则按照体积重的计抛条件计算</p>
                                            </div>
                                        </el-popover>
                                    </div>
                                </label-item>
                                ...
                                <el-card>
                                    ...
                                    <el-form :model="curDetail2" label-width="90px">
                                        ...
                                        <el-form-item label="起重：" v-if="curDetail2.is_volumn_weight">
                                            <el-input  :disabled="!editable" v-number:int  v-model="curDetail2.volume_min_weight" class="inline input-width"></el-input><span> g</span>&nbsp;
                                            <span>实重与材积差异倍数：</span><el-input :disabled="!editable" v-number:fixed="2" v-model="curDetail2.weight_multiple" class="inline input-width"></el-input><span> 倍</span>&nbsp;
                                            <span>单边长：</span><el-input :disabled="!editable" v-number:fixed="2" v-model="curDetail2.volume_length" class="inline input-width"></el-input><span> cm</span>&nbsp;
                                            <span>材积(kg)=长(cm)*宽(cm)*高(cm)÷</span>
                                            <integer-input v-model="curDetail2.volumn_weight"
                                                           :min="1"
                                                           :disabled="!editable" class="inline input-width">
                                        ...
                                        <!-- 偏远费设置 -->
                                        <district-fee class="district-fee" v-show="curDetail2.is_remote === 1" :navIndex="curIndex" :remote="curDetail2.remote" :locations="curDetail2.locations" :editable="editable"/>
                                        <el-form-item label-width="10px" v-if="curDetail2.is_stage_fee">
                                            <table>
                                                <tr v-for="(sub,i) in curDetail2.stages" :key="i">
                                                    <td>分组{{i+1}}</td>
...
//                this.curTitle = item.label;
            ...
            get_country(index, arry) {
                let det = "details_" + this.effData.arriveable_type;
                this.curDetail2.locations.splice(0);
                arry.forEach(row => {
                    this.curDetail2.locations.push(Object.assign({is_remote: 0}, row));
                });
            },
            change_status() {
