<template>
    <page class="p-index">
        ...
        <search-card @search="search" @init-params="clear" :params="paramsData" :clears="clears"
            ...
            <label-item label="单价：" class="inline ml-sm">
                <el-input v-sf.per_cost_start type="number" @keyup.enter.native="search" v-model="paramsData.per_cost_start"
                          class="width-xs inline enter-result"></el-input>
                <span>~</span>
                <el-input v-sf.per_cost_end   type="number" @keyup.enter.native="search" v-model="paramsData.per_cost_end"
                          class="width-xs inline mr-sm enter-result"></el-input>
            </label-item>
        </search-card>
        ...
        <apply-stock v-model="applyVisible"
                     :select-sku="selectData"
                     :warehouse_id="warehouse_id" @search="() => {}"></apply-stock>
    </page>
</template>
<style lang="stylus">
    .warehouse-reminder {
...
<script>
    import {api_get_developer_list} from "@/api/warehouse-cargo";
...
//                        {label:'商品移库',value:1,apis:url_local_shif},
        ...
        methods: {
            get_staffs(){
               this.$http(api_get_developer_list).then(res => {
                  this.developerList = res.map(row => {
                       return {
                           value: row.id,
                           label: row.realname
                       }
        ...
        computed: {
            ...
            isFBA() {
                return [489,187].includes(this.warehouse_id);
            },
            warehouse_name() {
                if(this.warehouses) {
