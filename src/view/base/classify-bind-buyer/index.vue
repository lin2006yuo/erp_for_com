        ...
        <search-card @search="init" @enter='init' :params="form" :clears="clears">
            <label-item label="分类：">
                <!-- <el-autocomplete
                        ...
                        icon="search"
                ></el-autocomplete> -->
                <super-select   v-model="form.classify"
                                storageKey="classifySelect"
                                :options="classfyData"
                                :placeholder="'请输入子类'"
                                 v-sf.classify>
                </super-select>
            ...
            <label-item label="采购员：" class="ml-sm mr-sm">
                <!-- <el-autocomplete
                        ...
                        icon="search"
                ></el-autocomplete> -->
                <super-select   v-model="form.buyer"
                                storageKey="buyerSelect"
                                :options="buyerData"
                                :placeholder="'请输入采购员'"
                                v-sf.buyer>
                </super-select>
...
<script>
    ...
    export default{
        ...
        methods:{
            ...
            // },
            get_classify(){
                // return api_product_category({subclass:1});
                this.$http(api_product_category,{subclass:1})
                    .then(res=>{
                        res=res.map(item=>{
                            return {label:item.name,value:item.id};
                        })
                        console.log("数据:",res);
                        this.classfyData=res;
                    })
            },
            ...
            get_buyer(){
                // return api_get_seller();
                this.$http(api_get_seller)
                    .then(res=>{
                         res=res.map(item=>{
