<template>
    <search-card @search="search" :params="searchData" :clears="clears">
        ...
        <label-item label="账号简称：" class="inline">
            ...
            <super-select
                v-model="searchData.account_id" class="inline width-xs"
                filterable
                clearable
                ref="leader"
                storage-key="wishManageSearch"
...
<script>
    import {api_wish_acount, api_acount_simple} from '@/api/monitor-wish'
    export default {
        name: "manageSearch",
        data() {
            return {
        ...
        methods: {
            ...
            getAccount() {
               this.$http(api_acount_simple).then(res => {
