            ...
            <template v-for="(data, index) in tableData">
                <tr :class="[data.show ? 'active' : '',data.heighLight?'active-color':'',data.lock_update===2?'redborder':'']"  @click="addClass(index)"  :key="`${data.id}`">
                    ...
                    <td style="position: relative">
                        ...
                        <div class="icon-wrap">
                            <i v-if="data.tort_platform===1" class="pointer tort-icon" @click="showTort(data)">侵权</i>
                    ...
                    <td>
                        <div>
                            ...
                            <div>
                                <times v-if="hasCreateTime(data)" :time="data.create_time"></times>
                                <label v-else>-- --</label>
                            </div>
                        </div>
                    </td>
                ...
                <template v-if="data.show">
                    <tr style="font-weight:bold" :key="`@{data.id}${index}`" class="variant">
...
<script>
    ...
    export default{
        ...
        methods:{
            showTort(row){
                this.tortShow = true;
                this.tortTitle = `${row.goods_spu} 侵权记录`;
