...
<script>
    //未发货
    import {api_get_unshipped, api_get_unshipped_export} from '@/api/warehouse-data-record'
    //获得本地仓库
    ...
    export default {
        ...
        methods: {
            batch_report() {
                ...
                return this.$http(api_get_unshipped_export, params, header).then(res => {
                    if(res.join_queue === 1) {
                        this.visible = true;
                        this.$message({
                            type: 'success',
            ...
            init(flag) {
                ...
                this.loading = true;
                if(flag) {
                    this.$http(api_get_unshipped, params).then(res => {
